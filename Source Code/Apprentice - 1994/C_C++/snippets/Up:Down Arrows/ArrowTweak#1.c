...
pascal Boolean DialogFilter(DialogPtr theDialog, EventRecord *theEvent, short *theItem)
{
	...
	switch (theEvent->what) 
	{
		...
		case mouseDown:
			...
			plainArrowPict = GetPicture(140); 	//¥ Both arrows, no hit.
			upArrowPict = GetPicture(141);		//¥ Both, top arrow hit.
			downArrowPict = GetPicture(142);	//¥ Both, bottom hit.
			...
			upArrowRect.bottom -= 9;	//¥ The bottom is 9 pixels up.
			downArrowRect = iRect;		//¥ dawnArrow is iRect, too.
			downArrowRect.top += 9;		//¥ The top is 9 pixels down.
...
main()
{
	...
	if ((theDialog = GetNewDialog(140, NIL, (WindowPtr)-1L)) == NIL) {
		SysBeep(1);
		ExitToShell();
	}
