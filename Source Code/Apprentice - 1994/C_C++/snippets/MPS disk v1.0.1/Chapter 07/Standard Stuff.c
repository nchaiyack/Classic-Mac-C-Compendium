	...
	switch ( part ) {
		...
		case inZoomOut:
			if (TrackBox(theWindow, event->where, part)) {
				...
				DoZoomWindow(theWindow, part, TRUE);				/* +++ */
...
void AdjustMenus()
{
	...
	if (gFirstWindow != NIL) {										/* +++ */
		EnableItem(menu, iTileWindows);								/* +++ */
		EnableItem(menu, iStackWindows);							/* +++ */
	} else {														/* +++ */
		DisableItem(menu, iTileWindows);							/* +++ */
		DisableItem(menu, iStackWindows);							/* +++ */
	}																/* +++ */
}
		...
		case mWindows:												/* +++ */
			switch (menuItem) {										/* +++ */
				...
				case iLine31:										/* +++ */
					break;											/* +++ */
				default:											/* +++ */
					DoSelectFromWindowsMenu(menuItem);				/* +++ */
					break;											/* +++ */
			}														/* +++ */
			break;													/* +++ */
	}
