...
Boolean		gInModalState;		/*	Set to true whenever we want to treat the
									frontmost window modally. This Boolean
									alters our mouse-down handling routine and
									our menu setup routine. +++ */
...
void MainEventLoop()
{
	...
	while ( !gQuit ) {
		...
		//
		// Note: all of the stuff from here to the check of "gotEvent"
		// is just so we can handle modeless dialogs. It involves some
		// pretty hairy logic and, even so, still doesn't handle things
		// like hitting <Return> or <Command-.>. In light of this, some
		// people recomment _against_ using the Dialog Manager for
		// modeless dialogs, and suggest that you simple handle dialogs
		// just as you would any other window. In an extreme, yet
		// superlative, example of this, MacApp implements all of its
		// modal and modeless dialogs using its own code.
		//
		// Systems before 7.0 would corrupt low-memory if you called
		// IsDialogEvent() without any open windows.
		//
...
/*******************************************************************************
	...
	+++ Note our additional check for gModalState in this routine. This is
	+++ one of the few places we have to make this check in order to support
	+++ movable-modal windows (or even plain old modal dialogs if we wanted
	+++ to manage them by hand). The other place would be in our menu setup
	+++ routines.
