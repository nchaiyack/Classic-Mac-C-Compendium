...
/*******************************************************************************
	...
	When they arenÕt needed, they are simply hidden. One advantage to this is
	...
	semi-modal state in that they canÕt select any other windows in the
	...
	ÒgInModalState.Ó When that variable is set to TRUE, two things happen.
...
/*******************************************************************************
	...
	items to their Òcommand-key-is-not-downÓ state.
...
/*******************************************************************************
	...
	This routine is passed to ModalDialog as a Òfilter procedureÓ. This means
	...
	If the user presses a key, weÕd like to take a look at it and see if we
	...
	ÒitemHitÓ to the ID number of the item we want to click on.
	...
	roundrect with corner radii of 16 pixels. However, if youÕve ever followed
	that advice for really big buttons, youÕll see that the result looks
	really dumb. This is because the buttons themselves are not always drawn
	with a 16 pixel radius, which means that the button and frame donÕt always
	...
	button and divide it by 2. However, we donÕt let the value for the radius
	...
	done for that particular event, and that it doesnÕt need to do anything
	...
	TRUE because we donÕt want the Dialog Manager to do what it would normally
...
/*******************************************************************************
	...
	If we havenÕt already done so, create the modeless dialog. Next, show the
...
/*******************************************************************************
	...
	that indicates what weÕre doing. Once all thatÕs done, we show the window
...
/*******************************************************************************
	...
	is currently set. If they are the same, we donÕt need to do anything.
	...
	currently selected. If thatÕs the case, there are no buttons to be turned
...
/*******************************************************************************
	...
	GetDItem. Returned in the ÒkindÓ parameter is a number that identifies
	...
	This is to take care of Òexcessive flashingÓ as Online Companion puts it.
	...
	controlÕs title in the ControlRecord, and reshows the control by calling
	ShowControl. At this point, the control is properly shown on the screen
	with its correct, new title.

	However, thereÕs a little time bomb lurking in the works. When HideControl
...
/*******************************************************************************
	...
	off state (sometimes called the Òbeing given the birdÓ state). The state
	the we want the items to be in is passed in the ÒflippedOnÓ parameter.

	We decide which items to change and how to change them by using a couple
	of ÔSTR#Õ resources. Each of these resources contains one string for each
	of the items in the dialog box. The first ÔSTR#Õ resource contains the
	strings for the items when they are in the off state. The other ÔSTR#Õ
	contains the strings that get shown when the items are in the on state.

	It doesnÕt make sense to change the text of all items. For example, the
	icon doesnÕt have any text to change. Also, the OK has only one state. To
	handle items that donÕt change, we enter zero-length strings in their
	place in the ÔSTR#Õ resource. When we come to one of those strings, we
	just skip over that item.

	Once weÕve changed the text of all the affected items, we make a note of
	the state weÕve left them in. This way, if we call this routine to put the
	items into State A, and theyÕre already in State A, there is nothing to
	do, and we just skip out of the function.

	By the way, take a look at how we determine how many strings there are in
	the ÔSTR#Õ resources. The format of a ÔSTR#Õ resource is as follows:
	...
	two ÔSTR#Õ resources and the number of items in the dialog. In our
	example, we have 26 items in the dialog and 26 strings in each ÔSTR#Õ
...
/*******************************************************************************
	...
	NULL. We have our own copy so that we donÕt have to link with ANSI.
...
/*******************************************************************************
	...
	Convert a lower case letter to upper case. If weÕre running under System
