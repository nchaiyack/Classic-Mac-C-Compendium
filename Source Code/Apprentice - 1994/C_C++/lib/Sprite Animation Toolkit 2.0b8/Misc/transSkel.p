...
{They include some bug fix and hiearcical menu support.}
...
{Filter procedure, Added by Ingemar 18/9 -93:}
			...
			begin
{Kolla om FrontWindow har whFilter!}
				dh := WHandlerHnd(GetDHandler(FrontWindow));
				filtered := false;
				hasfilter := dh <> nil;
				if hasfilter then
					hasfilter := dh^^.whFilter <> nil;
				if hasfilter then
					callpfilter(FrontWindow, theEvent, filtered, dh^^.whFilter);
				DoDialog := filtered;
