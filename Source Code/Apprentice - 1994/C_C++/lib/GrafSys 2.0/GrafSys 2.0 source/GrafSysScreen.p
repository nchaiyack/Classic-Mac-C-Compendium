...
{currentBuf^[offset].transformedZ := z;}
			if current3DPort^.projection = parallel then begin
				currentBuf^[offset].Screenx := Trunc(x) + current3DPort^.center.h;
				currentBuf^[offset].Screeny := -Trunc(y) + current3DPort^.center.v
			end
			else begin
				zbyd := 1 / (z / current3DPort^.d + 1);
				currentBuf^[offset].Screenx := Trunc(x * zbyd) + current3DPort^.center.h; (* do perspective transformation *)
				currentBuf^[offset].Screeny := -Trunc(y * zbyd) + current3DPort^.center.v;
			end;
