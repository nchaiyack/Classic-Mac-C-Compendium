...
{endV := theScrnObj^.Point[ep];       (* now we have start & endpoint for clipping in 3D *)
		...
		if ((s[3] <= 0) and (e[3] <= 0)) or ((s[3] > 0) and (e[3] > 0)) then
			begin
				if ((s[3] <= 0) and (e[3] <= 0)) then (* no line is drawn *)
					skipThis := TRUE
				else
					begin (* whole line can be drawn, transfer it to the line buffer *)
...
(* now we have to perspective-project the intersection point *)
				if current3Dport^.projection = perspective then
					begin
						zbyd := 1 / (d[3] / thed + 1);
...
{          the original																		}
	...
	begin
		fIndex := fIndex - 1;
		if (fIndex < 0) or (fIndex > numPoints) then
			begin
		...
		if (tIndex < 0) or (tIndex > numPoints) then
			begin
				ErrorCode := cIllegalPointIndex;
				AddLine := -1;
				Exit(AddLine);
	...
	begin
		if (LineIndex > numLines) or (LineIndex < 0) then
			begin
				ErrorCode := cIllegalLineIndex;
				DeleteLine := FALSE;
				Exit(DeleteLine);
...
(* look if a point is referenced by any of the lines *)
		hasRef := FALSE;
		lineIndex := 1;
		while not hasRef and (lineIndex <= numLines) do
			begin
...
(* do clipping *)
				...
				case clipMode of
					...
					(* clipped to the projection plane                                                         	*)
									if newLine or clippedThis then
										begin
...
(* first, set the current color to black *)
		...
		if Autoerase or UseBounds then
			begin
...
(* first, set the current color to black *)
	...
	(* now begin drawing all lines of the object *)
		...
		while index <= screenBufNumLines do
			begin
