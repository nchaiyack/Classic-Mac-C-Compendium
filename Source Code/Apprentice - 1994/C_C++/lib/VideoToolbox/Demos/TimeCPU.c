...
void TimeCPU(void)
{
	...
	if(quickDraw>=gestalt8BitQD){
		...
		ffprintf(o,"%7.0f	µs	SetPixelsQuickly(,,,,%d);// %d-bit pixels\n",s*1e6
		...
		s=StopTimerSecs(timer)/n-overhead;
		ffprintf(o,"%7.0f	µs	GetPixelsQuickly(,,,,%d);// %d-bit pixels\n",s*1e6
			,rowLength,(**(**device).gdPMap).pixelSize);
		n*=10;
	...
	s=StopTimerSecs(timer)/n-overhead;
	ffprintf(o,"%7.2f	µs	*paL++=*pbL++;		// long, memory to memory\n",s*1e6);
	...
	if(quickDraw>=gestalt8BitQD){
		...
		if(device!=NULL){
			...
			if(paL!=NULL){
				...
				ffprintf(o,"%7.2f	µs	*paL++=*pbL++;		// long, memory to video memory\n",s*1e6);
			}
		}
	}
	DisposPtr(buffer);
	...
	if(1){			// time long arithmetic
		...
		s=StopTimerSecs(timer)/n-overhead;
		ffprintf(o,"%7.2f	µs	jL=kL;			// long, register to register\n",s*1e6);
	...
	if(1){				// time short arithmetic
		...
		s=StopTimerSecs(timer)/n-overhead;
		ffprintf(o,"%7.2f	µs	jH=kH;			// short, register to register\n",s*1e6);
	...
	if(1){					// time Fixed
		...
		s=StopTimerSecs(timer)/n-overhead;
		ffprintf(o,"%7.1f	µs	xF=FixMul(yF,zF);	// Fixed\n",s*1e6);
		...
		s=StopTimerSecs(timer)/n-overhead;
		ffprintf(o,"%7.1f	µs	xF=FixDiv(yF,zF);\n",s*1e6);
		...
		s=StopTimerSecs(timer)/n-overhead;
		ffprintf(o,"%7.1f	µs	xF=FixRatio(123,1234);\n",s*1e6);
		...
		s=StopTimerSecs(timer)/n-overhead;
		ffprintf(o,"%7.1f	µs	xF=DoubleToFix(y);\n",s*1e6);
		...
		s=StopTimerSecs(timer)/n-overhead;
		ffprintf(o,"%7.1f	µs	x=FixToDouble(xF);\n",s*1e6);
	}
	...
	if(1){				// time rand()
		...
		DisposPtr(buffer);
		ffprintf(o,"%7.1f	ms	RandFill(,%ld);	// i.e. %4.1f µs/byte\n"
			,s*1e3,n,s*1e6/n);
		n/=10;
		...
		s=StopTimerSecs(timer)/n-overhead;
		ffprintf(o,"%7.1f	µs	i=randU();		// i.e. %4.1f µs/byte\n",s*1e6, s*1e6/2.);
		...
		s=StopTimerSecs(timer)/n-overhead;
		ffprintf(o,"%7.1f	µs	i=rand(); 		// i.e. %4.1f µs/byte\n",s*1e6, s*1e6/1.);
		...
		s=StopTimerSecs(timer)/n-overhead;
		ffprintf(o,"%7.1f	µs	i=nrand(128);\n",s*1e6);
		...
		s=StopTimerSecs(timer)/n-overhead;
		ffprintf(o,"%7.1f	µs	elems68k(&x,FRANDX);	// i.e. x=Randomx()\n",s*1e6);
		n*=5;
	}
	DisposeTimer(timer);
