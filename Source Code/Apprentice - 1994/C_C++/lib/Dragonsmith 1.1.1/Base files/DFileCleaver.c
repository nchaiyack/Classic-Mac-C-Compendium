/*
	...
	NOTE:	This sub-Dragon is in development.  I wrote it to do a very specific task: split up dragonsmith10b2.hqx
			...
			There's a lot of room for improvement Ñ preferences handling in particular.  You might notice that it's being
	...
	Copyright © 1992Ð1994 by Paul M. Hoffman
...
#define	cNIL			'\0'
#define	cCOLON		':'
#define	cELLIPSIS	'É'		// Yes, that's one char (option-semicolon)
...
#define	rAskSegSize		130		// ID of the "Segment SizeÉ" dialog
...
void DFileCleaver::ReadPrefs (void)
{
	...
	if (strategy != NULL) {
		...
		segmentSize = (*strategy)->segSize;		// PMH Ñ we ignore everything else in the struct!!
	}
	askForSpecs = SplitSpecsInvalid ();		// PMH Ñ get askForSpecs from the prefs file?
}
...
void DFileCleaver::DoOptionsMenu (short itemNum)
{
	switch (itemNum) {
		case iSplitAt:
			// PMH Ñ we need to put something here (obviously)
			break;
		case iSegmentSize:
			(void) AskForSplitSpecs ();	// PMH Ñ need to rearrange, rename methods
			break;
		default:
			break;
	}
...
OSErr DFileCleaver::SplitFile (FSSpec *srcFile)
{
	...
	if (PBDataForkSize (curDocPB) <= segmentSize)
		return noErr;		// PMH Ñ or should we return an error?
			
	err = FSpOpenDF (srcFile, fsRdPerm, &srcRefNum);
	if (err == noErr) {
		bufferSize = segmentSize;
		if (!ok) return -1;		// PMH Ñ impromptu error code
		
		splitType = 0;			// PMH Ñ hard-coded garbage
		if (!ok) return -1;		// PMH Ñ impromptu error code
...
OSErr DFileCleaver::ExtractSegFile (FSSpec *srcFile, short srcRefNum, short segNum)
{
	...
	destFile.vRefNum = srcFile->vRefNum;		// PMH Ñ leave segments in the same place
	...
	if (err == noErr) {
		...
		if (err == noErr) {
			...
			if (copyErr == noErr || copyErr == eofErr) {
				...
				(void) FlushVol (NULL, destFile.vRefNum);		// Ignore any errors Ñ what would we do??
			}
		}
		if (err != noErr)
// BEGIN bug-fix 1.1.1 Ñ Change pointer to the destination file's refnum to a pointer to its FSSpec!!
...
void DFileCleaver::MakeSegFileName (unsigned char *sourceName, short segNum)
{
	...
	unsigned short	len;
	unsigned char		partString [] = "\p-pt";	// PMH Ñ hard-coded string
...
OSErr DFileCleaver::CopyBetweenFiles (short source, short dest)
{
	...
	if (splitType == -1)								// PMH Ñ hard-coded garbage
		return CopyByTextMatch (source, dest);		// PMH Ñ change this?
	else
...
{
	return TRUE;	// PMH Ñ always replace
}
...
Boolean DFileCleaver::SplitSpecsInvalid (void)
{
	switch (splitType) {
		case kSplitByNumSegs:
			return TRUE;
			break;
		case kSplitBySegSize:			// PMH Ñ we only do kSplitBySegSize
			if (segmentSize < 1)
				return TRUE;
			break;
...
Boolean DFileCleaver::SplitFailed (FSSpec *sourceFss, OSErr err)
{
	...
	return FALSE;		// PMH Ñ I'd really like to put something more sophisticated hereÉ
}
