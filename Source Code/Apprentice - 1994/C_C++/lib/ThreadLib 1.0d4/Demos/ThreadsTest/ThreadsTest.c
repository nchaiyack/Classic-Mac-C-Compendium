...
void main(void)
{
	long threadsAttr;
		...
		if (MacHasTMON()) {
			/* use TMON */
			DebugStr((StringPtr) "\pªtraps /check=1");
			DebugStr((StringPtr) "\pªtraps /purge=1");
			DebugStr((StringPtr) "\pªtraps /scramble=1");
			DebugStr((StringPtr) "\pªtraps set discipline heap ..:inThisapp");
		}
		else {
			/* assume MacsBug */
	...
	if (! gQuit) {
		...
		/* Execute debugger commands to disable the debug options installed
			on startup. */
		DebugStr((StringPtr) "\pªtraps clear ..//; dsca off; atc; g");
	#endif /* DEBUGGER_CHECKS */
}
