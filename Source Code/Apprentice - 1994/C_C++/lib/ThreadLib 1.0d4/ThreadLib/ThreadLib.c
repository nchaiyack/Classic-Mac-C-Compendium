...
/*	ÄThreadError returns the last error that occurred, or noErr if the last
...
/*	ÄThreadCount returns the number of threads in the queue. */
...
/*	ÄThreadMain returns the main thread, or THREAD_NONE if there are no
	threads. */
...
/*	ÄThreadActive returns the currently active thread, or THREAD_NONE if
...
/*	ÄThreadFirst returns the first thread in the queue of threads, or
...
/*	ÄThreadNext returns the next thread in the circular queue of threads. */
...
/*	ÄThreadStatus returns the status of the thread as set with ThreadStatusSet.
...
/*	ÄThreadStatusSet sets the status code for the thread. It is the
...
/* ÄThreadData returns the data field of the thread. The application can
...
/* ÄThreadDataSet sets the data field of the thread. The application can
	use the thread's data field for its own purposes. */
...
/*	ÄThreadStackMinimum returns the recommended minimum stack size for
...
/*	ÄThreadStackDefault returns the default stack size for a thread. This
...
/*	ÄThreadStackSpace returns the amount of stack space remaining in the
...
/*	ÄThreadStackFrame returns information about the specified thread's
...
/* ÄThreadSleepSet sets the amount of time that the specified thread will
...
/*	ÄThreadSchedule returns the next thread to activate. Threads are maintained
...
/*	ÄThreadActivate activates the specified thread. The context switch is
...
/*	ÄThreadYield activates the next scheduled thread as determined by
...
/*	ÄThreadYieldInterval returns the maximum time till the next call to
...
/*	ÄThreadEnd removes the thread from the queue and disposes of the memory
...
/*	ÄThreadBeginMain creates the main application thread and returns the main
...
/*	ÄThreadBegin creates a new thread and returns the thread's serial number.
