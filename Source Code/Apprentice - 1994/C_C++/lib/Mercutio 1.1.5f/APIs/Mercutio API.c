...
long	GetMDEFVersion (MenuHandle menu)
{
	...
	SetPt(&pt,0,0);
	((MDEFProc) *proc)(getVersionMsg, menu, &dummyRect, pt, &dummyInt);
...
StringHandle	GetMDEFCopyright (MenuHandle menu)
{
	...
	SetPt(&pt,0,0);
	((MDEFProc) *proc)(getCopyrightMsg, menu, &dummyRect, pt, &dummyInt);
	HSetState(proc, state);
...
Boolean IsCustomMenu (MenuHandle menu)
{
	...
	SetPt(&pt,0,0);
	((MDEFProc) *proc)(areYouCustomMsg, menu, &dummyRect, pt, &dummyInt);
	HSetState(proc, state);
...
long PowerMenuKey (long theMessage, short theModifiers, MenuHandle hMenu)
{
	...
	else
	{
		...
		((MDEFProc) *proc)(mMenuKeyMsg, hMenu, &dummyRect, pt, &theModifiers);
		HSetState(proc, state);
		return( _Point2Long(topLeft(dummyRect)));
	}
}
