...
void								DisposeTextStorage(TextStorageRec* Storage)
	{
		...
		for (Scan = 0; Scan < Limit; Scan += 1)
			{
				ReleasePtr(ArrayGetElement(Storage->LineMap,Scan));
			}
		DisposeArray(Storage->LineMap);
...
char*								TextStorageGetLineCopy(TextStorageRec* Storage, long LineIndex)
	{
		...
		Copy = CopyPtr(ArrayGetElement(Storage->LineMap,LineIndex));
		if (Copy != NIL)
			{
...
char*								TextStorageGetActualLine(TextStorageRec* Storage, long LineIndex)
	{
		...
		ERROR((LineIndex < 0) || (LineIndex >= TextStorageGetLineCount(Storage)),
			PRERR(ForceAbort,"TextStorageGetLine:  Line index out of range"));
		return ArrayGetElement(Storage->LineMap,LineIndex);
	}
...
long								TextStorageGetLineLength(TextStorageRec* Storage, long LineIndex)
	{
		...
		return PtrSize(ArrayGetElement(Storage->LineMap,LineIndex));
	}
...
MyBoolean						TextStorageChangeLine(TextStorageRec* Storage, long LineIndex,
											char* NewLine)
	{
		...
		ReleasePtr(ArrayGetElement(Storage->LineMap,LineIndex));
...
MyBoolean						TextStorageBreakLine(TextStorageRec* Storage, long LineIndex,
											long CharIndex)
	{
		...
		OldLine = ArrayGetElement(Storage->LineMap,LineIndex);
...
MyBoolean						TextStorageFoldLines(TextStorageRec* Storage, long LineIndex)
	{
		...
		FirstLine = ArrayGetElement(Storage->LineMap,LineIndex);
		SecondLine = ArrayGetElement(Storage->LineMap,LineIndex + 1);
		...
		if (StartLine == EndLine)
			{
				...
				/* special case */
				Line = ArrayGetElement(Storage->LineMap,StartLine);
		 ...
		 else
			{
				...
				for (LineScan = StartLine + 1; LineScan <= EndLine - 1; LineScan += 1)
					{
						...
						if (!TextStorageChangeLine(Copy,TargetLineTracker,
							ArrayGetElement(Storage->LineMap,LineScan)))
							{
								goto FailurePoint2;
							}
						TargetLineTracker += 1;
				...
				/* munch last line */
				Line = ArrayGetElement(Storage->LineMap,EndLine);
											...
											long StartLine, long StartChar, long EndLine, long EndChar)
	{
		...
		if (StartLine == EndLine)
		 ...
		 else
			{
				...
				/* munch last line */
				Line = ArrayGetElement(Storage->LineMap,StartLine + 1); /* line after */
		...
		for (Scan = 0; Scan < TotalNumLines; Scan += 1)
			{
				...
				LineCopy = CopyPtr(ArrayGetElement(Stuff->LineMap,Scan));
				if (LineCopy == NIL)
					{
					 FailurePoint1:
						return False;
		...
		for (Scan = 0; Scan < Limit; Scan += 1)
			{
				...
				DataTemp = ArrayGetElement(Storage->LineMap,Scan);
				CheckPtrExistence(DataTemp);
				if (WriteToFile(FileRefNum,DataTemp,PtrSize(DataTemp)) != 0)
					{
						return False;
					}
				if (Scan != Limit - 1)
					{
						if (WriteToFile(FileRefNum,EOLN,EOLNLength) != 0)
							{
								return False;
							}
					}
