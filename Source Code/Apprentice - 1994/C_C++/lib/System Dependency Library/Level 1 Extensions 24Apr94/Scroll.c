...
static char					RawDownArrowHilited[] =
	{
...
static char					RawRightArrowHilited[] =
	{
...
static char					RawLeftArrowHilited[] =
	{
...
typedef enum {eNoButton EXECUTE(= -134), eMinusButton, ePlusButton,
							ePageMinus, ePagePlus, eDohickyBox} States;

/* states for throttle control */
typedef enum {eWaitThrottle1 EXECUTE(= -552), eWaitThrottle2, eNoThrottle} Throttles;
...
void						ScrollHitProc(ScrollRec* TheBar, ModifierFlags Modifiers,
									...
									void (*ScrollHook)(long Parameter, ScrollType How, void* Refcon))
	{
	 ...
	 LoopPoint:
		if ((ThrottleState != eNoThrottle) && ((State == eMinusButton) || (State == ePlusButton)))
			{
				double					When;
