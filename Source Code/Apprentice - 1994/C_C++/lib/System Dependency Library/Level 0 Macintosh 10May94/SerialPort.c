...
MyBoolean						WriteSerialPort(SerialPortRec* SerialPort, long Length, char* Data)
	{
	 ...
	 ReEntryPoint:
		...
		SerialPort->WriteCellArray[ChosenWriteCell].MyPB.ioParam.ioCompletion = &Callback;
		...
		if (Length > WRITECELLSIZE)
			{
				...
				PBWrite((void*)&(SerialPort->WriteCellArray[ChosenWriteCell]),True/*async*/);
...
static pascal void	Callback(void)
	{
		asm
			{
				/* this is one of those hideous old callback routine types from the */
				/* days before refcons. */
				move.w		#0,OFFSET(WriteCell,InUseFlag)(A0)
				move.l		#0,OFFSET(WriteCell,NumBytes)(A0)
			}
	}
