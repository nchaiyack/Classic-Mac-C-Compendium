...
typedef enum {eAwaitingConnection, eConnectionPending} PortStates;
...
typedef enum {eWriteIdle, eWriteInProgress, eWriteFinished} SessionWriteStates;

typedef enum {eReadIdle ,eReadInProgress, eReadFinished} SessionReadStates;

typedef enum {eSessionNormal, eSessionDataArrived, eSessionClosed} SessionStates;
		...
		for (Scan = 0; Scan < Limit; Scan += 1)
			{
				...
				Session = ArrayGetElement(SessionArray,Scan);
		...
		for (Scan = 0; Scan < Limit; Scan += 1)
			{
				...
				Port = ArrayGetElement(PortArray,Scan);
...
NetEvents						NetUpdate(SessionIDType** SessionNumber)
	{
		...
		ERROR(!Initialized,"NetUpdate:  not initialized");
				...
				do
					{
						...
						Session = ArrayGetElement(SessionArray,SessionScan);
...
static void					ReinstallPPCInform(PortIDType* Port)
	{
		...
		Port->ThePPCInformPBRec.ioCompletion = &MyPPCInformCompletionRoutine;
...
NetErrors						NetOpenSession(char* PortString, char* MachineString,
											SessionIDType** SessionOut, NetworkTypes WhichNetwork)
	{
		...
		EXECUTE(ThePPCStartPBRec.ioCompletion = (void*)0x81818181;)
...
static void					UpdateRead(SessionIDType* Session)
	{
		...
		Session->ThePPCReadPBRec.ioCompletion = &MyPPCReadCompletionRoutine;
		...
		if (Session->SendHead != NIL)
			{
				...
				Session->ThePPCWritePBRec.ioCompletion = &MyPPCWriteCompletionRoutine;
