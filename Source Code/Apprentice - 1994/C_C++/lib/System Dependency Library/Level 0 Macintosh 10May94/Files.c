...
/* with a leading period; Macintosh puts them in a "Preferences" folder... */
/* this routine does not perform operations on file descriptors. */
...
MyBoolean						WriteSChar(FileType* FileRef, char TheCharacter)
	{
		ERROR(TheCharacter == 0x80,PRERR(AllowResume,"WriteChar writing 0x80"));
		return (WriteToFile(FileRef,&TheCharacter,1) == 0);
	}
...
MyBoolean						WriteUChar(FileType* FileRef, unsigned char TheCharacter)
	{
		return (WriteToFile(FileRef,(char*)&TheCharacter,1) == 0);
	}
...
MyBoolean						WriteSShort(FileType* FileRef, short TheShort)
	{
		...
		ERROR(TheShort == 0x8000,PRERR(AllowResume,"WriteShort writing 0x8000"));
		Buffer[0] = TheShort & 0xff;
		Buffer[1] = (TheShort >> 8) & 0xff;
		return (WriteToFile(FileRef,Buffer,2) == 0);
	}
...
MyBoolean						WriteSLong(FileType* FileRef, long TheLong)
	{
		...
		ERROR(TheLong == 0x80000000,PRERR(AllowResume,"WriteLong writing 0x80000000"));
		Buffer[0] = TheLong & 0xff;
		Buffer[1] = (TheLong >> 8) & 0xff;
		Buffer[2] = (TheLong >> 16) & 0xff;
		Buffer[3] = (TheLong >> 24) & 0xff;
		return (WriteToFile(FileRef,Buffer,4) == 0);
	}
...
MyBoolean						ReadSChar(FileType* FileRef, char* DataOut)
	{
		...
		if (ReadFromFile(FileRef,Buffer,1) == 0)
			{
				*DataOut = Buffer[0];
				return True;
			}
...
MyBoolean						ReadUChar(FileType* FileRef, unsigned char* DataOut)
	{
		...
		if (ReadFromFile(FileRef,Buffer,1) == 0)
			{
				*DataOut = (unsigned char)Buffer[0];
				return True;
			}
		 else
			{
...
MyBoolean						ReadSShort(FileType* FileRef, short* DataOut)
	{
		...
		if (ReadFromFile(FileRef,Buffer,2) == 0)
			{
				*DataOut = ((short)(unsigned char)Buffer[0] & 0x00ff)
...
MyBoolean						ReadUShort(FileType* FileRef, unsigned short* DataOut)
	{
		...
		if (ReadFromFile(FileRef,Buffer,2) == 0)
			{
				*DataOut = ((unsigned short)(unsigned char)Buffer[0] & 0x00ff)
					| ((unsigned short)(unsigned char)Buffer[1] << 8);
				return True;
			}
		 else
			{
...
MyBoolean						ReadSLong(FileType* FileRef, long* DataOut)
	{
		...
		if (ReadFromFile(FileRef,Buffer,4) == 0)
			{
				*DataOut = ((long)(unsigned char)Buffer[0] & 0x00ff)
					| (((long)(unsigned char)Buffer[1] << 8) & 0xff00)
					| (((long)(unsigned char)Buffer[1] << 16) & 0xff0000)
...
MyBoolean						ReadULong(FileType* FileRef, unsigned long* DataOut)
	{
		...
		if (ReadFromFile(FileRef,Buffer,4) == 0)
			{
				*DataOut = ((unsigned long)(unsigned char)Buffer[0] & 0x00ff)
					| (((unsigned long)(unsigned char)Buffer[1] << 8) & 0xff00)
					| (((unsigned long)(unsigned char)Buffer[1] << 16) & 0xff0000)
					| ((unsigned long)(unsigned char)Buffer[1] << 24);
				return True;
			}
		 else
			{
