		...
		if (StartupList == NIL)
		 ...
		 else
			{
				if (ArrayGetLength(StartupList) != 0)
					{
						*ReturnStuff = ArrayGetElement(StartupList,0);
...
void					ClearStartupDocuments(void)
	{
		...
		if (StartupList != NIL)
			{
				while (ArrayGetLength(StartupList) != 0)
					{
						ReleasePtr(ArrayGetElement(StartupList,0));
						ArrayDeleteElement(StartupList,0);
					}
		...
		AERemoveEventHandler(kCoreEventClass,kAEOpenDocuments,&MyHandleODoc,False);
