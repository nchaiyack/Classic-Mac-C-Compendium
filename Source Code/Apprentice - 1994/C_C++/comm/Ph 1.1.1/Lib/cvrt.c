...

{
	...
	char				*s;					/* pointer to cur pos in str */
	...
	if (tconSpecified) {
		if ((tconRez = GetResource('TCON', tconID))  && 
			...
			AddResource(tconRez, 'TCON', tconID, "\p");
			if (rCode = ResError()) {			
	...
	if (tagSpecified) {
		if ((tagRez = GetResource('TAG ', tagID))  && 
			...
			AddResource(tagRez, 'TAG ', tagID, "\p");
			if (rCode = ResError()) {			
	...
	if (cellSpecified) {
		if ((cellRez = GetResource('CELL', cellID))  && 
			...
			AddResource(cellRez, 'CELL', cellID, "\p");
			if (rCode = ResError()) {			
	...
	while (true) {
		...
		while (fgets(str, 255, input)) {
			...
			if (len && *str == '\\') {
				...
				if (len >= 5 && !strncmp(str, "\\tcon", 5)) {
					...
					*((short*)r) = atoi(str+5);
					...
					for (i = 0; i < picBands; i++) {
						if (p+13 - (unsigned char*)*theRez > 0x7f00) {
							fprintf(stderr, 
						...
						*pp = strLen;
						...
						if (!nextOnly || (onlyCode & docScreen)) {
							...
							if (cellSpecified) {
								*t++ = id-firstID;
								offset = pp - (unsigned char*)*theRez;
								*t++ = offset >> 8;
								*t++ = offset & 0xff;
							}
						}
					...
					*p++ = 2;
					...
					ktcon = atoi(s);
					for (i = 0; i < ktcon; i++) {
						if (p+6 - (unsigned char*)*theRez > 0x7f00) {
							fprintf(stderr,
								"#### %s - too much text in STR# resource (>32K).\n",
								argv[0]);
							return 2;
				...
				if (p+len+1 - (unsigned char*)*theRez > 0x7f00) {
