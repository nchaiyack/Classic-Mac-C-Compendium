...
static OSErr Write (Handle repHandle, OSType creator, FSSpec *fSpec)

{
	...
	rCode = fsu_FSpCreate(fSpec, creator, 'TEXT');
	if (rCode == dupFNErr) {
		if (rCode = fsu_FSpDelete(fSpec)) return rCode;
		if (rCode = fsu_FSpCreate(fSpec, creator, 'TEXT')) return rCode;
	} else if (rCode) {
		return rCode;
	}
	...
	if (rCode = fsu_FSpOpenDF(fSpec, fsRdWrPerm, &refNum)) return rCode;
...
OSErr rep_Save (Handle repHandle, Str255 prompt, 
	Str255 defName, OSType creator, Boolean *good)

{
	StandardFileReply		reply;			/* standard file reply */
	OSErr						rCode;			/* result code */
	CursHandle				watch;			/* handle to watch cursor */
	
	/* Call StandardPutFile.  Return if dialog canceled. */
	
	utl_FixStdFile();
	sfu_StandardPutFile(prompt, defName, &reply);
	if (!(*good = reply.sfGood)) return noErr;
	
	/* Create and write the file. */
	
	watch = GetCursor(watchCursor);
	SetCursor(*watch);
	rCode = Write(repHandle, creator, &reply.sfFile);
	InitCursor();
	if (rCode) {
		(void) fsu_FSpDelete(&reply.sfFile);
		return rCode;
	}
	return FlushVol(nil, reply.sfFile.vRefNum);
}
