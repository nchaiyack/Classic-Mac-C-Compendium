#ifndef lint
static char sccsid[] = "@(#)uuq.c	1.4 (Berkeley/gnuucp) 9/29/87";
#endif
...
char **argv;
{
	...
	while (--argc > 0) {
		...
		case 'u':
			...
			break;
		case 'd':
			setuid(getuid());	/* FIXME, system dep. */
			rmjob = &argv[0][2];
			break;
		case 'b':
	...
	read_params(uuControl);	/* Find out all sorts of good stuff */
	chdir(Spool);
...
char *filename;
{
	struct job *jp;
	struct sys *sp;
	char sbuf[NAMESIZE+1], str[256], nbuf[256];
	char  *jptr, *wrkvec[20];
	char grade;
	FILE *fp, *df;
	struct stat statb;
	...
	strncpy(sbuf, filename, NAMESIZE);
	sbuf[NAMESIZE] = '\0';
	...
	*jptr++ = 0;
	...
	if ((fp = fopen(munge_filename(filename), "r")) == NULL) {
		perror(munge_filename(filename));
		return(0);
	}
	files = 0;
	bytes = 0;
	...
	while (fgets(str, sizeof str, fp)) {
		...
		if (W_TYPE[0] == 'S') {
			if (strcmp(W_DFILE, "D.0") &&
			    stat(munge_filename(W_DFILE), &statb) >= 0)
				bytes += statb.st_size;
			else if (stat(munge_filename(W_FILE1), &statb) >= 0)
				bytes += statb.st_size;
		}
		/* amusing heuristic */
#define	isXfile(s)	(s[0]=='D' && s[strlen(s)-WSUFSIZE]=='X')
		if (gotname == 0 && isXfile(W_FILE1)) {
			if ((df = fopen(munge_filename(W_FILE1), "r")) == NULL)
				continue;
	...
	fclose(fp);
	if (rmjob) {
		remove(munge_filename(filename));
		fprintf(stderr, "Removing command file %s\n", filename);
		exit(0);
	}
	if (files == 0) {
		static char *wtype = "X";
		static char *wfile = "no commands (forced poll?)";
		W_TYPE = wtype;
		W_FILE1 = wfile;
	}
	jp = (struct job *)malloc(sizeof(struct job));
	...
	stat(munge_filename(filename), &statb);
