...
#ifndef lint
char sysdep_version[] = "@(#)sysdep.mac gnuucp Version Fort Pond Research-6.14";
#endif
...
long xgetc_buf_pos = 0;
long xgetc_buf_count = 0;
unsigned char xgetc_buf[SER_BUF_SIZE];

int xgetc_fill_buff()
{
	long count;
	SerGetBuf(inRefNum, &count);
	if (count > SER_BUF_SIZE) count = SER_BUF_SIZE;
	if (FSRead(inRefNum, &count, xgetc_buf) != noErr)
		return(0);
	else
		xgetc_buf_count = count;
		xgetc_buf_pos = 0;
		return(count);
	}
...
xgetc()
{
	long count;
	long i;
	if (xgetc_buf_count > 0) 
		{
	    	unsigned char data;
	    	data = xgetc_buf[xgetc_buf_pos];
	...
	count = 0;
	while ((TickCount() < i) && xgetc_buf_count <= 0) 
	{
		HandleEvents();
		xgetc_fill_buff();
		if (xgetc_buf_count > 0) break;
		gnusleep(1);
		}
	if (xgetc_buf_count > 0) 
		{
	...
	short			tType)				/* OS or Toolbox trap				*/
{
	...
	if ( (tType == ToolTrap)  &&						/* Toolbox trap		*/
		 ...
		 ((theWorld.machineType > envMachUnknown)  &&	/* É512KE, Plus, or	*/
		 (theWorld.machineType < envMacII))) ) {		/* ÉSE				*/
