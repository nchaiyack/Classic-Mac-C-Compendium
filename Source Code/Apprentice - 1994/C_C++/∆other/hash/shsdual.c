/* @(#)shsdual.c	10.1 3/25/94 08:04:04 */
/*
    ...
    SHS_INFO *odd;		/* odd byte digest */
{
     ...
     * update both halves
     */
    shsUpdate(even, even_buf, (len+1)/2);
    COUNT(even, (len+1)/2);
    shsUpdate(odd, odd_buf, len/2);
    COUNT(odd, len/2);
...
dualStream(pre_str, pre_len, stream, even, odd)
    ...
    SHS_INFO *odd;		/* odd byte digest */
{
    BYTE data[2*READSIZE];	/* our read buffer */
    BYTE even_buf[READSIZE];	/* even half */
    BYTE odd_buf[READSIZE];	/* off half */
    ...
    while ((bytes = fread((char *)data, 1, READSIZE*2, stream)) > 0) {
         ...
         */
	for (i=0, olen=0, p=data; i < bytes-1; i+=2, ++olen, p+=2) {
	    even_buf[olen] = *p;
	    odd_buf[olen] = *(p+1);
	}
	if (bytes & 0x1) {
	 ...
	 */
	if (epartial) {
	    if (even->datalen == 0 && (elen&(SHS_CHUNKSIZE-1)) == 0) {
		shsfullUpdate(even, even_buf, elen);
		epartial = 0;
	    } else {
		shsUpdate(even, even_buf, elen);
	    }
	} else if ((elen&(SHS_CHUNKSIZE-1)) == 0) {
	    shsfullUpdate(even, even_buf, elen);
	} else {
	    shsUpdate(even, even_buf, elen);
	    epartial = 1;
	 ...
	 */
	if (opartial) {
	    if (odd->datalen == 0 && (olen&(SHS_CHUNKSIZE-1)) == 0) {
		shsfullUpdate(odd, odd_buf, olen);
		opartial = 0;
	    } else {
		shsUpdate(odd, odd_buf, olen);
	    }
	} else if ((olen&(SHS_CHUNKSIZE-1)) == 0) {
	    shsfullUpdate(odd, odd_buf, olen);
	} else {
	    shsUpdate(odd, odd_buf, olen);
	    opartial = 1;
    ...
    SHS_INFO *odd;		/* odd byte digest */
{
	 ...
	 */
	if (even->datalen > 0) {
	    shsUpdate(even, (BYTE *)zero, SHS_CHUNKSIZE - even->datalen);
	    COUNT(even, SHS_CHUNKSIZE - even->datalen);
	}
	if (odd->datalen > 0) {
	    shsUpdate(odd, (BYTE *)zero, SHS_CHUNKSIZE - odd->datalen);
	    COUNT(odd, SHS_CHUNKSIZE - odd->datalen);
	}
    }
