...

{
	...
	while (true) {
		...
		breakInfo.offsetB = breakInfo.offset = theLine - *theStrings;
		...
		while (true) {
			...
			if (allBlank) {
				...
				breakInfo.offset = theLine - *theStrings;
				if (endOfDoc) break;
...

{
	...
	if (!oldRom) SetFractEnable(false);
