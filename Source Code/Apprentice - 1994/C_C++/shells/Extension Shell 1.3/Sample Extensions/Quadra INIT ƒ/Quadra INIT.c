...
{	int		i;
	...
	// run if we don't have it. We beep, and post a relevent
	// error message.
	...
	// We don't post an error, but just show our disabled icon(s).
	...
	// and install the code we have to install.
	else
		{
		// Icon details
		...
		// We install one code resource, and use an address table.
		gTheParamBlock->installAddressTable		= true;
		gTheParamBlock->addressTableSelector	= kMacTypeAddressTable;
		gTheParamBlock->numCodeResources		= 1;


		// Details for a replacement Gestalt selector.
		// Details for a Gestalt selector
		gTheParamBlock->theCodeResources[kMacType].resType		= kMacTypeSelectorResType;
		gTheParamBlock->theCodeResources[kMacType].resID		= kMacTypeSelectorResID;
		gTheParamBlock->theCodeResources[kMacType].codeType		= kGestaltSelectorType;
		gTheParamBlock->theCodeResources[kMacType].theCodeThing.theGestaltSelector.theSelector = kMacTypeSelectorType;
		gTheParamBlock->theCodeResources[kMacType].theCodeThing.theGestaltSelector.overwriteExistingSelector = true;
		}
}
