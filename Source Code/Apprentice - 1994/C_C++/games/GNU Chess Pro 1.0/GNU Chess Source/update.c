...
pascal void UpdateChronos(WindowPtr theWindow, int ItemNo)
{
  ...
  if (s < 0)
	...
	time[8] = '0'+s%10;
	TextMode(srcCopy);
	MoveTo(nameRec.left+i-5-StringWidth(time),nameRec.top+15);
	DrawString(time);
  ...
  if (s < 0)
	...
	time[8] = '0'+s%10;
	TextMode(notSrcCopy);
	MoveTo(nameRec.right-5-StringWidth(time),nameRec.top+15);
	DrawString(time);
...
pascal void UpdateValue(WindowPtr theWindow, int ItemNo)
{
	...
	if (showvalue) {
		...
		FillRect(&ValueRec,whitePat);
		MoveTo(ValueRec.right-5-StringWidth(str),ValueRec.top+15);
		DrawString(str);
	} else FillRect(&ValueRec,whitePat);
	SetPort(WindThink);
}
...
pascal void UpdateCase(WindowPtr theWindow, int ItemNo)
{
	...
	if (MouseX)
	{
		...
		FillRect(&CaseRec,whitePat);
		DrawChar(" abcdefgh"[MouseX]);
		DrawChar(" 12345678"[MouseY]);
	} else FillRect(&CaseRec,whitePat);
	SetPort(SavePort);
}
