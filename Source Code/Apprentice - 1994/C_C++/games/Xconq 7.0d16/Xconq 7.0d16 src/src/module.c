...
load_default_game()
{
    char *defaultname = standardgamename;
    Module *module;
...
int dowarn;
{
	...
	if (g_random_state() >= 0) init_xrandom(g_random_state());
...
Module *module;
{
	...
	if ((basemodule = find_game_module(basename)) == NULL) {
...
int dowarn;
{
	...
	init_warning("\"%s\" is already open, ignoring attempt to reopen",
		     module->name);
	return FALSE;
    }
    /* Don't open if already loaded. */
    if (module->loaded && dowarn) {
	init_warning("\"%s\" is already loaded, ignoring attempt to reload",
		     module->name);
	return FALSE;
    }
    if (module->contents) {
	...
	if (dowarn) {
	    if (module->name) {
	    	init_warning("Can't find module \"%s\" anywhere!", module->name);
	    } else {
...
Side *side;
{
	...
	init_warning("\"%s\" could not be opened!", module->name);
