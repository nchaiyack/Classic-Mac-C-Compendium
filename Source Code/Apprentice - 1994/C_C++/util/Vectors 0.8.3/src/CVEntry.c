...
extended cvQuadtof(StringPtr str)
{
	...
	if(ns=='s'&&ew=='w')
		return(180+ang);
	if(ns!='n' && ns!='s')
		CorrectDialog(c2pstr((char*)str),(StringPtr)"\pQuad bearings begin with an ÔNÕ or ÔSÕ");
	else if(ew!='e' && ew!='w')
		CorrectDialog(c2pstr((char*)str),(StringPtr)"\pThis quad bearing has no ÔEÕ or ÔWÕ");
	else 
...
#pragma unused (x)
	...
	/* Make sure the title fits. Truncate it and add an ellipses (ÒÉÓ) */
	...
	if(newWid > wid) {
		...
		wid -= CharWidth('É'); 		/* subtract width of ellipses} */
