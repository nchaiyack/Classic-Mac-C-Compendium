...
char *PreferenceName=".gzip_prefs"; /* not used, but required by ThinkCPosix */
/*#include <console.h>*/
#include "MacErrors.h"
#include "thePrefs.h"
#include "SPDProg.h"
...
/*    progname = basename(argv[0]);*/
    ...
    if ((z_len == 0 && !decompress) || z_len > MAX_SUFFIX) {
        sprintf(strerr, "incorrect suffix '%s'", optarg);
        Calert(strerr);
        do_exit(ERROR);
    }
...
#ifdef _SPD_PROG_
 		sprintf(SPDpstr, "%s: %s -> %s",progname, ifname, ofname);
...
local int create_outfile()
{
...
#else
	flags = O_WRONLY | O_CREAT | O_EXCL | O_TEXT; /* SPDsoft */
#endif /*MACOS*/
    }
    for (;;) {
	...
	{
	FInfo	vFInfo;
	extern long GvRefNum;
			
			CtoPstr(ofname);
			GetFInfo(ofname,GvRefNum,&vFInfo);
			...
			SetFInfo(ofname,GvRefNum,&vFInfo);
			PtoCstr(ofname);
	}
    ...
    char *dir;
{
    ...
    if (dirp == NULL) {
	sprintf(strerr, "%s unreadable", dir); Calert(strerr);
	exit_code = ERROR;
	return ;
    }
    /*
	...
	if (len + NLENGTH(dp) + 1 < MAX_PATH_LEN - 1) {
	    ...
	    sprintf(strerr,"%s/%s: pathname too long\n", dir, dp->d_name);
	    Calert(strerr);
	    exit_code = ERROR;
	}
    }
...
struct option init_longopts[] =
{
 ...
 /* {"pkzip",      0, 0, 'k'},    force output in pkzip format */
	...
	memcpy(longopts,init_longopts,sizeof(init_longopts));
