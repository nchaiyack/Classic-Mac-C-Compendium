...
in any way that restricts its use by any others, including its authors.
	...
	mpark@nb.utmem.edu or mpark@utmem1.utmem.edu
	...
	The text engine is from TE32K, by Roy Wood. I have extensively 
	modified it for the family of applications related to PlainText.
	
	That family is a number of tools for mapping for which a worksheet-
	based front end was desirable.
...
static void setUseWorksheet(void);
...
void MakeWorksheetDoc(FSSpecPtr);
...
{	
	...
	ConvertMenu=GetMenu(CONVERT_ID);
	InsertMenu(ConvertMenu,0);
...
static void DoAboutDialog(void)
{
	...
	while (itemHit>10) ModalDialog(SimpleDialogFilter,&itemHit);
	DisposDialog(abtDlg);
#endif

	dT = (DialogTHndl)GetResource('DLOG',1002);
	LoadResource((Handle)dT);
	...
	abtDlg=GetNewDialog(1002,NULL,(WindowPtr)(-1));
	version = GetResource('vers',2);
	...
	while (itemHit>10) {
		MoveTo(280,240);
		DrawString((StringPtr) *version + *(*version+6) + 7);
...
void AdjustMenus(WindowPtr wind)
{
	...
	if(wind) {
		...
		EnableItem(EditMenu,iWorksheet);
	...
	SetMenuAbility(EditMenu,iShowClipboard,TE32KScrpHandle && TE32KScrpLength > 0);
...
short DoCreatePrefsFile(short aVRefNum, long aDirID, ConstStr255Param rName)
{
	Handle h;
	short resNum=-1,ID;
	OSErr resErr;
...
{
	Str255 worksheetChoice,buf;
	
	GetItem(EditMenu,iWorksheet,worksheetChoice);
	GetIndString(buf,129,1);
	gUseWorksheet = !IUCompString(worksheetChoice,buf);
	DoReadPrefs();
}
...
static void setUseWorksheet(void)
{
	...
	if(gUseWorksheet) {
		GetIndString(buf,129,2);
		SetItem(EditMenu,iWorksheet,buf);
		if(gWorkSheetDoc)
			CloseADoc((WindowPtr)gWorkSheetDoc);
		gUseWorksheet = !gUseWorksheet;
		gWorkSheetDoc=0;
	} else {
		GetIndString(buf,129,1);
		SetItem(EditMenu,iWorksheet,buf);
	...
	CalcMenuSize(EditMenu);		/* because there is a bug in the Menu Manager */
	DeleteMenu(EDIT_ID);		/* now remove menu so we can manipulate it */
	(**EditMenu).menuProc=0;	/* set to proc id, instead of proc handle */
	ChangedResource((Handle)EditMenu);	/* now save the resource */
	WriteResource((Handle)EditMenu);
	ReleaseResource((Handle)EditMenu);
	EditMenu=GetMenu(EDIT_ID);			/* and get it again */
	InsertMenu(EditMenu,FIND_ID);
	DrawMenuBar();
}
...
WindowPtr OpenNamedFile(ConstStr255Param fileName)
{
	FSSpec spec;
	char *p;
	HFileInfo pB;
...
Boolean OpenDocFromFinder(void)
{
	...
	AppFile theApp;
	FSSpec spec;
	Boolean fileOpened = false, hasFSSpec;
	OSErr err;
	long response=0;
...
void OpenNewDoc(void)
{
	...
	if(newDoc = MakeDoc('TEXT',&spec)) {
		if(((*newDoc->makeWindow)((WindowPtr)newDoc)) && ((*newDoc->initDoc)((WindowPtr)newDoc))) {
			...
			tH = (TE32KHandle)newDoc->docData;
			pageWidth=(**newDoc->fPrintRecord).prInfo.rPage.right;
			pageWidth += ((**tH).destRect.left - (**tH).viewRect.left);
			pageWidth -= 2*LeftMargin;
			r = ((WindowPtr)newDoc)->portRect;
			windowSize=r.right-r.left;
			if(pageWidth<windowSize) {
				windowSize=r.bottom-r.top;
				SizeWindow((WindowPtr)newDoc,pageWidth+16,windowSize,false);
				SetTERect((WindowPtr)newDoc);
				SizeScrollBars((WindowPtr)newDoc);
			}
...
int ConfirmDialog(short strID,StringPtr message)
{
	...
	*(buf+*buf+1)=0;
	*a=sprintf((char*)a+1,(char*)buf+1,p2cstr(message));
	c2pstr(message);

	confirmDialog=GetNewDialog(rConfirmDialog,(Ptr)&DlogStor,(WindowPtr)(-1));
	...
	if(doc->marks) {
		...
		*/
		...
		for(i=0;i<nMarks;i++,mark+=mSize) {
			...
			*/
			if(mSize && docLength>=0 && (((MarkRecPtr)mark)->selEnd < 0 ||
						((MarkRecPtr)mark)->selEnd>docLength ||
						((MarkRecPtr)mark)->selStart < 0 ||
