/*
	...
	Written by:
		Hugh Svendsen, ATG Sound
		A. Phil Sohn, ATG Sound
		Eric Behnke
		Jeff Boone
	...
	Change History (most recent first):
	   ...
	   <LW5>	  3/8/93	ejb		define new trap DSPLoadSlotDrivers
	   <LW4>	 2/10/93	ejb		add prototypes for new traps: CreateAliasToResfile and
									RebootCPUDevice
	   <LW3>	12/21/92	ejb		added new internal trap DSPGetResource
	   <LW2>	12/18/92	JDB		EJB: Added DeviceCall and DriverCall traps.
	  <SM14>	12/10/92	JDB		Added DSPLoadKernel & DSPAllocateSegment Traps
	  <SM13>	 12/2/92	JDB		Making all calls traps: Part II
	  <SM12>	 12/2/92	JDB		Making all calls traps: Part I
	  <SM11>	 12/1/92	ejb		changed sample and frame rates from unsigned long to fixed
									numbers.
	  <SM10>	11/20/92	JDB		Added DSPPatchPrevAndNext trap number.
	   <SM9>	11/13/92	JDB		Added public selectors and trap numbers for
									DSPAllocateDriverFIFO and DSPDisposeDriverFIFO.
	   <SM8>	10/27/92	JDB		Added DSPGetPtrSize trap number for the (internal) call used for
									parameter checking.
	   <SM7>	 9/11/92	APS		Renamed DSPAllocateModule to DSPCreateModule. The original name
									was confusing, and I wanted to write another DSPAllocateModule.
	   <SM6>	  9/9/92	APS		Fixed a couple of typos.
	   <SM5>	 6/23/92	JDB		DSPFIFOClearInterrupt, DSPFIFOSetRefCon, DSPFIFOReset and
									DSPSetTaskRefCon now return OSErr’s.
	   <SM4>	 6/22/92	JDB		d12 final candidate roll-in.
		<97>	 6/22/92	JDB		Moved SignIn/SignOut CPUDevice/IODevice traps from DSPManager.h
									to DSPManagerPrivate.h
		<96>	 6/22/92	APS		Added traps for DSPGetAvailableOnChipMemory and
									DSPSetSectionSize.
		<95>	 6/22/92	APS		Changed the DSPCPUDeviceParamBlk by adding cpuTimeShareFreqand
									cpuClientCallbackProc.
	   <SM3>	 6/21/92	ejb		Checked in BOMII d12 release.
  	--------------------------------------------------------------------------------------------------
		<94>	 6/19/92	JDB		Added traps to get and set the MessageActionProc for a FIFO.
		<93>	 6/16/92	APS		Modified DSPSetGPBMode.
		<92>	 6/16/92	APS		Added DSPGPBModeIndicator.
		<91>	 6/15/92	JDB		Changed DSPProcessMessage to DSPClientMessageDispatch.
		<90>	 6/12/92	JDB		Added kdspProcessMessage trap constant.
		<89>	  6/9/92	EJB		Changed iopbCPUDeviceIndex to iopbIODeviceINdex since this is
									what is documented and it makes more sense this way.
		<88>	  6/4/92	APS		Renamed DSPSetFIFOMessageThreshold to
									DSPFIFOSetMessageThreshold.
		<87>	  6/4/92	APS		Added the DSPDisposeZone trap.
		<86>	  6/3/92	JDB		Added the ConnectHIHOSections trap number. Also added the
									kdspHIHOConnection to the possible connection types.
		<85>	  6/2/92	APS		Added traps for DSPFIFOGetMessageDispatch, DSPUpdateGPBPrefFile,
									DSPSetFIFOMessageThreshold, DSPFIFOSwap, DSPSetGPBMode,
									DSPDontUpdateGPBPrefs, and DSPUpdateGPBPreferenceFile.
		<84>	 5/29/92	EJB		Changed SignInIODevice, SignOutIODevice, and SignOutCPUDevice
									APIs to be multi-dsp aware.
		<83>	 5/28/92	JDB		Adding DSPGetTaskInfo and DSPSetCPUDeviceBondage calls.
		<82>	 5/28/92	APS		Fixed the arguments to DSPDisposeFIFO.
		<81>	 5/21/92	JDB		Added cpuProcessorNum to parameters of DSPSignInCPUDevice to
									support multiple DSPs.
		<80>	 5/21/92	APS		Made Rev. 4 Linked FIFO changes.
		<79>	 4/24/92	APS		Updated kdspManagerBuildVersion. The FIFO APIs have changed
									among other things.
		<78>	 4/21/92	APS		Added DSPAddressInZone
		 4/16/92 JDB - Added the calls to support the new task synchronization model.
		 4/15/92 APS - Changed the Dispatch mechanism
		04/08/92 JDB - Removing the DSPFIFOPtr and DSPFIFOHandle declarations.
		 4/ 3/92 APS - Added Support for LinkedFIFOs
		10/15/91 APS - Removed DSPEnoughGPB
		10/15/91 JDB - Added reference number data types for DSPModule, DSPSection
						& DSPFIFO
		09/27/91 APS - Removed elements associated with Bombugs
		05/22/91 APS - Removed DSPInsertRelativeTask
...
pascal OSErr DSPFIFOFill (DSPFIFORefNum theFIFORefNum, unsigned long theSize, 
		Ptr logical, Ptr physical, Boolean fifoFull,
		MessageActionProc theInterrupt)
