/*
	...
	Change History (most recent first):
	   ...
	   <LW4>	 3/21/93	ejb		add sio feature bit: kdspRegisterDSPOutputSifter;  remove
									kdspStdTelecomCompatible feature bit; redefine
									kdspStdSndCompatible to be bit zero.
	   <LW3>	  2/4/93	ejb		replace CanBeBusMaster with  CanOnlyAccessDSPZone
	   <LW2>	  2/4/93	ejb		Remove obsolete cpu constant for for '32c ' devices; define cpu
									device feature bi CanBeBusMaster
	  <SM14>	12/10/92	JDB		ejb: Code review changes, Beta buster release.
	  <SM13>	 12/2/92	JDB		Added error for inserting a task when the driver is handling a
									frame overrun condition.
	  <SM12>	 12/1/92	ejb		add support for 'dcon' selector.
	  <SM11>	11/13/92	JDB		Moved more public selectors from DSPConstantsPrivate.h
	  <SM10>	10/27/92	JDB		Moved the public driver selectors from DSPConstantsPrivate.h
	   <SM9>	 10/6/92	JDB		Moved kdspSIOTypeDevice and kdspBIOTypeDevice from private to
									public.
	   <SM8>	  9/9/92	APS		Changed kdspSectionsNotInSameModule to
									kdspSectionsNotInSameTask, which is what it should have been in
									the first place
	   <SM7>	 8/27/92	APS		Moved a few section flags to the private file.
	   <SM6>	 7/15/92	JDB		Changed our error return codes to the new “blessed” numbers.
	   <SM5>	 6/24/92	ejb		Merged in constants from DSPDriver.h, a file which is being
									obsoleted.
	   <SM4>	 6/22/92	JDB		d12 final candidate roll-in.
		<97>	 6/22/92	APS		Added kdspTaskNotInstalled to the errors.
	   <SM3>	 6/21/92	ejb		Checked in BOMII d12 release.
  	--------------------------------------------------------------------------------------------------
		<96>	 6/18/92	APS		Put the original kdspSmoothModule back in the GPBFlags. Moved
									some of these flags to the private file.
		<95>	 6/18/92	APS		Moved around the module flag so that the inital sectn can be
									stored in the upper 16 bits.
		<94>	 6/17/92	APS		Moved some of the module flags to DSPConstantsPrivate.h.
		<93>	 6/17/92	EJB		Moving in evt selector for num realtime frames from private
									constants file.
		<92>	 6/17/92	APS		Added kdspTwoStacks.
		<91>	 6/17/92	APS		Added kdspOnChipStack and kdspOffChipStack.
		<90>	 6/17/92	EJB		Moving some DSP API constants from DSP3210.h into this file for
									Snoopy.
		<89>	 6/16/92	APS		Added kdspUseActual.
		<88>	 6/15/92	APS		Added some comments.
		<87>	 6/15/92	EJB		Enhance BIO Pin support.
		<86>	 6/15/92	EJB		Enhance BIO Pin support.
		<85>	 6/14/92	EJB		Simplify the GPB messages.
		<84>	 6/14/92	EJB		Simplify the GPB messages.
		<83>	 6/12/92	APS		Added constants for run time patching of link nodes.
		<82>	  6/9/92	EJB		Changed FIFO Message priority.
		<81>	  6/9/92	JDB		Added the kdspDeviceCantBeSlave error code.
		<80>	  6/4/92	JDB		Changing the kdspExternal section flag to be the same as
									kdspLeaveSection.
		<79>	  6/3/92	APS		Added kdspVanillaProgram as a module flag.
		<78>	  6/3/92	EJB		Changes to module and section flags for HIHO buffers.
		<77>	  6/3/92	EJB		Changes to module and section flags for HIHO buffers.
		<76>	  6/3/92	EJB		Changes to module and section flags for HIHO buffers.
		<75>	  6/3/92	EJB		Changes to module and section flags for HIHO buffers.
		<74>	  6/2/92	EJB		Removed runtime error messages.
		<73>	  6/1/92	EJB		Changes in FIFO messages.
		<72>	  6/1/92	APS		Added kdspDontUpdateGPBPref for module flags.
		<71>	 5/28/92	EJB		Add new FIFO message for fifo interrupt when fifo goes not
									empty. HipNet needs this for HappyFAX.
		<69>	 4/20/92	EJB		Add some HIHO constants.
	    <67>	 4/16/92	JB		Adding errors for synchronizing tasks.
				 4/15/92 	APS 	Added kdspLoadFIFOSection and kdspSaveFIFOSection
		 		 4/ 3/92 	APS 	Added Errors for LinkedFIFOs
