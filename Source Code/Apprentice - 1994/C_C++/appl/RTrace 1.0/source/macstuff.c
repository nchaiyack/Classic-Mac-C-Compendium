...
void install_get_event_vbl(void)
{
	...
	/* Save the applications’s A5 where the VBL tasks can find it; right 
	 	before the VBL Task structure.  We need the VBL Tasks to have access
	 	to the application globals, so they need to know what the application’s
...
pascal void set_event_flag(void)
{
	...
	/* The base address of this tasks VBL structure is now in A0.  We can use
		this to get the application’s A5 global pointer.  We need this to be
		able to access gZ80Yield */
	asm {
		move.l A5, -(SP)			; save current A5
		move.l -8(A0), A5			; get applications’s A5
		}
	...
	
{
	...
	if (status_dialog_visible) set_status_text("\pPreparing for Render…");
	...
	if ((!scene_in_memory) || (!rendering_same_file) || (texture_mode_changed))
		...
		if (status_dialog_visible) set_status_text("\pPreprocessing .sff file…");
	...
	if (GetCtlValue(animate_checkbox))
		...
		for (t = tstart; t < tend*1.001; t += tstep, frame_number++)
			...
			sprintf(string, "Rendering frame %ld of %ld…", frame_number, frames);
