...
var
	...
	CH, SH, CP, SP, CB, SB, xv, yv, zv, X, Y, Z, X3, Y3, Z3, AM, BM, CM, DM, EM, FM, GM, HM, IM, D, P, B, H, U, vc, U1, V1: real;
	V: array[1..maxa, 1..3] of real;
	E: array[1..maxa] of real;
	data: array[1..maxd] of real;
	w: windowptr;
	ticks: longint;

procedure setdataarray;
begin
	data[1] := 0;
	data[2] := -2;
	data[3] := 46;
...
begin
	...
	for Ec := 1 to maxe do
		begin
			...
			if E[Ec] > 0 then
				begin
					moveto(round(U1 * 2 + 75), round(V1 + 20));
					lineto(round(U * 2 + 75), round(Vc + 20));
				end;
			U1 := U;
			V1 := Vc;
		end;
...
procedure winit (var w: windowptr);
begin
	w := getnewwindow(r1, nil, windowptr(-1));
	showwindow(w);
	setport(w);
	moveto(50, 240);
	textsize(18);
	textface([Shadow]);
	drawstring('Eddy'' s Shuttle Viewer ');
	moveto(50, 260);
	textsize(12);
	textface([bold]);
	drawstring('Click and hold button down to STOP');
	moveto(50, 280);
	textsize(10);
	textface([italic]);
	drawstring('ThinkPas Code by Eddy Vasile. Plot Points from anonymous source');
	moveto(200, 170);
	textsize(10);
	drawstring('Wait for a couple of views before clicking');
	delay(100, ticks);
end;

procedure initvars;
	var
		position, j, ec, pc: integer;
begin
	position := 0;
	for Pc := 1 to maxv do
		begin
