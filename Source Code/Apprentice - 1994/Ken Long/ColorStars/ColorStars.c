...
#define num_stars 180	//¥ Reduce number for more speed (try 18).
...
RgnHandle			GrayRegion, mBarRgn;
short				*mBarHeightPtr;
short				oldMBarHeight;
Rect				windowBounds;
CWindowPtr			theWindow;
short				H_far, V_far;
double				star_H [num_stars] , star_V [num_stars] , distance [num_stars] ;
double				shipSpeed;
...
void SetUpWindow (void) 
{
	...
	theWindow = NewCWindow (0L, &windowBounds, "\pNameless", true, 
							plainDBox, (CWindowPtr)-1L, true, 0);
	SetPort (theWindow);
