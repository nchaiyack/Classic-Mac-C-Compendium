...
static char			currentDirName[512];
...
static int MapVolume(void);
static int  ListDirectory(int dirsFRefNum,int filesFRefNum,long theIODirID);
...
static int CreateRestoreQuit(char *thePrompt,int theDefault);
static int ContinueRenameDeleteQuit(char *thePrompt,int theDefault);
...
static void DoHelpBox(int helpResWIND,int helpResTEXT,int helpResSTYL);
...
char	tempString[256];
	...
	if ((errCode = SetupVolRefNum()) != noErr)
		return(errCode);
	...
	{
		errCode = createFolder(STUDENTDIRECTORY);
	...
	do
	{
		userChoice = CreateRestoreQuit("Create volume map or Restore volume info?",2);
...
{
int				errCode,j;
int				dirsFRefNum,filesFRefNum;
long			freeBytes,byteCount;
char			tempString[256],dirsFName[256],filesFName[256];
HParamBlockRec	myHPB;
	...
	if (errCode!=noErr && errCode!=fnfErr)
	{
		sprintf(errorString,"MapVolume:\rFSOpen('%s') error %d",FILELISTNAME,errCode);
		ErrorAlert(errorString);
		return(errCode);
	}
	...
	{
		if ((errCode = Create(filesFName,sysFolderVRefNum,CREATOR,'Stuf'))!=noErr)
		{
			sprintf(errorString,"MapVolume:\rCreate('%s') error %d",FILELISTNAME,errCode);
			ErrorAlert(errorString);
			return(errCode);
		}
		
		else if ((errCode=FSOpen(filesFName,sysFolderVRefNum,&filesFRefNum))!=noErr)
		{
			sprintf(errorString,"MapVolume:\rFSOpen('%s') error %d",FILELISTNAME,errCode);
			ErrorAlert(errorString);
			return(errCode);
	...
	if ((errCode = SetEOF(filesFRefNum,0L))!=noErr)
	{
		sprintf(errorString,"MapVolume:\rSetEOF('%s') error %d",FILELISTNAME,errCode);
		ErrorAlert(errorString);
	...
	if (errCode!=noErr && errCode!=fnfErr)
	{
		sprintf(errorString,"MapVolume:\rFSOpen('%s') error %d",MAPFILENAME,errCode);
		ErrorAlert(errorString);
		FSClose(filesFRefNum);
		return(errCode);
	}
	...
	{
		if ((errCode = Create(dirsFName,sysFolderVRefNum,CREATOR,'Stuf'))!=noErr)
		{
			sprintf(errorString,"MapVolume:\rCreate('%s') error %d",MAPFILENAME,errCode);
			ErrorAlert(errorString);
			FSClose(filesFRefNum);
			return(errCode);
		}
		
		else if ((errCode=FSOpen(dirsFName,sysFolderVRefNum,&dirsFRefNum))!=noErr)
		{
			sprintf(errorString,"MapVolume:\rFSOpen('%s') error %d",MAPFILENAME,errCode);
			ErrorAlert(errorString);
			FSClose(filesFRefNum);
	...
	{
		sprintf(errorString,"MapVolume:\rSetEOF('%s') error %d",MAPFILENAME,errCode);
		ErrorAlert(errorString);
		FSClose(dirsFRefNum);
		FSClose(filesFRefNum);
		return(errCode);
	...
	myHPB.volumeParam.ioNamePtr = (StringPtr) &(theVolInfo.name);
	...
	if (errCode != noErr)
	{
		sprintf(errorString,"MapVolume:\rPBHGetVInfo() error %d",errCode);
		ErrorAlert(errorString);
		FSClose(dirsFRefNum);
		FSClose(filesFRefNum);
		return(errCode);
	...
	PtoCstr(theVolInfo.name);
	strcpy(currentDirName,theVolInfo.name);
	
	theVolInfo.attrib = myHPB.volumeParam.ioVAtrb;
	
	for (j=0;j<32;j++)
		theVolInfo.finderInfo[j] = ((char *) &(myHPB.volumeParam.ioVFndrInfo))[j];
		
	byteCount = sizeof(VolumeInfo);
	
	if ((errCode = FSWrite(dirsFRefNum,&byteCount,&theVolInfo))!=noErr)
	{
		sprintf(errorString,"MapVolume:\rFSWrite error %d",errCode);
		ErrorAlert(errorString);
		FSClose(dirsFRefNum);
		FSClose(filesFRefNum);
		dirsFRefNum = 0;
	...
	errCode = ListDirectory(dirsFRefNum,filesFRefNum,0L);
...
char	tempString[256];
	...
	do
	{
		...
		if (errCode!=noErr && errCode!=fnfErr)
		{
			sprintf(errorString,"ListDirectory:\rPBGetCatInfo error %d",errCode);
			ErrorAlert(errorString);
			return(errCode);
		}
		...
		else if (errCode == noErr)
		{
			...
			if (myCPB.hFileInfo.ioFlAttrib & 0x10) /* is a folder */
			{
				...
				byteCount = sizeof(DirInfo);
				
				if ((errCode = FSWrite(dirsFRefNum,&byteCount,&theDirInfo))!=noErr)
				{
					sprintf(errorString,"ListDirectory:\rFSWrite(%s) error %d",MAPFILENAME,errCode);
					ErrorAlert(errorString);
					return(errCode);
				}
			}
			...
			else
			{
				...
				byteCount = sizeof(FileInfo);
					
				if ((errCode = FSWrite(filesFRefNum,&byteCount,&theFileInfo))!=noErr)
				{
					sprintf(errorString,"ListDirectory:\rFSWrite(%s) error %d",MAPFILENAME,errCode);
					ErrorAlert(errorString);
					return(errCode);
	...
	if (errCode == noErr)
	{
		...
		do
		{
			...
			else if (errCode == noErr && myCPB.hFileInfo.ioFlAttrib & 0x10 && myCPB.dirInfo.ioDrDirID != studentWorkDirID)
			{
				PtoCstr(tempString);
				strcpy(errorString,currentDirName);
				...
				errCode = ListDirectory(dirsFRefNum,filesFRefNum,myCPB.hFileInfo.ioDirID);
...
EventRecord		theEvent;
	...
	theDPtr = GetNewDialog(CRDQDLOG, 0L,(WindowPtr) -1L);
	SetPort(theDPtr);
	...
	GetDItem(theDPtr, 6, &type, &theItem, &tempRect);
	SetDItem(theDPtr, 6, type, HiliteDefaultButton, &tempRect);
	
	strcpy(tempString,thePrompt);
	CtoPstr(tempString);
	GetDItem(theDPtr, 1, &type, &theItem, &tempRect);
...
EventRecord		theEvent;
	...
	theDPtr = GetNewDialog(CRQDLOG, 0L,(WindowPtr) -1L);
	SetPort(theDPtr);
...
FileInfo		swap;
	...
	SetInfoMessage("Now sorting files list....");
	...
	for (i = 0;i < numFiles;i++)
	{
		...
		for (j = i+1L;j < numFiles;j++)
		{
			if (CompareFiles(&(listOfFiles[j]),&(listOfFiles[least])) < 0)
				least = j;
		}
		...
		{
			swap = listOfFiles[i];
			listOfFiles[i] = listOfFiles[least];
			listOfFiles[least] = swap;
		}
		
		sprintf(errorString,"Sorting %ld/%ld = %ld%%  (%s)",i+1,numFiles,(long) ((100L * (i + 1L))/numFiles),listOfFiles[i].name);
		SetInfoMessage(errorString);
	}
	
	
	SetInfoMessage("Writing sorted file list....");
	...
	{
		DisposeFileList();
		sprintf(errorString,"SortFileList:\rFSOpen('%s') error %d",FILELISTNAME,errCode);
		ErrorAlert(errorString);
		return(errCode);
	}
	...
	if ((errCode = SetFPos(filesFRefNum,fsFromStart,0L))!=noErr)
	{
		DisposeFileList();
		FSClose(filesFRefNum);
		sprintf(errorString,"SortFileList:\rSetFPos('%s',fsFromStart) error %d",FILELISTNAME,errCode);
		ErrorAlert(errorString);
		return(errCode);
	}
	...
	if ((errCode = SetEOF(filesFRefNum,0L))!=noErr)
	{
		DisposeFileList();
		FSClose(filesFRefNum);
		sprintf(errorString,"SortFileList:\rSetEOF('%s') error %d",FILELISTNAME,errCode);
		ErrorAlert(errorString);
		return(errCode);
	}
	...
	if ((errCode = FSWrite(filesFRefNum,&byteCount,listOfFiles))!=noErr)
	{
		DisposeFileList();
		sprintf(errorString,"SortFileList:\rFSWrite(%s) error %d",FILELISTNAME,errCode);
		ErrorAlert(errorString);
		FSClose(filesFRefNum);
		return(errCode);
	}
	...
	{
		DisposeFileList();
		sprintf(errorString,"SortFileList:\rFSClose(%s) error %d",FILELISTNAME,errCode);
		ErrorAlert(errorString);
		return(errCode);
	}
