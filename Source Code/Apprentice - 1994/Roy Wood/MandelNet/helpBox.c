...
pascal void HelpScrollProc(theControl, theCode)
...
DoHelpBox(helpResWIND,helpResTEXT,helpResSTYL)
int		helpResWIND,helpResTEXT,helpResSTYL;
{
GrafPtr			oldPort;
WindowPtr		helpWPtr;
TEHandle		helpTEH;
...
char			theChar;
	...
	if (!(helpWPtr = GetNewWindow(helpResWIND,NULL,(WindowPtr) -1L)))
		FatalError("\p Can't allocate memory for help window");
	
	SetPort(helpWPtr);
	
	CenterWindow((WindowPtr) helpWPtr);
	ShowWindow((WindowPtr) helpWPtr);
	...
	if (!(helpTEH = TEStylNew(&destRect,&viewRect)))
		FatalError("\p Can't allocate memory for help text record");
	...
	if (!helpText || !helpStyle)
		ErrorAlert("\p Can't access help 'TEXT' and 'STYL' resources");
	
	else
	{
		HLock(helpText);
		
		TEStylInsert(StripAddress(*helpText),SizeResource(helpText),helpStyle,helpTEH);
		...
		do
		{
			...
			if (GetNextEvent(everyEvent,&theEvent))
			{
					...
					if (cntlCode != 0)
					{
						if (whichControl == vScrollControl)
						{
							if (cntlCode == inThumb)
							{
								...
								TrackControl(vScrollControl,theEvent.where,NULL);
							...
							cntlCode = TrackControl(doneControl,theEvent.where,NULL);
				...
				else if (theEvent.what==keyDown || theEvent.what==autoKey)
				{
					...
					if (theChar == (char) 0x0d || theChar == (char) 0x03)
						cntlCode = inButton;
				}
