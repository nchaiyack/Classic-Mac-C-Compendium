...
#define		PREFS_FILE_NAME			"\pMSG Demo prefs"
...
void DefaultPrefs(void)
{
	...
	Mymemcpy(gMyName, bob, bob[0]+1);
	gMyOrg[0]=0x00;
	gSoundToggle=gWhichPict=0xFF;
	gIsReversed=0x00;
	gLastWipe=-1;
...
void CopyGlobalsToPrefs(void)
{
	...
	thePrefs.isreversed=gIsReversed;
	thePrefs.isfirstpict=gWhichPict;
	thePrefs.lastwipe=gLastWipe;
	thePrefs.wipestatus=gWipeStatus;
...
void CopyPrefsToGlobals(void)
{
	...
	gIsReversed=thePrefs.isreversed;
	gWhichPict=thePrefs.isfirstpict;
	gLastWipe=thePrefs.lastwipe;
	gWipeStatus=thePrefs.wipestatus;
}
