...
Boolean GetSourceFile(FSSpec *sourceFS, Boolean useOldFile)
{
	Point				where;
	OSErr				isHuman;
...
Boolean GetDestFile(FSSpec *destFS, Boolean *deleteTheThing)
{
	unsigned int		count;
	Str255				otherName;
	StandardFileReply	reply;
	...
	if (gStandardFile58)
		StandardPutFile("\p5AVE K0NVERTED TEXT A5...", destFS->name, &reply);
	else
	{
		where.h = (gMainScreenBounds.right - 304)/2;
		where.v = (gMainScreenBounds.bottom - 184)/3;
		SFPutFile(where, "\p5AVE K0NVERTED TEXT A5...", destFS->name, 0, &oldReply);
		reply.sfGood = oldReply.good;
		if (reply.sfGood)
        {
			isHuman=GetWDInfo(oldReply.vRefNum, &reply.sfFile.vRefNum,
