...
void InitMSGGraphics(void)
{
	...
	gWindowWidth[kAbout]=200;
	gWindowHeight[kAbout]=220;
	...
	StuffHex(gWindowTitle[kHelp], "\p0448454c50");
...
void OpenTheWindow(int index)
{
	...
	else ErrorString("\pTHERE 15 N0T ENUF MEM0RY T0 0PEN THE W1ND0W!!", "\p");
}
	...
	if (((theDepth = GetWindowDepth(index)) > 2) && (gMaxDepth[index]>2))
	{
		...
		if (myCGrafPtr[index]==0L)
		{
			...
			if(myBits[index] == 0L)
			{
				CloseCPort(myCGrafPtr[index]);
				myCGrafPtr[index]=0L;
				ErrorString("\pTHERE 15 N0T ENUF MEM0RY T0 0PEN THE W1ND0W!!", "\p");
			}
