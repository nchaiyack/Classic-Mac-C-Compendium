...
Purpose:	This module handles the standard file dialogs for getting
			saved games and saving current games.
...
Boolean GetSourceFile(FSSpec *gameFile, Boolean AllOfEm, Boolean useOldGame)
{
	Point				where;
	OSErr				isHuman;
	...
	else
	{
		...
		if (AllOfEm)
			...
			SFGetFile(where, "\p", 0L, 3, typeList, 0L, &oldReply);
...
Boolean GetDestFile(FSSpec *gameFile, Boolean *deleteTheThing, Boolean isRealGame)
{
	unsigned int		count;
	Str255				otherName;
	StandardFileReply	reply;
	...
	if (gStandardFile58)
		if (isRealGame)
			StandardPutFile("\pSave Pentominoes game as...", "\p", &reply);
		else
			StandardPutFile("\pSave Pentominoes solution...", "\p", &reply);
	else
	{
		where.h = (gMainScreenBounds.right - 304)/2;
		where.v = (gMainScreenBounds.bottom - 184)/3;
		if (isRealGame)
			SFPutFile(where, "\pSave Pentominoes game as...", "\p", 0, &oldReply);
		else
			SFPutFile(where, "\pSave Pentominoes solution...", "\p", 0, &oldReply);
		reply.sfGood = oldReply.good;
		if (reply.sfGood)
        {
			isHuman=GetWDInfo(oldReply.vRefNum, &reply.sfFile.vRefNum,
