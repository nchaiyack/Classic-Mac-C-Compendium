   ...
   the file you selected (or returns FALSE if you cancelled) */
{
	...
	if (gStandardFile58)	/* in system 7 or later, all the work is done for us */
		StandardGetFile(0L, 2, theTypes, &reply);	/* reply's got an FSSpec in it afterwards */
	else	/* pre-system-7, we have to do a bit of work manually */
	{
		...
		SFGetFile(where, "\p", 0L, 2, theTypes, 0L, &oldReply);	/* old routine */
		...
		if (reply.sfGood)	/* if user selected a file */
		{
			...
			/* get the name */
			Mymemcpy(reply.sfFile.name, oldReply.fName, oldReply.fName[0]+1);
   ...
   do it manually. */
{
	...
	else
	{
		...
		myFSS->parID=parID;
		Mymemcpy(myFSS->name, fileName, fileName[0]+1);
	}
