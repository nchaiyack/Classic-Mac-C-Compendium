...
LCDEFRadioButton::Draw3DEffects(void) const
{
	...
	switch (value) {
		case Value_Off	:	// empty/convex
			...
			if ( !mIndentWhenHighlighted ) {
				DrawConvexRadioBtn();
			} else {
				DrawEmbeddedRadioBtn();
			}
			break;
...
void LCDEFRadioButton::DrawConvexRadioBtn(void) const
{
	ForeGrey(0xEEEE);
...
void LCDEFRadioButton::DrawConcaveRadioBtn(void) const
{
	ForeGrey(0xEEEE);
...
void LCDEFRadioButton::DrawEmbeddedRadioBtn(void) const
{
	Rect	radioBox = mRadioCheckRect;
	::InsetRect(&radioBox, -1, -1);
