...
void InitNimOneGame(void)
{
	short			i, val;
	
	gThisGameNumRows=gNimNumRows;
	gThisGameMaxPerRow=gNimMaxPerRow;
	gThisGameDistribution=gNimDistribution;
...
void NimDrawWindow(WindowRef theWindow, short theDepth)
{
	Rect			destRect;
	short			i;
	WindowRef		curPort;
...
void NimClick(WindowRef theWindow, Point thePoint)
{
...
void NimIdle(WindowRef theWindow)
{
	CheckNimEndGame(theWindow);
...
static	void DoNimComputerMove(WindowRef theWindow)
{
	...
	if (nimSum==magicSum)
	{	/* do random move */
		do { i=(Random()&0x7fff)%gThisGameNumRows; } while (gNimRowValues[i]==0);
		ShowNimComputerMove(theWindow, i, 1+(Random()&0x7fff)%gNimRowValues[i]);
	}
	else
	{	/* do calculated move */
		foundMagic=FALSE;
