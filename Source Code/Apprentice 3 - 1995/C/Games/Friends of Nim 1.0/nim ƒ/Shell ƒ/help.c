...
void OpenTheHelpWindow(WindowRef theWindow)
{
	TEHandle		hTE;
...
void DisposeTheHelpWindow(WindowRef theWindow)
{
	TEHandle		hTE;
...
void KeyPressedInHelpWindow(WindowRef theWindow, unsigned char keyPressed)
{
	ObscureCursor();
...
void MouseClickedInHelpWindow(WindowRef theWindow, Point mouseLoc)
{
	short			i;
	...
	if (newMain!=-1)
	{
		Draw3DButton(&gMainTopicRect[newMain], gMainTopicTitle[newMain], 0L,
			GetWindowDepth(theWindow), kSquareButton+kButtonIsDown+kButtonIsPopup);
		...
		{
			Draw3DButton(&gMainTopicRect[newMain], gMainTopicTitle[newMain], 0L,
				GetWindowDepth(theWindow), kSquareButton+kButtonIsPopup);
		}
...
void DrawTheHelpWindow(WindowRef theWindow, short theDepth)
{
	GrafPtr			curPort;
	short			i;
	Rect			tempRect;
	...
	for (i=0; i<gNumMainTopics; i++)
	{
		Draw3DButton(&gMainTopicRect[i], gMainTopicTitle[i], 0L, theDepth, kSquareButton+kButtonIsPopup);
	}
