...
void GammaFadeOut(void)
{
	...
	/* Grab the main deviceÕs gamma table; VideoStatus in Sample.a */
	error = MyVideoStatus( (**mainDevice).gdRefNum, cscGetGamma,
			(Ptr)&currentGamma );

	/* Grab a pointer to the main deviceÕs gamma table */
	...
	/* Copy main deviceÕs gamma into new gamma except for table itself */
