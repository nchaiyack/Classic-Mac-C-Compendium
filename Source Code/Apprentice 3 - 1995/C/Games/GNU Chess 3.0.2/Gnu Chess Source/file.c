	...
	else if (c == 0 && i > 0)
	  {
	    ...
	    if (entry!=NULL && mp!=NULL) {
		    ...
		    *mp = 0;
			...
			if (entry != NULL) free((Ptr)entry);
			if (mp != NULL) free((Ptr)mp);
		}
	  }
      fclose (fd);
    }
...
void SaveGame()
{
	...
	SFPutFile(where, (ConstStr255Param)"", (ConstStr255Param)fileName, 0L, &reply);
	if (!reply.good) return;
	FSDelete(reply.fName, reply.vRefNum);
	Create( reply.fName, reply.vRefNum, 'GCHS', 'GCsg' );
	if ( FSOpen( (ConstStr255Param)reply.fName, reply.vRefNum, &outRef ) == noErr) {
		SetCursor(*ClockCursor);
		count = sizeof(header);
...
ListGame ()
{
	...
	if ( FSOpen( (ConstStr255Param)reply.fName, reply.vRefNum, &outRef ) == noErr) {
		SetCursor(*ClockCursor);
		theString = GetString(_LISTHEAD_STR);
		HLock((Handle)theString);
		FSWriteStr( outRef, (char *)"\p\r");
  		FSWriteStr( outRef, *(char **)theString);
  		FSWriteStr( outRef, (char *)"\p         ");
  		FSWriteStr( outRef, *(char **)theString);
		FSWriteStr( outRef, (char *)"\p\r");
