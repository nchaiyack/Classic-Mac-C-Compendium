...
static int const_cmp_kind(Const cleft, Const cright)		/*;const_cmp_kind*/
{
	...
	if (ckind == CONST_OM) chaos(__FILE__, __LINE__, "const comparison left operand not defined");
	if (ckind != cright->const_kind) {
#ifdef DEBUG
		zpcon(cleft); 
		zpcon(cright);
#endif
		chaos(__FILE__, __LINE__, "const comparison operands differing kinds");
	}
	return ckind;
}
...
int const_cmp_undef(Const cleft, Const cright)		/*;const_cmp_undef*/
{
	...
	zpcon(cright);
#endif
	chaos(__FILE__, __LINE__, "const comparison not defined for these constant types");
...
void node_free(Node node)									/*;node_free*/
{
	 ...
	 */
	chaos(__FILE__, __LINE__, "node free");
 ...
 * called AS
 */
char *kind_str(unsigned short as)		/*;kind_str*/
{
...
char *nature_str(short na)								/*;nature_str*/
{
...
char *newat_str(char *ttt)                                                                                    /*newat_str*/
{
/*
* generates a unique string of the form tttn8888 which is guaranteed
* not to have been declared elsewhere, even in a different
* compilation unit
*/
...
for ( ;
   sprintf(s = smalloc(6+strlen(ttt)), "%sn%04d", ttt, n),
   dcl_get(DECLARED(scope_name), s);
   n += 1)
	;
return s;
}

char *str_newat(char *ttt)											/*;str_newat*/
{
return newat_str(ttt);
}
...
Symbol sym_new_noseq(short na)							/*;sym_new_noseq*/
{
...
Symbol sym_new(short na)										/*;sym_new*/
{
	/* allocate new symbol table entry, nature na.
	...
	if (seq_symbol_n > (int) seq_symbol[0])
		 chaos(__FILE__, __LINE__, "sym_new seq_symbol_n exceeds allocated length");
	if (seq_symbol_n == (int)seq_symbol[0]) {
...
/* Not Used */
static int sym_free(Symbol sym);
...
char *attribute_str(short attrnum)						/*;attribute_str*/
{
	...
	if (attrnum > 52) chaos(__FILE__, __LINE__, "attribute_str: invalid internal attriubte code");
...
int in_qualifiers(unsigned short kind)						/*;in_qualifiers*/
{
...
{
	chaos(__FILE__, __LINE__, strjoin(s, " not implemented"));
}
...
#ifdef DEBUG
	//fprintf(stdout, "const of kind %d, expect %d\n", con->const_kind, ctyp);
#endif
	chaos(__FILE__, __LINE__, "const not of expected kind");
}
...
Node node_new_noseq(unsigned short na)					/*;node_new_noseq*/
{
	char *np;
	Node p;
	int	i;
...
Node node_new(unsigned short na)									/*;node_new*/
{
	Node p;
	...
	if (seq_node_n > (int) seq_node[0]) 
		chaos(__FILE__, __LINE__, "node_new seq_node_n exceeds allocated length");
