...
Segment main_data_segment(void) 						/*;main_data_segment*/
...

{
...
#ifdef MAC_GWADA
	axq_file = ifopen(PREDEFNAME, ".axq", "rb", 0);
#else
	...
	for (rec=read_init(axq_file); rec != 0; rec=read_next(axq_file, rec)) {
		...
		if(nsegs != 1) chaos(__FILE__, __LINE__, "read_predef_axq data segment number invalid");
