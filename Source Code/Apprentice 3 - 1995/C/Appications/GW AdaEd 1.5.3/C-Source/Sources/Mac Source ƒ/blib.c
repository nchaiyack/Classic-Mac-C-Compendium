...
static char *fab_read_binding_ais(char *fname, char *uname);
...
static void main_code_segment()						/*;main_code_segment */
{
	...
	if (symbol_main_task_type == (Symbol)0)
		chaos(__FILE__, __LINE__, "glib.c main_code_segment  symbol_main_task_type not defined");
...
ifile = fab_ifopenread(fname, "axq", 0);
if (is_predef) {
...
for (rec = fab_read_init(ifile); rec != 0; rec = fab_read_next(ifile, rec)) {
	...
	if (!is_main_unit) { /* read only if NOT main unit (it has no ais info*/
		...
		sym = fab_getsym(ifile/*, "ud-unam"*/);
		ud->ud_unam = sym;
		ud->ud_useq = S_SEQ(sym);
		ud->ud_unit = S_UNIT(sym);
		fab_get_unit_unam(ifile/*, sym*/);
