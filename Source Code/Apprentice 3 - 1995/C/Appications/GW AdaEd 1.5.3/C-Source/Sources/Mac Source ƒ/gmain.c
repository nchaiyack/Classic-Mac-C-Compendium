...
static void preface(void);
static void exitf(int);
static void init_gen(void);
static void finit_gen(void);
...
IFILE *TREFILE;
#endif
IFILE	*AISFILE, *AXQFILE, *STUBFILE;
...
void adamain (int argc, char **argv);
...
#if __profile__
if (noErr == ProfilerInit(collectSummary, bestTimeBase // best is new with CW5
, 1100, 700)) {
#endif
	while ((c = getopt (argc, argv, "g:l:m:ni:")) != EOF)
		...
		case 'm': /* specify main unit name */
			MAINunit = mmalloc((unsigned) strlen(optarg)+1);
	...
	if (errflg) {
		// How do we display this message?
		ShowMessage("Internal Error (usage): adagen -aAbglnmMnrstw file");
		exitp(RC_ABORT);
	}
...
//	}
	 ...
	 */
#ifdef MAC_GWADA
	/* Previous comment not true on the mac.  A file name must always
	 * be specified.
	 */
	...
	PtoCstr(fileNoExt);
	MSGFILE = efopenl((char *)&fileNoExt, "msg", "a");
#else
	...
	ParseFile(tmpStr, flPath+flName, fileNoExt);
	PtoCstr(fileNoExt);
	errfile = efopenl((char *)&fileNoExt, "err", "a");
#endif
...
// commented out, perhaps we'll run faster [Fabrizio Oddone]
//	setbuf(MSGFILE, (char *) 0);
	...
	if (!errors) {
		...
		// this code was in exitf() - had to moved here because the
		// mac remove() routine will not remove files if they are
		// still opened.  So, close all files and then remove them
...
//	DebugStr("\pProfilerDump imminent!");
	if (ProfilerDump("\padagen.prof"))
		SysBeep(30);
	ProfilerTerm();
	}
#endif
	// this was exitf(), but since files were closed up above, then
	// call exitp() directly.
	exitp(RC_SUCCESS);
#else
	// this section changed enough... this would be an invalid
	// conditional section.
...
static void preface(void)										/*;preface*/
{
	...
	if (gen_option) {
		 ...
		 */
#ifdef MAC_GWADA
		TREFILE  = fab_ifopenread(AISFILENAME, "aic", 0);
#else
		...
		for (indx = 1; indx <= last_index; indx++) {
			...
			unit_name =
#ifdef MAC_GWADA
		fab_read_ais(AISFILENAME, (char *) 0, indx, TRUE, TRUE)
#else
		read_ais(AISFILENAME, TRUE, (char *) 0, indx, TRUE)
#endif
			;
#ifdef MAC_GWADA
			TREFILE  = fab_ifopenread(AISFILENAME, "trc", 0);
#else
		...
		load_tre
			...
			if (errors) {
				...
				user_info(strjoin(formatted_name(unit_name), "abandoned"));
			}
			else {
...
static void init_gen(void)											/*;init_gen*/
{
	/*
...
#ifdef MAC_GWADA
		fab_read_ais(AISFILENAME, body_name, 0, TRUE, TRUE)
#else
...
static void finit_gen(void)											/*;finit_gen*/
{
	/*
...
//	int i;
	...
	if (errors) {
		...
		//i = getc(stdin);
		// What to do to display this message?
#else
		_outtext(msg);
