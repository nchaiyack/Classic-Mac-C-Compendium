...
#define	isatty(t)	true	// I couldn't find this routine anywhere,
#endif		// so I set it to true (false did not work) [Fabrizio Oddone]
...
#ifdef MAC_GWADA
static void xputc(char A, FILE *B);

void xputc(char A, FILE *B)
{
...
/* Checks that an fopen succeeded, exec_raise USE_ERROR if not */
...
#ifdef MAC_GWADA
		if (IOFDESC == stdin) {
			c = scrGetc();			// stay out until we get a key event
		}
		else
			c = fgetc(IOFDESC);
#else
...
/* if need to distinguish binary and text files */
static FILE *open_in_default_vol(char *fname, char *mode)
{
	FILE *fp;
	char *fullName = NULL;

	// We use the default volume if and only if the file name
	// starts with ":" and contains a ":".  This
	// means the path is a full path...  partial paths, or just
	// file name start at the default volume...
	...
	if (fname[0] != ':') {
		...
		for (i = 0; i < len; i++) {
			if (fname[i] == ':') {
				// We have a FULL PATH name - no initial ":" and at
				// least one ":" inside the name
	
				// do I/O using fname that was passed in (unaltered)
				fullName = malloc(strlen(fname) + 1);
	...
	if (fullName == NULL) {
		// We don't have a colon in the name, name passed in is the
		// file name only
		Str255 volName;
	
		GetDataFilePath(volName);
		fullName = mmalloc(volName[0] + strlen(fname) + 1);
		BlockMoveData(&volName[1], fullName, volName[0]);
		fullName[volName[0]] = '\0';
...
FILE *fopen_bin(char *fname, char *fmode)					/*;fopen_bin*/
{
	...
	strcpy(mode,fmode);
	strcat(mode,"b");
	return open_in_default_vol(fname, mode);
}
