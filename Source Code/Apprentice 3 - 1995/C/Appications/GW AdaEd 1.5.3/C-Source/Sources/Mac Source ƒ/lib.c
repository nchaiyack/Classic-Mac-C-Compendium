...
int init_predef(void)				/*;init_predef*/
{
	char *lname;
	char *t_name;
...
#ifdef MAC_GWADA
	/* Open this file in binary mode on the Mac */
	LIBFILE = fab_ifopenread("predef", "lib", 0);
#else
...
int retrieve(char *name)							/*;retrieve*/
{
	...
	if (!streq(fname, AISFILENAME) && !in_aisunits_read(name)){
		if (
#ifdef MAC_GWADA
		fab_read_ais(fname, name, 0, TRUE, FALSE)
#else
		read_ais(fname, FALSE, name, 0, TRUE)
#endif
...
char *unit_name_names(char *u)				/*;unit_name_names*/
{
	...
	if (u == NULL || strlen(u) <= 2)
	    chaos(__FILE__, __LINE__, "unit_name_names: invalid unit name");
...
int is_subunit(char *u)				/*;is_subunit*/
{
	...
	if (u == NULL) chaos(__FILE__, __LINE__, "is_subunit: null pointer");
...
Unitdecl unit_decl_new(void)				/*;unit_decl_new*/
{

	return (Unitdecl) fcalloct(sizeof(Unitdecl_s), "unit-decl-new");
}

Stubenv stubenv_new(void)					/*;stubenv_new*/
{
	return (Stubenv) fcalloct(sizeof(Stubenv_s), "stubenv-new");
}
...
Symbol getsymptr(short seq, short unit)		/*;getsymptr*/
{
	...
	if (unit == 0 ) {
		...
		if (seq>0 && seq <= tup_size(init_symbols)) {
			...
			chaos(__FILE__, __LINE__, "unit 0 error getsymptr");
	}
	if (unit <= unit_numbers) {
		...
		if (seq <= tup_size(symptr)) {
			...
			chaos(__FILE__, __LINE__, "getsymptr error"); return (Symbol) 0;
 	}
	chaos(__FILE__, __LINE__, "getsymptr unable to find node"); return (Symbol) 0;
}
