...
static void putdcl(IFILE *ofile, Declaredmap d)						/*;putdcl*/
{
	...
	save_filedmap = filedmap = (f_dmap_s *)
	  fcalloct(n * sizeof(f_dmap_s), "put-dcl-save-filedmap");
	dptrs =
...
static void putunt(IFILE *ofile, char *desc, unsigned int n)		/*;putunt*/
{
	...
	if (n > 200) chaos(__FILE__, __LINE__, "putunt: absurd unit number");
...
static void putnval(IFILE *ofile, Node node)					/*;putnval*/
{
	...
	if (nk == as_simple_name || nk == as_int_literal || nk == as_real_literal
	  ...
	  || nk == as_task_stub) {
		...
		if (ck == CONST_INT)
			...
			chaos(__FILE__, __LINE__, "putnval: CONST_CONSTRAINT_ERROR");
		}
	}
		...
		if (stup != (Tuple)0) {
			n = tup_size(stup);
			if (n != 2)
				chaos(__FILE__, __LINE__, "putnval: bad nval for instantiation");
	 ...
	 * 			now attached to n_list of as_record_aggregate )
...
static void putovl(IFILE *ofile, Symbol sym)					/*;putovl*/
{
	...
	if (nat == na_package|| nat == na_package_spec
	  ...
	  || nat == na_task_type || nat == na_task_obj) {
		...
		chaos(__FILE__, __LINE__, "putovl surprising case!");
	}
}
...
array_case:
	...
	case	na_block:
		...
		chaos(__FILE__, __LINE__, "putsig: signature for block");
		break;
	case	na_constant:
	case	na_in:
	case	na_inout:
	...
	case	na_generic_procedure_spec:
		if (tup_size(sig) != 4)
			chaos(__FILE__, __LINE__, "putsig: bad signature for na_generic_procedure_spec");
	...
	case	na_generic_package:
		...
		if (tup_size(sig) != 5)
			chaos(__FILE__, __LINE__, "putsig: bad signature for na_generic_package_spec");
		tup = (Tuple) sig[1];
		/* the first component is a tuple of pairs, just write count
		 * and the values of the successive pairs 
		 */
...
record_case:
	...
	case	na_void:
		...
		if (streq(ORIG_NAME(sym), "$used") ) {
			...
			zpsym(sym);
#endif
			chaos(__FILE__, __LINE__, "putsig: na_void, not $used");
		}
		break;
	case	na_obj:
	...
	default:
		...
		zpsym(sym);
#endif
		chaos(__FILE__, __LINE__, "putsig: default");
	}
}
...
void write_stub(Stubenv ev, char *stub_name, char *ext)			/*;write_stub*/
{
	...
	if (STUBFILE == NULL) {
		FileType('ASTn');
		STUBFILE = ifopen(AISFILENAME, ext, "wb", 0);
	}
	ofile = STUBFILE;
	begpos = write_next(ofile); /* start record*/
...
/* uindex - unit number */
{
	...
	rara = (long *)fcalloct((nodes+1)* sizeof(long), "write-tre-rara");
	dpos = iftell(ofile);
...
/* or is this any SVR4 system? */
	...
	long numFiles;
	long count;
	Str255	fullPath;	// fullPath to the library
	FSSpec fsSpec;		// folder
	FSSpec file;		// files in folder
//	OSErr	err;
#endif
...
//	char *s2;
	...
	// loop for all items in this directory: LIBRARY_PREFIX

	// Make a Pascal string out of LIBRARY_PREFIX
	BlockMoveData(LIBRARY_PREFIX, &fullPath[1], strlen(LIBRARY_PREFIX));
	StrLength(fullPath) = strlen(LIBRARY_PREFIX);
	FSMakeFSSpecCompat(0, 0, fullPath, &fsSpec);

	numFiles = CountItemsInFolder(&fsSpec);
	for (count = 1; count <= numFiles; count++) {

		GetItemInFolder(count, &fsSpec, &file);
		if (!ItemIsFolder(&file) && AdaGetFullPath(&file, fullPath)) {
			Str255 name, ext;

			// get full path name to 'file', put it in s1

			ParseFile(fullPath, flName, name);
			ParseFile(fullPath, flExtension, ext);

	   		/* ignore files that don't have a dot in it. */
	   		if (ext[0] == 0)
	   			continue;

	   		/* only consider of files of the form xxx.yyy where
	   		 * yyy is one of the Ada/Ed extensions 
	   		 */

			PtoCstr(name);
			PtoCstr(ext);
	   		if ((streq((char *)&ext,".trc") ||	// ParseFile returns
	   			 streq((char *)&ext,".axq") ||	// the '.' as part of
	   			 streq((char *)&ext,".st1") ||	// the extension.
			 	 streq((char *)&ext,".st2") ||
			 	 streq((char *)&ext,".exe"))
			 	 && !tup_memstr((char *)&name, active_files)) {

	      		FSpDeleteCompat(&file);		// ignore errors
	   		}
		}
	}

	/* remove the current aic file */
	s1 = strjoin(AISFILENAME, ".AIC");
	CtoPstr(s1);
	FSMakeFSSpecCompat(fsSpec.vRefNum, fsSpec.parID, (unsigned char *)s1, &file);
	FSpDeleteCompat(&file);
...
void ifdelete(char *fname)										/*;ifdelete*/
{
	...
	if (strlen(LIBRARY_PREFIX)) { /* prepend library prefix if present */
		...
		if (strcmp(&tname[len1-len2], DIR_DELIMITER) != 0)
			strcat(tname, DIR_DELIMITER);
