...
static Slot *fab_get_slot(fabIFILE *file, int nmax, int *dim);
static Slot *get_slot(IFILE *, int, int *);


Segment segment_new(short kind, int initdim)		/*;segment_new*/
{
Segment	s;
...
kind = fab_getnum(file/*,"segment-kind"*/);
dim = fab_getnum(file/*,"segment-dim"*/);
seg = segment_new(kind, dim);
//DebugStr("\pfab_segment_read");
fab_fread(seg->seg_data, seg->seg_size * dim, file->fh_file);
...
efclose(axq_file);
#endif
for (rec = fab_read_init(axq_file); rec != 0; rec = fab_read_next(axq_file, rec)) {
	funame = fab_getstr(axq_file/*,"axq-unit-name"*/);
	(void)fab_getnum(axq_file/*,"axq-num"*/);
	...
	genpos = fab_getlong(axq_file/*,"axq-gen-pos"*/);
	/* position to start of generator info */
	fab_ifseek(axq_file/*, "axq-gen_info"*/, genpos);
	for (si=0;si<4;si++) {
		nsegs = fab_getnum(axq_file/*,"number-segments"*/);
		for (sj=1; sj <= nsegs; sj++) {
			snum = fab_getnum(axq_file/*,"axq-segment-num"*/);
			newseg = fab_segment_read(axq_file);
			if (newseg->seg_kind == SEGMENT_KIND_CODE) {
				if (snum >= axq->axq_code_segments_dim)  {
					chaos(__FILE__, __LINE__, "code_segment_number error");
				}
			...
			if (newseg->seg_kind==SEGMENT_KIND_DATA) {
				if (snum >= axq->axq_data_segments_dim)  {
					chaos(__FILE__, __LINE__, "data_segment number too big");
				}
...
start_pos = fab_iftell(file); /* save pos so can restore after reading slots*/
dpos = file->fh_slots;/* get offset for slots */
if (dpos == 0l)
	chaos(__FILE__, __LINE__, "get_cde_slots: slot offset zero");
fab_ifseek(file, dpos);/* position to start of slot info */
n_code = fab_getnum(file/*,"n-code"*/);
n_data = fab_getnum(file/*,"n-data"*/);
n_exception = fab_getnum(file/*,"n-exception"*/);
axq->axq_code_slots_dim = n_code + 1;
axq->axq_code_slots = fab_get_slot(file,n_code,&dim);
axq->axq_code_segments_dim = dim;
axq->axq_data_slots_dim = n_data + 1;
axq->axq_data_slots = fab_get_slot(file,n_data,&dim);
axq->axq_data_segments_dim = dim;
axq->axq_exception_slots_dim = n_exception + 1;
axq->axq_exception_slots = fab_get_slot(file, n_exception, &dim);
axq->axq_exception_slots_dim = dim;
fab_ifseek(file, start_pos);/* restore position */
...
n = fab_getnum(file/*,"slot-entries"*/);
for (i = 1; i <= n; i++) {
	exists = fab_getnum(file/*,"slot-exists"*/);
	if (exists) {
		slot = (Slot) emalloct(sizeof(Slot_s), "get-slot-1");
		slot->slot_seq = fab_getnum(file/*,"slot-seq"*/);
		slot->slot_unit = fab_getnum(file/*,"slot-unit"*/);
		slot->slot_number = fab_getnum(file/*,"slot-number"*/);
		slot->slot_name = fab_getstr(file/*,"slot_name"*/);
#ifdef MONITOR
		slot->slot_file = fab_getstr(file/*,"slot_file"*/);
#endif
...
#ifdef MONITOR
			slot->slot_file = getstr(file,"slot_file");
#endif
			if (slot->slot_number >= *dim) {
				*dim = slot->slot_number + 1;
			}
