...
static void fab_get_local_ref_maps(fabIFILE *ifile, int units);
static void get_local_ref_maps(IFILE *, int);
static void put_local_ref_maps(IFILE *, int);
static void relocate_slots_a(void);
static void relocate_slots_b(void);
...
int load_unit(char *unit, int tree_is_needed)					/*;load_unit*/
{
	...
	if (fname == (char *)0) {
		...
		file_retrieved = 
		  (
#ifdef MAC_GWADA
		fab_read_ais(fname, unit, 0, tree_is_needed, FALSE) != NULL);
#else
		read_ais(fname, FALSE, unit, 0, tree_is_needed) != NULL);
#endif
...
void load_library(Axq axq)									/*;load_library*/
{
...
#ifdef MAC_GWADA
	unit_count = fab_getnum(ifile/*, "lib-unit-count"*/);
	n = fab_getnum(ifile/*, "lib-n"*/);
	empty_unit_slots = fab_getnum(ifile/*, "lib-empty-slots"*/);
	tmp_str = fab_getstr(ifile/*, "lib-tmp-str"*/);
#else
...
#ifdef MAC_GWADA
		uname = fab_getstr(ifile/*, "lib-unit-name"*/);
		unumber = fab_getnum(ifile/*, "lib-unit-number"*/);
		aisname = fab_getstr(ifile/*, "lib-ais-name"*/);
		compdate = fab_getstr(ifile/*, "comp-date"*/);
		symbols = fab_getnum(ifile/*, "lib-symbols"*/);
		nodes = fab_getnum(ifile/*, "lib-nodes"*/);
#else
...
#ifdef MAC_GWADA
		pUnit->isMain = fab_getnum(ifile/*, "lib-is-main"*/);
#else
...
#ifdef MAC_GWADA
		comp_status = fab_getnum(ifile/*, "lib-status"*/);
#else
	...
	for (i = 1; i <= n; i++) {
...
#ifdef MAC_GWADA
		parent = fab_getnum(ifile/*, "lib-parent"*/);
#else
...
#ifdef MAC_GWADA
		cur_level = fab_getnum(ifile/*, "lib-cur-level"*/);
#else
...
#ifdef MAC_GWADA
		m = fab_getnum(ifile/*, "stub-file-size"*/);
#else
		...
		for (j = 1; j <= m; j++)
#ifdef MAC_GWADA
			tup[j] = (char *) fab_getnum(ifile/*, "stub-files"*/);
#else
...
#ifdef MAC_GWADA
	n = fab_getnum(ifile/*, "precedes-map-size"*/);
#else
	...
	for (i = 1; i <= n; i += 2) {
...
#ifdef MAC_GWADA
		PRECEDES_MAP[i] = (char *) fab_getnum(ifile/*, "precedes-map-ent"*/);
		m = fab_getnum(ifile/*, "precedes-map-set-size"*/);
#else
		...
		for (j = 1; j <= m; j++) {
			...
			precedes = set_with(precedes,
			  (char *) fab_getnum(ifile/*, "precedes-map-ent"*/));
#else
			precedes = set_with(precedes,
...
#ifdef MAC_GWADA
	n = fab_getnum(ifile/*, "compilation_table_size"*/);
#else
	...
	for (i = 1; i <= n; i++)
#ifdef MAC_GWADA
		compilation_table[i] = (char *) fab_getnum(ifile/*, "compilation-table-ent"*/);
#else
		compilation_table[i] = (char *) getnum(ifile, "compilation-table-ent");
#endif
	/* late_instances */
#ifdef MAC_GWADA
	n = fab_getnum(ifile/*, "late-instances-size"*/);
#else
	...
	for (i = 1; i <= n; i++)
#ifdef MAC_GWADA
		late_instances[i] = fab_getstr(ifile/*, "late-instances-str"*/);
	n = fab_getnum(ifile/*, "interfaced-procedures-size"*/);
#else
...
#ifdef MAC_GWADA
		  (char *) fab_getnum(ifile/*, "interfaced-procedures-num"*/);
		interfaced_procedures[i+1]= fab_getstr(ifile/*, "interfaced-procedures-str"*/);
#else
...
#ifdef MAC_GWADA
	interface_counter = fab_getnum(ifile/*, "interface-counter"*/);
	n = fab_getnum(ifile/*, "units-size"*/);
	for (i = 1; i <= n; i++) {
		pUnits[i]->libInfo.currCodeSeg =
		  (char *) fab_getnum(ifile/*, "current-code-seg"*/);
	}
	n = fab_getnum(ifile/*, "units-size"*/);
#else
...
#ifdef MAC_GWADA
	fab_get_local_ref_maps(LIBFILE, n);
	cde_pos = fab_get_cde_slots(LIBFILE, axq);
#else
...
for (unit = 1; unit <= units; unit++) {
	...
	defined = fab_getnum(ifile/*, "local-ref-map-defined"*/);
	if (!defined) continue;
	n = fab_getnum(ifile/*, "local-ref-map-size"*/);
	local_ref_map = tup_new(n);
	pUnits[unit]->libInfo.localRefMap = (char *) local_ref_map;
	for (i = 1; i <= n; i += 2) {
		sym = fab_getsymref(ifile/*, "local-ref-map-sym"*/);
		local_ref_map[i] = (char *) sym;
		off = fab_getnum(ifile/*, "local-ref-map-off"*/);
...
void write_glib(void)											/*;write_glib*/
{
...
//	LIBFILE = ofile;
		...
		if (compiling_predef) { /* trace for predef build */
			...
			if (i <= 14) {
				if (!streq(pUnit->name, predef_unit_name(i))) {
					chaos(__FILE__, __LINE__, "predef unit name error");
				}
			}
		}
	...
	put_local_ref_maps(ofile, unit_numbers);
	put_cde_slots(ofile/*, 0*/);/* write slots info and close file */
//	LIBFILE = NULL;
}

static void relocate_slots_a(void)							/*;relocate_slots_a*/
{
...
static void relocate_slots_b(void)							/*;relocate_slots_b*/
{
	int 	i, n;
	Slot 	slot;
	Symbol 	sym;
...
#ifdef MAC_GWADA
	FileType('ASTn');
	STUBFILE = ifopen(AISFILENAME, "st2", "r+b", 0);
#else
...
#ifdef MAC_GWADA
	FileType('AAXQ');
	AXQFILE = ifopen(AISFILENAME, "axq", "r+b", 0);
#else
...
#ifdef MAC_GWADA
		fab_read_ais(fname, uname, 0, FALSE, FALSE);
#else
