...
static Symbol getsym(IFILE *ifile, char *desc)								/*;getsym*/
{
	Symbol	sym, tmp_sym;
...
Node getnodptr(short seq, short unit)		/*;getnodptr*/
{
	Tuple	nodptr;
	Node	node;
	...
	if (unit == 0) {
		...
		if (seq>0 && seq <= tup_size(init_nodes)) {
			...
			chaos(__FILE__, __LINE__, "error for unit 0 in getnodptr");
		}
	}
	if (unit <= unit_numbers) {
		struct unit *pUnit = pUnits[unit];
		nodptr = (Tuple) pUnit->treInfo.tableAllocated;
		if (seq == 0) chaos(__FILE__, __LINE__, "getnodptr seq 0");
	...
	chaos(__FILE__, __LINE__, "getnodptr unable to find node");
  ...
  int comp_index, int tree_is_needed)  /*;read_ais*/
{
	...
	if (is_aic_file) {
		...
		FileType('AAIC');
		ifile = ifopen(full_fname, "aic", "rb", 0);
#else
...
#ifdef MAC_GWADA
		FileType('AAXQ');
		ifile = ifopen(full_fname, "axq", "rb", 0);
#else
...
int read_lib(void)					/*;read_lib*/
{
	...
	tmp_str = fab_getstr(ifile/*, "tmp-str"*/);
#else
	...
	for (i = 1;i <= unit_count; i++) {
		...
		pUnit = pUnits[fab_getnum(ifile/*, "lib-unit-number"*/)];
	...
	for (i = 1; i <= n; i++) {
		...
		for (j = 1;j <= m;j++)
#ifdef MAC_GWADA
			tup[j] = (char *) fab_getnum(ifile/*, "stub-file"*/);
#else
...
Tuple add_tree_node(Tuple tup, Node nod)				/*;add_tree_nodes */
{
int		seq;
...
void retrieve_generic_tree(Node node1, Node node2)	/*;retrieve_generic_tree*/
{
	...
	if (tup_size(tup) != 0) {
		unum = N_UNIT(node1);
		fab_retrieve_tree_nodes(NULL, tup, unum);
	}
}

char *lib_aisname(void)										/*;lib_aisname*/
{
...
#ifdef MAC_GWADA
	LIBFILE = fab_ifopenread(LIBFILENAME, "", 0);
#else
	LIBFILE = ifopen(LIBFILENAME, "", "r", 0);
#endif
	ifile = LIBFILE;
#ifdef MAC_GWADA
	spos = fab_iftell(ifile); /* get current offset in file */
	unit_count = fab_getnum(ifile/*, "lib-unit-count"*/);
	n = fab_getnum(ifile/*, "lib-n"*/);
	empty_unit_slots = fab_getnum(ifile/*, "lib-empty-slots"*/);
	tmp_str = fab_getstr(ifile/*, "tmp-str"*/);
...
static void get_unit_unam(IFILE *ifile, Symbol sym)			/*;get_unit_unam*/
/*  
