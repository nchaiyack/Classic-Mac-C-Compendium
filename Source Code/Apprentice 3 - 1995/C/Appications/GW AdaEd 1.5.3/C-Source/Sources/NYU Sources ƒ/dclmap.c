...
static unsigned short dcl_lookup(char *s)			/*;dcl_lookup*/
{
	...
	if ((DSTRINGS_MAXLEN - dstrings_maxlen) < dstrings_expand)
		capacity(__FILE__, __LINE__, "dstrings full");
...
Declaredmap dcl_new(short nh)	/*;dcl_new*/
{
/* Allocate declared map with nh hash headers(0->10 headers */

Declaredmap dm;

// faster than the stupid ANSI calloc [Fabrizio Oddone]
dm = (Declaredmap) fcalloct(sizeof(Declaredmap_s), "dcl-new-dm");

if (nh == 0)
	nh = 4;
dm->dmap_curlen = 0;
dm->dmap_maxlen = nh;
dm->dmap_table = (struct Dment *) fcalloct(nh * sizeof(Dment), "dcl-new-dmap-table");
...
static Symbol dcl_getp(Declaredmap dmap, char *s, int ifvis) /*;dcl_getp*/
{
	...
	if (dmap == (Declaredmap) 0)
		chaos(__FILE__, __LINE__, "dcl_getp: declared map null pointer");
	idnum = dcl_lookup(s);
	p = dmap->dmap_table;
...
void dcl_put_vis(Declaredmap dmap, char *s, Symbol sym, Boolean vis)
/*;dcl_put_vis*/
{
	...
	if (dmap == (Declaredmap) 0)
		chaos(__FILE__, __LINE__, "dcl_put: declared map NULL");
	idnum = dcl_lookup(s);
	p = dmap->dmap_table;
#ifdef DEBUG
	...
	for (i = 0; i < dmap->dmap_curlen;(i++, p++)) {
		if (idnum == p->dment_i.dment_idnum) {/* if match */
			chaos(__FILE__, __LINE__, strjoin("dcl_put_vis found duplicate entry", s));
...
//	Fordeclared fd;
	...
	if (dm == (Declaredmap) 0) chaos(__FILE__, __LINE__, "dcl_copy: declared map NULL");
...
dmap_chk(Declaredmap dmap, char *lab)				/*;dmap_chk*/
{
	...
	for (i=0; i<dmap->dmap_curlen; i++, p++) {
		if (p->dment_symbol == (Symbol)0) {
			...
			chaos(__FILE__, __LINE__, "dmap_chk");
		}
	}
}
