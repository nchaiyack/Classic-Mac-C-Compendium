...
void gen_kvc(int opc, int k, Const ref, char *c)					/*;gen_kvc*/
{
	...
	if (ctype == CONST_INT) {
		...
		chaos(__FILE__, __LINE__, "gop const undefined case");
	}
}
...
void assemble(Op op)										/*;assemble*/
{
	...
	if (code == I_LABEL) {
		...
		chaos(__FILE__, __LINE__, "I_EQUAL opcode encountered");
	}
	else if (code == I_END) {
		return;
	}
	...
	if (code == I_DATA || code == I_CASE_TABLE) {
		...
		case(A_IMM):
			...
			if (type == OP_INT) { /* handle integer immediate values */
				...
				zpop(op);
#endif
				chaos(__FILE__, __LINE__, "gen.c A_IMM not supported for this case");
			}
			break;
...
static void asm_exception(Symbol sym)					/*;asm_exception*/
{
	...
	if (exists) {
		...
		chaos(__FILE__, __LINE__, "gen.c: cannot find exception value ");
	}
}
...
static char *gs_end(void)									/*;gs_end*/
{
