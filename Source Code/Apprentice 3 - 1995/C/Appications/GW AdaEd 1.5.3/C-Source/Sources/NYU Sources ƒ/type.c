...
#define NAMESIZE  119
...

		{
...

			{
				...
				AbortMessage("Cannot open CWKLIB", RC_ABORT);
...

{
	...
	for (value = lbd; value <= ubd; value++) {
		...
		if (str == (char *) 0) {
			...
			chaos(__FILE__, __LINE__, "type.c: init_enum cannot find literal value");
...

{
	...
	case(co_index):
		...
		if ((nb_dim == 1) && global_flag) {
			...
			if (low_const->const_kind == CONST_INT)
				...
				chaos(__FILE__, __LINE__, "low bound not int");
			...
			if (high_const->const_kind == CONST_INT)
				...
				chaos(__FILE__, __LINE__, "high bound not int");
	...
	case(co_delta):
		...
		if (consT->const_kind != CONST_RAT)
			...
			chaos(__FILE__, __LINE__, "arg not rational");
...

{
	...
	if (value->const_kind != CONST_RAT || small->const_kind != CONST_RAT) {
		...
		chaos(__FILE__, __LINE__, "rat_tof arguments not rationals");
