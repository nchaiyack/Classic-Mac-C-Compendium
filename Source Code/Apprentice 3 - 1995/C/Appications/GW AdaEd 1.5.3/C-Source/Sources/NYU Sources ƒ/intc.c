		...
		for (;;) {
			...
			if (field >= first_field && field <= last_field) {
			  ...
			  ||(field > last_field && next_case == -1)) {
				...
				exec_raise(CONSTRAINT_ERROR, "Record component not present");
				return;
			}
	  ...
	  || type_type == TT_D_ARRAY) {
		...
		if (type_type == TT_D_ARRAY) {
			...
			if (nb_dim == 1) {
				...
				exec_create((int)WORDS_S_ARRAY, &type_base, &type_off, &new_type_ptr);
				...
				exec_create(template_size, &type_base, &type_off, &new_type_ptr);
				...
				for (i = 1; i <= nb_dim; i++) {
					...
					high = get_variable_bound(a_type_ptr, discr_list);
					a_type_ptr += 2;
					exec_create(WORDS_I_RANGE, &bas2, &off2, &ptr2);
...
void record_move(int *ptr_a, int *ptr_v, int *ptr_t)       /*;record_move*/
{
	...
	case TT_D_RECORD:
		...
		while (i-- > 0) {
			if (*ptr_a++ != *ptr_v++) {
				exec_raise(CONSTRAINT_ERROR, "Discriminant");
				return;
			}
		}
		break;
...
void membership(void)												/*;membership*/
{
	int     some_bool;
...
int qual_index(int *type_ptr1, int *type_ptr2)      /*;qual_index*/
{
	...
	if (TYPE(type_ptr1) == TT_U_ARRAY || TYPE(type_ptr1) == TT_C_ARRAY) {
		if (TYPE(type_ptr2) == TT_U_ARRAY || TYPE(type_ptr2) == TT_C_ARRAY) {
			...
			exec_raise(SYSTEM_ERROR, "qual index on TT_D_ARRAY");
			return FALSE;
#ifdef TBSN
			return qual_index(type_ptr2, type_ptr1);
#endif
...
void qual_discr(int bse, int off)							 /*;qual_discr*/
{
	...
	if (TYPE(ptr) == TT_RECORD)
		exec_raise(SYSTEM_ERROR, "Qual discr on simple record");
		...
		exec_raise(SYSTEM_ERROR, "Qual discr on TT_D_RECORD");
		return;
#ifdef TBSN
		nb_discr = C_RECORD(ptr)->nb_discr_c - 1;
		ptr1 = ADDR(bse, off) + 1;/* skip constrained flag */
		...
		exec_raise(SYSTEM_ERROR, "Unknown record type in qual discr");
}

void allocate_new(void)											/*;allocate_new*/
{
    ...
    if (ACCESS(ptr1)->collection_avail > 0) {
       ACCESS(ptr1)->collection_avail = ACCESS(ptr1)->collection_avail - value;
		...
		exec_raise(STORAGE_ERROR, "collection exhausted");
	    return;
	}
	...
	case TT_U_RECORD:
	case TT_V_RECORD:
		exec_raise(SYSTEM_ERROR, "Allocate unconstrained record");
		break;
...
void exec_create(int size, int *bse, int *off, int **ptr)			/*;exec_create*/
{
	 ...
	 * Procedure exec_create is only used for object creation.
	 */
	...
	if (size < 0 || size >max_mem) {
		exec_raise(SYSTEM_ERROR, "Ridiculous size for object creation");
	...
	if (heap_next > heap_addr + max_mem - size) {
		if(!allocate_new_heap()) {
			exec_raise(STORAGE_ERROR, "Object creation");
			*ptr = heap_addr + WORDS_PTR + 1;
			*off = *ptr - heap_addr;
			*bse = heap_base;
			return;
...
void allocate(int size, int *bse, int *off, int **ptr)			/*;allocate*/
{
	...
	if (size < 0) {
		exec_raise(SYSTEM_ERROR, "Ridiculous size for object allocation");
		*ptr = heap_addr + WORDS_PTR + 1;
		*off = *ptr - heap_addr;
		*bse = heap_base;
		return;
	...
	if (heap_next > heap_addr + max_mem - size) {
		if(!allocate_new_heap()) {
			exec_raise(STORAGE_ERROR, "Allocator");
			*ptr = heap_addr + WORDS_PTR + 1;
			*off = *ptr - heap_addr;
			*bse = heap_base;
			return;
...
void push_task_frame(int first)							/*;push_task_frame*/
{
	if (heap_next > heap_addr + max_mem - 4 - 2*WORDS_PTR)
		exec_raise(STORAGE_ERROR, "Tasking");
	else {
...
int pop_task_frame(void)										/*;pop_task_frame*/
{
...
/* WARNING:
deallocate actually doesn't deallocate anything, so memory is used
_very_ inefficiently.
If you are looking for the cause of the "memory leaks", well, it's here.
I didn't write this code, anyway :-| [Fabrizio Oddone]
*/
		...
		for (i = 0; i < nb_discr; i++) {
			...
			if ( I_RANGE(type_discr)->ilow > new_discr_list [i]
			  || I_RANGE(type_discr)->ihigh < new_discr_list [i]) {
				exec_raise (CONSTRAINT_ERROR, "Discr. does not hold in bounds");
			}
			field_ptr += 3;
			discr_ptr += 2;
		}
