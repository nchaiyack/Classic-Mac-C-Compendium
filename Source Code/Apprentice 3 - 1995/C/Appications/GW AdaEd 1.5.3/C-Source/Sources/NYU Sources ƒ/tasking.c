...
static void done_creation(void);
static void create_task(int, int, struct rts_item *, int);
static void done_activation(void);
static void activate_self(int, int);
static void kill(int);
static void abortme(void);
static void post_abort_one(void);
static void post_complete_task_one(void);
static void post_complete_block(void);
static void post_complete_block_two(void);
static void task_term(void);
static void free_task_space(void);
static void check_termination(int, int);
static void check_done(int, int);
static void check_unterm(void);
...
static void post_entry_call(void);
...
static int remove_serviced(void);
...
void initialize_tasking(void)							/*;initialize_tasking*/
{
	int     i;
	int    *p;
//	long   itime();
	...
	p = STACK(0) = (int *) mmalloc((unsigned) sizeof(int) * main_task_size);
	ORIG(0) = 0;
	if (p == (int *)0) {
		scrPutStr("Unable to allocate stack\n");
		exitp(RC_ABORT);
	}
...
void start_creation(int templ_base, int templ_off)			/*;start_creation*/
{
	...
	rts = (struct rts_item *) mmalloc(sizeof(struct rts_item));
	if (rts == (struct rts_item *)0) {
		exec_raise(STORAGE_ERROR, "Allocating space for task");
		return;
	}
	rts->tcbs = (int *) mmalloc((unsigned) sizeof(int)*mult);
	if (rts->tcbs == (int *)0) {
		exec_raise(STORAGE_ERROR, "Allocating space for task");
		return;
	}
...
static void done_creation(void)									/*;done_creation*/
{
	if (MY_EXCEPTION == STORAGE_ERROR) {
		exec_raise(STORAGE_ERROR, "Not enough space for new tasks");
		MY_EXCEPTION = 0;
	}
  ...
  int my_id)													/*;create_task*/
{
	...
	p=STACK(task_nr) = (int *) mmalloc((unsigned) sizeof(int)*new_task_size);
...
#ifdef TRACE
		if (tasking_trace) {
			sprintf(scrBuffer, "task %d creating task %d\n",tp,task_nr);
			scrPutStr(scrBuffer);
		}
#endif
...
static void done_activation(void) 							/*;done_activation*/
{
	if (MY_ABNORMAL) {
		abortme();
		return;
	...
	if (MY_EVENT == TASKERR_EVENT) {
		exec_raise(TASKING_ERROR, "Tasking error in activation");
		MY_EXCEPTION = 0;
	}
	else if (MY_EVENT == PROGERR_EVENT) {
		exec_raise(PROGRAM_ERROR, "Activating an unelaborated task");
		MY_EXCEPTION = 0;
	}
	MY_ACTION = NO_ACTION;
}
...
#ifdef TRACE
	if (tasking_trace) {
		sprintf(scrBuffer, "task %d activating task %d\n",task_master,tp);
		scrPutStr(scrBuffer);
	}
#endif
}
...
static void kill(int task)											/*;kill*/
{
...
#ifdef TRACE
		if (tasking_trace) {
			sprintf(scrBuffer, "task %d aborting terminated task %d\n",tp,task);
			scrPutStr(scrBuffer);
		}
#endif
	...
	if (tasking_trace && TCB_ABNORMAL(task)) {
		sprintf(scrBuffer, "task %d aborting task %d\n",tp,task);
		scrPutStr(scrBuffer);
	}
#endif
	...
	case WAIT:
		...
		break;
	default: 
		exec_raise(SYSTEM_ERROR, "Aborting task in unknown state");
		break;
...
static void abortme(void)							/*;abortme */
{
	purge_rdv(tp);
...
static void post_abort_one(void)					/*;post_abort_one*/
{
...
/*  used to perform task termination. They are divided into two sets: those */
...
void complete_task(void)								/*;complete_task*/
{
	MY_STATUS = COMPLETED;
...
static void post_complete_task_one(void)				/*;post_complete_task_one*/
{
	check_termination(MY_MASTER_TASK, MY_MASTER_BLOCK);
...
void complete_block(void)										/*;complete_block*/
{
...
static void post_complete_block(void)						/*;post_complete_block*/
{
	if (MY_ABNORMAL == 0)
...
static void post_complete_block_two(void)			/*;post_complete_block_two*/
{
	MY_STATUS = ACTIVE;
}
...
static void task_term(void)											/*;task_term*/
{
	int block;
...
static void free_task_space(void)							/*;free_task_space */
{
	int	block;
...
static void check_unterm(void)									/*;check_unterm*/
{
...
void terminate_unactivated(void) 					/*;terminate_unactivated */
{
...
/* Procedure to exec_raise TASKING_ERROR in all tasks waiting for or engaged   */
...
#ifdef TRACE
	if (rendezvous_trace) {
		sprintf(scrBuffer, "task %d delaying %ld effective delay %ld \n", tp,
...
static void post_wait(void)											/*;post_wait*/
{
	MY_STATUS = ACTIVE;
	if (MY_ABNORMAL) 
		abortme(); 
...
//	long	my_reset_time(), itime();
	...
	if (TCB_IO_ITEM(task) == NULL) {
		new_item=(struct io_item_type *)mmalloc(sizeof(struct io_item_type));
		if (new_item == (struct io_item_type *)0) {
			exec_raise(STORAGE_ERROR, "Allocating space for timer chain");
			return(NULL);
		}
	}
...
static int get_io(struct io_item_type *item)						/*;get_io*/
{
	...
	case 0    :           
		...
		break;       /* innner remove*/
	default :             
		exec_raise(SYSTEM_ERROR, "Unexpected event");
		break;
	}
	II_FLAG(item) = 0;
	return wake;
 ...
 *  that the interpreter return to the highest level without executing any  *
...
static void schedule(int action)								/*;schedule*/
{
			...
			if (item != (struct rts_item *)0) {
				...
				case READY :
					...
					/* i.e. task x switching to itself. */
						sprintf(scrBuffer, "task %d switching to task %d \n",
							tp,item->tcbs[i]);
						scrPutStr(scrBuffer);
					}
#endif
				...
				case CREATE :
					...
					break;
				default :
					exec_raise(SYSTEM_ERROR, "Unexpected type");
					break;
				}
	...
	{ /* No active task was found, error condition */
		MY_STATUS = ACTIVE;
		exec_raise(SYSTEM_ERROR, "No activatable task");
		MY_EXCEPTION = 0;
		return;
	}
	finish_action(item);
...
//	long	itime();
	...
	case NO_ACTION   	: 				
		break;
	default : 
		exec_raise(SYSTEM_ERROR, "Tasks awakened in an unknown state");
		MY_EXCEPTION = 0; 				
		break;
	}
	...
	if (tp == 0) {		/* we schedule IDLE */
		if (next_clock_flag) {		   /* exists task waiting? */
			...
			exec_raise(PROGRAM_ERROR, "System inactive(deadlock)");
	}
	MY_ACTION = NO_ACTION;
}
 ...
 * The routines which are called by the interpreter directly are:           *
 * 	- selective_wait                                                    *
 * 	- end_rendezvous                                                    *
 * 	- entry_call                                                        *
...
void selective_wait(int num_alts)							/*;selective_wait */
{
	...
	if ((num_open_alts == 0) && (delay_index == -1) && (term_index == -1)){
		exec_raise(PROGRAM_ERROR, "No open alternatives in select");
		return;
	}
...
#ifdef TRACE
		if (rendezvous_trace) {
			sprintf(scrBuffer, "task %d waiting for a rendezvous \n", tp);
			scrPutStr(scrBuffer);
		}
#endif
  ...
  int open_ai[])									/*;post_selective_wait*/
{
	...
	if ((num_alts != 0) && (accept_index == -1)) {
		exec_raise(SYSTEM_ERROR, "Nonexistant alternative selected");
		return;
	}
...
void end_rendezvous(void)										/*;end_rendevous*/
{
	int	caller;
...
#ifdef TRACE
	if (rendezvous_trace) {
		sprintf(scrBuffer, "task %d end rendezvous with task %d \n",tp, caller);
		scrPutStr(scrBuffer);
	}
#endif
	...
	if (MY_ABNORMAL) {              /* owner aborted, exec_raise TASKING ERROR */
...
void entry_call(long delay_time, int num_param)					/*;entry_call*/
{
	...
	if (is_callable(owner) == FALSE) {
		...
		case TERMINATED:
			exec_raise(TASKING_ERROR, "Call to an entry of a terminated task");
			return;
		case COMPLETED:
		default:
			exec_raise(TASKING_ERROR, "Call to an entry of a completed task");
			return;
		}
	}
...
#ifdef TRACE
	if (rendezvous_trace) {
		sprintf(scrBuffer, "task %d calling rendezvous with task %d entry %d\n",
		  tp, owner, entry_num);
		scrPutStr(scrBuffer);
	}
#endif
	...
	if (entry_num > TCB_NUM_ENTRIES(owner)) {
		exec_raise(SYSTEM_ERROR, "Nonexistant entry called");
		return;
	}
...
static void post_entry_call(void)							/*;post_entry_call*/
{
	...
	case ENDRDV_EVENT: 
		disable_io(&MY_IO_ITEM);
		if (MY_EXCEPTION)
			exec_raise(MY_EXCEPTION, "Exception propagated from called task");
		MY_EXCEPTION = 0;
		...
		break;
	case TASKERR_EVENT :
		exec_raise(TASKING_ERROR, "Entry call failed: called task terminated");
		MY_EXCEPTION = 0; 			
		break;
	default	: 
		exec_raise(SYSTEM_ERROR, "Unexpected event in entry call"); 	
		break;
	}
	MY_STATUS = ACTIVE;
}
																	...
																	/*;eput*/
{
	...
	item = (struct q_item*) mmalloc(sizeof(struct q_item));
	if (item == (struct q_item*)0) {
		exec_raise(STORAGE_ERROR, "Allocating space for entry queue");
		return;
	}
	ITEM_FLAG(item) = 1;
...
/*------------------------------------------------------------------------*
 *                          RAISE  IN  CALLER                             *
 * Procedure to exec_raise an exception in the task waiting for the completion *
 * of the current rendezvous						  *
 * -----------------------------------------------------------------------*/

void raise_in_caller(void)									/*;raise_in_caller*/
{
#ifdef GWUMON
...
#ifdef TRACE
	if (exception_trace) {
		sprintf(scrBuffer, "task %d raising exception %s in task %d\n",tp,
		  exception_slots[exr],MY_SERVICED);
		scrPutStr(scrBuffer);
	}
#endif
...
static void qinit(void)													/*;qinit*/
{
	int	i;
	struct ready_q	*q;

	for (i = 0; i < MAX_PRIO; i++) {
		ready_queue[i] = (struct ready_q *)mmalloc(sizeof(struct ready_q));
		q = ready_queue[i];
...
static void enqueue(int priority, int value)					/*;enqueue*/
{
	...
	item = (struct rts_item *) mmalloc(sizeof(struct rts_item));
	if (item == (struct rts_item *)0){
		exec_raise(STORAGE_ERROR, "Allocating space for ready queue");
		return;
	}
	item->tcbs = (int *) mmalloc(sizeof(int));
	if (item->tcbs == (int *)0){
		exec_raise(STORAGE_ERROR, "Allocating space for ready queue");
		return;
	}
...
static void context_switch(void)						/*;context_switch*/
{
	DEC(MY_NUM_EVENTS);
