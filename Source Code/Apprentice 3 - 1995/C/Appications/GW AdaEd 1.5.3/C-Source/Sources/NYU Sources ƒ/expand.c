...
void expand(Node node)												/*;expand*/
{
	...
	Fortup      ft1, ft2;
	Tuple       tup, tup1, tup2;
	Symbolmap   instance_map = nil, type_map;	// explicit initialization added to avoid warning [Fabrizio Oddone]
	...
	case(as_declarations):
		...
		if (N_LIST(node) == (Tuple)0)
			chaos(__FILE__, __LINE__, "expand.c: as_declarations N_LIST null");
		FORTUP(node1 = (Node), N_LIST(node), ft1);
			expand(node1);
			N_SIDE(node) |= N_SIDE(node1);
		ENDFORTUP(ft1);
	...
	case(as_int_literal):
		...
		if (adaval_overflow)
			chaos(__FILE__, __LINE__, "unable to convert integer literal");
		else
			N_VAL(node) = (char *) lv;
		N_SIDE(node) = FALSE;
		break;
		...
		if (node1 == OPT_NODE) {
			...
			FORTUP(stub_nam = (char *), tup, ft1);
				if (streq(unit_name_name(stub_nam), ORIG_NAME(sym2))) {
					if (!
#ifdef MAC_GWADA
		fab_read_ais(AISFILENAME, stub_nam, 0, TRUE, TRUE)
#else
		read_ais(AISFILENAME, TRUE, stub_nam, 0, TRUE)
#endif
					) break;
					tup  = SIGNATURE(sym2);
					node1 = (Node) tup[4];     /* body node */
