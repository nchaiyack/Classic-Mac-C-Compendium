 ...
 */
	...
	Boolean	simple_recursive_proc, has_separate_spec;
	...
	if ((tup_size(temp_relay_set) == 0 || simple_recursive_proc)
	  && ! has_separate_spec) {
		...
		segment_put_word(tseg, simple_recursive_proc ? 1 : 0 );
...
//		if (tup_size(temp_relay_set))
//			DebugStr("\pRelay set at level 1");
		if (tup_size(temp_relay_set) || simple_recursive_proc) {
#ifdef DEBUG
			FORTUP(name = (Symbol), temp_relay_set, ft1);
				zpsym(name);
			ENDFORTUP(ft1);
#endif
			chaos(__FILE__, __LINE__, "Relay set at level 1");
#ifdef TRACE
			if (debug_flag)
