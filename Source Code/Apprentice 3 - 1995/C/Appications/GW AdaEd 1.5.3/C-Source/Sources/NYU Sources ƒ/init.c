...
static Tuple precedes_map_new(void);
static void init_predef_exceptions(void);
...

{
	...
	if (compiling_predef) {
		...
		else chaos(__FILE__, __LINE__, "init_slots bad kind");
...

{
	...
	chaos(__FILE__, __LINE__, "max_slots bad kind");
...
#define NAMESIZE 119
...

	{
		...
		s_file = mmalloc(length * sizeof(char) );
