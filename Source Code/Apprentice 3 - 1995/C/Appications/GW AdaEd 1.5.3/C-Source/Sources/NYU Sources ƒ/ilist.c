...
void i_list1(int *ip, char *curcode)							/*;i_list1*/
{
	 ...
	 */
	if (debug_trace) {
		sprintf(scrBuffer, "tp=%1d,sp=%3d,sfp=%3d,bfp=%3d,exr=%2d,lin=%2d,tos=%3d,",
		  tp    ,sp    ,sfp    ,bfp    ,exr    ,lin    ,cur_stackptr);
		scrPutStr(scrBuffer);
		sprintf(scrBuffer, "cs=%2d,ip=%3d\n",cs,*ip);
		scrPutStr(scrBuffer);
	}
	else {
		sprintf(scrBuffer, "tos=%3d,cs=%2d,ip=%3d  ",cur_stackptr,cs,*ip);
		scrPutStr(scrBuffer);
	}
#ifdef DEBUG_FULL_STACK
	stdump(sfp);
	...
	*ip += 1;
	sprintf(scrBuffer, "%s ", op_name(opcode)); /* debug */
	scrPutStr(scrBuffer);
	switch(opcode) {
