	...
	if (sym == symbol_addui) {
		...
		if (is_simple_value(op1)) {
			if (sym == symbol_subui) {
				...
				else chaos(__FILE__, __LINE__, "eval:subui bad type");
			}
		...
		if (is_simple_value (op1)) {
			...
			else chaos(__FILE__, __LINE__, "fold_unop: bad kind");
		}
		else {		/*TBSL*/
			result = const_new(CONST_OM);
		}
		...
		if (is_simple_value(op1)) {
			if (sym == symbol_absi) {
				...
				else if (op1_kind == CONST_UINT)chaos(__FILE__, __LINE__, "fold_unit absi in uint");
				else chaos(__FILE__, __LINE__, "fold_unop: bad kind");
			}
	...
	if ( sym == symbol_addi || sym == symbol_addfl) {
		...
		if (is_const_int(op1)) {
			if (is_const_int(op2)) {
				...
				chaos(__FILE__, __LINE__, "fold_op: subi operand types");
			}
		}
	}
...
*/
		...
		if (INTV(op2) < 0) {
			...
			if (is_const_int(op1))
				...
				chaos(__FILE__, __LINE__, "expi: bad kind");
			const_check(op2, CONST_INT);
	  ...
	  || (sym == symbol_cat_ac) || (sym == symbol_cat_cc)) {
		...
		if (is_simple_value(op1)) {
			if (N_UNQ(opn) == symbol_and) {
				if (is_const_int(op1))
					...
					chaos(__FILE__, __LINE__, "fold_unop: bad kind");
			}
			else if (N_UNQ(opn) == symbol_or) {
				if (is_const_int(op1))
					result = int_const(INTV(op1)||INTV(op2));
				else
					chaos(__FILE__, __LINE__, "fold_unop: or bad kind");
			}
			else if (N_UNQ(opn) == symbol_xor) {
				result = test_expr((INTV(op1))!= (INTV(op2)));
			}
			else {
				chaos(__FILE__, __LINE__, "ERROR IN ES99");
			}
		}
	}
		...
		if (is_universal_real(op1) && is_universal_real(op2)) {
			...
			if (is_const_int(op1))
				...
				chaos(__FILE__, __LINE__, "error in ne case in const_fold");
		}
#endif
		if (const_same_kind(op1, op2))
		...
		if (is_simple_value(op1)) {
			...
			if (is_const_int(op1)) {
				...
				if (is_const_real(op1)) {
					...
					chaos(__FILE__, __LINE__, "fold_unop: lt bad kind ");
				}
			}
#endif
			if (const_same_kind(op1, op2))
		...
		if (is_simple_value(op1)) {
			...
			if (is_const_int(op1)) {
				...
				chaos(__FILE__, __LINE__, "fold_op: le bad kind");
			}
#endif
			if (const_same_kind(op1, op2))
		...
		if (is_simple_value(op1)) {
			...
			if (is_const_int(op1)) {
				...
				chaos(__FILE__, __LINE__, "fold_op: gt bad kind");
			}
#endif
			if (const_same_kind(op1, op2))
		...
		if (is_simple_value(op1)) {
			...
			if (is_const_int(op1))
				...
				chaos(__FILE__, __LINE__, "fold op ge bad kind");
#endif
			if (const_same_kind(op1, op2))
		...
		chaos(__FILE__, __LINE__, "fold_op: bad operator");
	}
	return result;
}
...
static Const fold_attr(Node node)		/*;fold_attr*/
{
	...
	/* Attributes of DISCRETE types: */
		...
		chaos(__FILE__, __LINE__, "value attrobute (eval.c)");
	}
	  ...
	  || attrkind == ATTR_SUCC) {
		...
		if (root_type(typ1) == symbol_integer) {
			if (is_const_om(first) || is_const_om(last))
				chaos(__FILE__, __LINE__, "eval WIDTH: unexpected const_kind");
		...
		if (typ2 == symbol_integer) {
		  ...
		  || typ2 == symbol_universal_fixed || typ2 == symbol_dfixed) {
			if (is_const_int(opd)) {
				...
				chaos(__FILE__, __LINE__, "const wrong type (eval.c)");
		}
		else
			result = const_new(CONST_OM);
	}
