...
static char *FREE_TUPLE = NULL;/* list of free (singleton) tuples */

char *set_arb(Set sp)											/*;set_arb*/
{
	/* pick arbitrary element from set */

	if ((int) sp[0] == 0) chaos(__FILE__, __LINE__, "set_arb - set already empty");
	return sp[1];
}
...
char   *set_from(Set sp)										/*;set_from*/
{
	...
	if (sp[0] == 0) chaos(__FILE__, __LINE__, "set_from null_set");
	n = (int) sp[0];
	sp[0] = (char *) (n-1);
	return sp[n];
}
...
Tuple tup_add(Tuple tpa, Tuple tpb)				/*;tup_add*/
{
	...
	if (tpa == (Tuple)0 ) chaos(__FILE__, __LINE__, " tup_add: first tuple null");
	if (tpb == (Tuple)0 ) chaos(__FILE__, __LINE__, " tup_add: second tuple null");
...
Tuple tup_exp(Tuple tp, unsigned int n)							/*;tup_exp*/
{
...
#ifdef CHAOS
	if (n == 0 || n>99999) chaos(__FILE__, __LINE__, "tup_exp: new length > 99999");
#endif
#ifdef SKIP
	if ((int)tp[0]>n) {
		/* adjust length */
	...
	/* if expanding smalloc singleton */
		...
		oldtp = tp;
		tp = (Tuple) fcalloct(sizeof(char *) * (unsigned) n+1,
		  "tup-new-smalloc-exp");
		tp[0] = (char *)n;
		tp[1] = oldtp[1];
...
char   *tup_frome(Tuple tp)									/*;tup_frome*/
{
	...
	if ((int) tp[0] == 0) chaos(__FILE__, __LINE__, "tup_frome: tuple already empty");
	tp[0] -= 1;
...
int   tup_mem(char *n, Tuple tp)								/*;tup_mem*/
{
	...
	if (tp == (Tuple)0) chaos(__FILE__, __LINE__, "tup_mem: tuple not defined");

	sz = tup_size(tp);
	if (sz<0 || sz >9999) chaos(__FILE__, __LINE__, "tup_mem: ridiculous tuple element count");
...
if (n == 1) {
	if (FREE_TUPLE) { /* if can take from free list */
    	tp = (Tuple) FREE_TUPLE;
    	FREE_TUPLE = (char *) tp[0];
		}
	else
    	tp= (Tuple) smalloc(2*sizeof(char *));
	tp[1] = NULL; /* clear value */
	}
else
	tp = (Tuple) fcalloct( (n + 1)* sizeof(char **), "tup-new");
#endif
tp[0] = (char *)n;
return tp;
}
...
int	tup_size(Tuple tp)											/*;tup_size*/
{
	...
	if (tp == (Tuple)0) chaos(__FILE__, __LINE__, "tup_size argument null pointer");

	n = (int)tp[0];
	if (n < 0 || n > 99999) chaos(__FILE__, __LINE__, "tup_size: size out of range");
#endif
	return (int) tp[0];
}
#endif
