...
void expand_line(void)											/*;expand_line*/
{
															...
															/*;replace_name*/
{
	...
	if (node == (Node)0)
		chaos(__FILE__, __LINE__, "replace_name called on null node");
...
// the following fix introduces problems; we keep it out for now
#define	DONT_DEFINE_THIS

#ifdef	DONT_DEFINE_THIS
	/*
	* This code was never going to work, because the size and
	* kind fields of type_name will not have been set by the time
	* the as_obj_decl comes to be elaborated. Better to ignore
	* deferred constants entirely until their full declarations
	* happen, since nobody is supposed to use them before then.
	* This does, however, create another (hopefully more
	* tolerable) bug if the deferred constant is used to
	* initialize a field of a record type.
	*/
			...
			if (N_KIND(node) == as_obj_decl) {
				...
				if (N_KIND(init_node) == as_insert ) {
				  ...
				  || N_KIND(init_node) == as_record_aggregate) {
					...
					chaos(__FILE__, __LINE__, "not so impossible expand2 problem");
	...
	if (nat == na_type) {
		...
		if (is_fixed_type(type_name)) {
			...
			if (small_node == OPT_NODE) {
				 ...
				 */
				chaos(__FILE__, __LINE__, "fixed with small OPT_NODE");
			}
