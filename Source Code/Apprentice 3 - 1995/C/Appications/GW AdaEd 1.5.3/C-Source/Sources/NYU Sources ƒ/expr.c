			...
			if (is_fixed_type(type_name)) {
				...
				if (is_float_type(type_name)) {
					...
					if (is_const_rat(value)) { /* try to cnvrt rtnl to real*/
						chaos(__FILE__, __LINE__, "expr.c: rational seen when real expected");
					}
...
void gen_attribute(Node node)								/*;gen_attribute*/
{
	...
	case(ATTR_AFT):	 /* Computed by the expander? TBSL */
		...
		if (rat_const_v->const_kind != CONST_RAT)
			chaos(__FILE__, __LINE__, "expr: argument not rational");
		 ...
		 */
		if (arith_overflow)
			chaos(__FILE__, __LINE__, "expr.c ATTR_AFT overflow during conversion");
	...
	case(ATTR_SAFE_LARGE):
		/* chaos(__FILE__, __LINE__, "expr.c - untranslated code reached"); */
		gen_kv(I_PUSH_IMMEDIATE, kind_of(symbol_float),
		      ...
		      || N_KIND(val_node) == as_real_literal) {
	    		...
	    		if (is_const_uint(exprv)) {
     				...
     				if (arith_overflow) {/* if cannot convert to integer */
						    chaos(__FILE__, __LINE__, "cannot convert uint to int in array_ivalue");
	     			}
					exprv = int_const(i);
	   			}
...
Segment record_ivalue(Node node)						/*;record_ivalue*/
{
	...
	Node	static_node, selector_node, val_node = nil;	// explicit initialization added to avoid warning [Fabrizio Oddone]
