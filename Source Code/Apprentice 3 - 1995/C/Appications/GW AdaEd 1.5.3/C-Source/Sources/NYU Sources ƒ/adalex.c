...
unsigned int src_index = UINT_MAX;		/* Index into source lines buffer */
...

{
	...
	if (avl_w -> txt_pos >= strlen(avl_w -> current_line -> line))  {

		avl_w -> current_line = avl_w -> current_line -> next;

		avl_w -> txt_pos = 0;

		if (avl_w -> current_line == avl_w -> head)

			return(EOF);

		

		old = _settextposition(1, 17);

		sprintf(txt,"%4d", avl_w -> current_line -> line_no - 1);  
	...
	if (avl_w -> current_line -> line [ avl_w -> txt_pos] == 1)

		return EOF;

	return avl_w -> current_line -> line [avl_w -> txt_pos++];
...
static int getline(void)											/*;getline*/
{
int ch, ind = 0;
int	retint = 0;
/*	if (avl_w -> current_line == avl_w -> head) */
...
if
	...
	for (;;) {
	#ifdef MAC_GWADA
		ch = fab_getc(gSrcFileP);
	#else
		ch = /*  AVL_GETC(); */  getc(gAdafile); 
	#endif
		if (ch == EOF) break;
	...
	#ifdef MAC_GWADA
			while ((ch = fab_getc(gSrcFileP)) != EOF && !(ch<=13 && ch>=10))
	#else
			while ((ch = getc(gAdafile)) != EOF && !(ch<=13 && ch>=10))
	#endif
				;
			break;
		}
...
struct prsstack *gettok(void)										/*;gettok*/
{
...
static int newnode(char *str, int len, int nmhash)				/*;newnode*/
{
	...
	node = (struct namelistmap *)mmalloc(sizeof(struct namelistmap));
// map-debug
//	if (!node)
//		DebugStr("\p");
	node->num = ++symcount;
	node->name = mmalloc((unsigned)(len + 1));
// map-debug
//	if (!node->name)
//		DebugStr("\p");
