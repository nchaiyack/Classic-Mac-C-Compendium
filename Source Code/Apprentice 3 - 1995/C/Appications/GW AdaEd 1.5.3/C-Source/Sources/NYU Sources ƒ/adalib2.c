...
/*      _outtext(" NYU Ada/Ed Librarian Version 1.11.1\n");

        _outtext(" Copyright (C) 1985-1992 by New York University.");
...

{
        ...
        char    *main_string, *fmt_name, *fmt_date;
        ...
        for (i = 1; i <= unit_count; i++) {
                ...
                fmt_date = convert_date(comp_date);

                fmt_name = formatted_name(uname);



                fprintf(fout,"%8s %11s %-15s %s\n",

                    status_str, main_string, fmt_date, fmt_name);
