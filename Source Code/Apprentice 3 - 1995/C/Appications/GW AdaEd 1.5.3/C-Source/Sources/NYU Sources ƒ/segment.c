...
void segment_append(Segment s, Segment sa)					/*;segment_append*/
{
	...
	if (s->seg_kind != SEGMENT_KIND_DATA)
		chaos(__FILE__, __LINE__, "segment_append not appending a data segment");
...
void seg_check(Segment seg)										/*;seg_check*/
{
	if (seg->seg_id != SEG_ID)
		chaos(__FILE__, __LINE__, "invalid segment - check word invalid ");
}
...
int segment_get_int(Segment s, int i)					/*;segment_get_int*/
{
	...
	seg_check(s);
	if (i >= s->seg_maxpos) {
		chaos(__FILE__, __LINE__, "segment.c: get_int retrieving from undefined location\n");
		return 0;
	}
	if (s->seg_kind == SEGMENT_KIND_DATA) {
...
void segment_put_byte(Segment s, int v)					/*;segment_put_byte*/
{
	...
	if (s->seg_kind != SEGMENT_KIND_CODE)
		chaos(__FILE__, __LINE__, "segment.c: segment_put_byte called on data segment");
...
void segment_put_const(Segment seg, Const con)			/*;segment_put_const*/
{
	if (con->const_kind == CONST_INT) {
		...
		zpcon(con);
#endif
		chaos(__FILE__, __LINE__, "segment.c - meaningless kind of literal");
	}
}
...
void segment_put_off(Segment s, int i, int v)				/*;segment_put_off*/
{
	...
	if (i >= s->seg_maxpos)
		chaos(__FILE__, __LINE__, "segment.c: segment_put_off of undefined location");
														...
														/*;segment_set_pos*/
{
	...
	if (offtyp != 0)
		chaos(__FILE__, __LINE__, "segment_set_pos bad offset type");
	s->seg_pos = pos;
}
