  ...
  /*;update_one_entry*/
{
	...
	Tuple	tup, gen_list, form_list, new_gen_list, new_form_list = nil, otup, ntup;	// explicit initialization added to avoid warning [Fabrizio Oddone]
...
*/
for (scanP = tup + 1, endP = tup + n; scanP <= endP; scanP++) {
	if (*scanP++ == (char *)symd) {
		*scanP = (char *)symv;
		return;
		}
