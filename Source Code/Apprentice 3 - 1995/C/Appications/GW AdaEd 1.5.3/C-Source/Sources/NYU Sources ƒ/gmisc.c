															...
															/*;select_entry*/
{
	...
	default:
		...
		printf("a_map_code: %d\n", a_map_code);
#endif
		chaos(__FILE__, __LINE__, "select entry bad a_map_code");
	}
...
int get_ivalue_int(Node node)								/*;get_ivalue_int*/
{
	...
	int n;
	if (! is_ivalue(node)  )
		chaos(__FILE__, __LINE__, "get_ivalue_int: arg not ivalue");
	v = (Const) N_VAL(node);
	n = get_const_int(v);
	return n;
}
...
int get_const_int(Const v)							/*;get_const_int*/
{
	...
	if (v->const_kind == CONST_INT)
		...
		if (!arith_overflow)
			 return n;
		chaos(__FILE__, __LINE__, "get_ivalue_int: cannot convert uint");
	}
	else
		chaos(__FILE__, __LINE__, "get_ivalue: const not int");
	return n;
}
...
void compiler_error(char *reason)							/*;compiler_error*/
{
	...
	if (debug_flag)
		printf("--> %s\n", reason);
	chaos(__FILE__, __LINE__, "compiler errror");
}
#endif
...
void labelmap_put(Symbol sym, int comp, char *val)			/*;labelmap_put*/
{
	...
	if (comp<1 || comp>LABEL_SIZE)
		chaos(__FILE__, __LINE__, "labelmap_put label code out of range");
	tup[comp] = val;
}
...
Tuple labelmap_get(Symbol sym)								/*;labelmap_put*/
{
	...
	if (tup == (Tuple)0) {
		...
		zpsym(sym);
#endif
		chaos(__FILE__, __LINE__, "labelmap_get label map is null tuple ");
	}
	return tup;
}
...
Tuple unit_slots_get(int unum)							/*;unit_slots_get*/
{
	...
	if (unum > n)
		chaos(__FILE__, __LINE__, "unit_slots_get unit number out of range");
