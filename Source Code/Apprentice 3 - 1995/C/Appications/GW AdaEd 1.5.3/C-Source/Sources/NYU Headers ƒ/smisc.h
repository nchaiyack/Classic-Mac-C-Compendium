...
Node node_new_noseq(unsigned short);
Node node_new(unsigned short);
int fx_mantissa(Rational, Rational, Rational);
void to_errfile(char *);
int needs_body(Symbol);
char *kind_str(unsigned short);
char *nature_str(short);
...
Symbol sym_new_noseq(short);
Symbol sym_new(short);
...
char *attribute_str(short);
int no_dimensions(Symbol);
int in_incp_types(Symbol);
int in_qualifiers(unsigned short);
