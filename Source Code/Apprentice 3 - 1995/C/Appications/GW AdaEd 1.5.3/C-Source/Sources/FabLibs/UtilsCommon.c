/*
Copyright й 1993,1994 by Fabrizio Oddone
...
static long RegisterStdConfigInfo(Handle regText, long curOffset, StringPtr tempS);
...
InitCursor();
dPtr = GetNewDialog(resId, nil, (WindowRef)-1L);
if (dPtr) {
	GetPort(&port);
...
if (splash) {
	ShowWindow(GetDialogWindow(agh));
	do {
		SystemTask();
			...
			switch(evrec.what) {
				...
				case updateEvt :
					...
					if ((DialogPtr)evrec.message == agh) {
						BeginUpdate(GetDialogWindow(agh));
						UpdateDialog(agh, agh->visRgn);
						EndUpdate(GetDialogWindow(agh));
						}
					else
...
if (hand) {
	...
	*pass = dPtr = GetNewDialog(kDLOG_About, *hand, (WindowRef)-1L);
	if (dPtr) {
		if (DrawQTPict) {
...
Handle	hKCHR;
UInt32	state;
long	/*virtualKey, */keyInfo, keyCID;
UInt16	keyCode;
...
Rect	tempRect;
Handle	ih;
//short	theItem = GetDialogKeyboardFocusItem(theD);
short	null;
short	iHit;
...
iHit = GetDialogKeyboardFocusItem(theD);
if (iHit == kItemQuantity) {
	if (keypressed >= 32 && ((thEv->modifiers & cmdKey) == 0)) {
		...
		if (result) {
			GetDialogItem(theD, iHit, &null, &ih, &tempRect);
			GetDialogItemText(ih, tempStr);
			if (StrLength(tempStr) >= kMaxCopiesChars) {
				(void)StopAlert_UPP(kALRT_WOWTHATMANYCOPIES, nil);
				result = false;
				}
			}
		}
...
if (lOffset) {
	...
	if (err == noErr) {
		...
		if (regText) {
			...
			lOffset = RegisterStdConfigInfo(regText, lOffset, tempS);
...
if (regText) {
	...
	if (noErr == ZeroScrap()) {
		HLockHi(regText);
		(void) PutScrap(InlineGetHandleSize(regText), 'TEXT', *regText);
		HUnlock(regText);
		}
	DisposeHandle(regText);
	}
...
long RegisterStdConfigInfo(Handle regText, long curOffset, StringPtr tempS)
{
StringPtr	myAppName = nil;
long	lOffset = curOffset;
long	gResp;
const short	dash = 0x012D;
const char	optspace = '╞';
...
/* Code from Appleи converted by Stefan Kurth in super-optimized (!) C */
