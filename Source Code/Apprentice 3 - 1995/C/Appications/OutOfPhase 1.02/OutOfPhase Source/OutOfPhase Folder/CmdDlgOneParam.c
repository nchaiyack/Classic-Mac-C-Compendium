...
#define BOXEDITX (BOXPROMPTX + 140)
#define BOXEDITY (BOXPROMPTY - 3)
#define BOXEDITWIDTH (140)
#define BOXEDITHEIGHT (21)
...
MyBoolean				CommandDialogOneParam(char* Prompt, char* BoxName, double* DataInOut)
	{
		...
		Window = (WindowRec*)AllocPtrCanFail(sizeof(WindowRec),
			"CommandDialogOneParam:  WindowRec");
		if (Window == NIL)
			{
			 FailurePoint1:
		...
		StringTemp = LongDoubleToString(*DataInOut,12,1e-6,1e6);
		if (StringTemp == NIL)
			{
			 FailurePoint6:
		...
		TextEditNewRawData(Window->EditBox,StringTemp,SYSTEMLINEFEED);
							...
							if (MenuItem == mPaste)
							...
							else
								{
									EXECUTE(PRERR(AllowResume,
										"CommandDialogOneParam: Undefined menu option chosen"));
								}
							break;
						case eKeyPressed:
							if (KeyPress == 13)
		...
		ERROR((DoItFlag != True) && (DoItFlag != False),PRERR(ForceAbort,
			"CommandDialogOneParam:  DoItFlag is neither true nor false"));
				...
				if (TextEditDoesItNeedToBeSaved(Window->EditBox))
					{
						...
						if (StringTemp == NIL)
							...
							{
								*DataInOut = StringToLongDouble(StringTemp,PtrSize(StringTemp));
								ReleasePtr(StringTemp);
								ReturnValue = True;
							}
					}
