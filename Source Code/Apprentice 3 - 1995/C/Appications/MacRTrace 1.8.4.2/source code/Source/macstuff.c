...
#else
	/* Save the applicationsÕs A5 where the VBL tasks can find it; right 
	 	before the VBL Task structure.  We need the VBL Tasks to have access
	 	to the application globals, so they need to know what the applicationÕs
...
VBL_task_A5_struct		*tempVBL;
	...
	/* The base address of this tasks VBL structure is now in A0.  We can use
		this to get the applicationÕs A5 global pointer.  We need this to be
...
/*	asm {
		...
		move.l -8(A0), A5			; get applicationsÕs A5
		}
*/
	get_another_event = TRUE;	/* tell rtrace to get another event */
