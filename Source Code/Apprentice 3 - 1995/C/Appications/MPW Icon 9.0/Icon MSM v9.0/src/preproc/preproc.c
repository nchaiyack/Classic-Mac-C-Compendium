 ...
 */
hidden novalue start_select Params((struct token *t));
hidden novalue end_select   Params((struct token *t));
hidden novalue incl_file    Params((struct token *t));
hidden novalue define       Params((struct token *t));
hidden int     expand       Params((struct token *t, struct macro *m));
hidden novalue toks_to_str  Params((struct str_buf *sbuf, struct token *t));

/*
 * start_select - handle #if, #ifdef, #ifndef
 */
static novalue start_select(t)
struct token *t;
   {
   struct token *t1;
   struct tok_lst *tlst;
 ...
 */
static novalue toks_to_str(sbuf, t)
struct str_buf *sbuf;
struct token *t;
   {
   char *s;
