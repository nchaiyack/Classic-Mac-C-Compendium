...
#ifndef MultiThread
#ifdef EventMon
word linenum;		/* source line number */
word column;		/* source column number */
static word lastline;		/* last source line number */
extern FILE *monfile;		/* monitoring file */
#endif					/* EventMon */
 ...
 */
...
#ifdef EventMon
#if UNIX
      if (ticker.l[0] + ticker.l[1] != oldtick)
	 InterpEVTick();
#endif					/* UNIX */
#endif					/* EventMon */
   ...
   if (!is:null(curpstate->eventmask) &&
	...
	if (!current_line_ptr ||
	    ...
	    if (lastline != linenum) {
	       lastline = linenum;
	       if (current_line_ptr->line >> 16)
		  EVVal(current_line_ptr->line, E_Loc);
	       }
	    }
	}
      }
...
#ifdef MultiThread
   if (!EventStream && is:null(curpstate->eventmask))
#else					/* MultiThread */
   if (!EventStream)
#endif					/* MultiThread */
      return;
