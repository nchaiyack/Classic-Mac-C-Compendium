...
#ifndef THINK_C  /* Think C uses tmpnam(), not mktemp() */
...
char *string2;
{
  ...
  cp = string2;  i = 0;
  while (*cp++) i++;         /* i is length of string2 */

  /* mallocate new string with length: |s1| + |s2| */
  ostring = NEW2(i+end1+1, char);
  
  cp = ostring;
  cp1 = string1;
  for (i = 0; i < end1; i++)	/* copy s1 to ostring */
    *cp++ = *cp1++;

  cp1 = string2;
  while (*cp++ = *cp1++) ;		/* now tack on s2 to ostring */

  return ostring;				/* return new string */
}
...
openfiles()
{
...
#else
#ifdef THINK_C
  char *tmp_base = ":";	 /* specify CWD on macs... */
#else
  char *tmp_base = "/tmp/b.";
#endif  /* THINK_C */
#endif
  ...
  { char foo[L_tmpnam];
    ...
    (void)tmpnam(foo);
    foo2 = stringappend(tmp_base, tmp_len, "act.");
    actfile = stringappend(foo2, strlen(foo2), &foo[0]);
    free(foo2);

    (void)tmpnam(foo);
    foo2 = stringappend(tmp_base, tmp_len, "attrs.");
    tmpattrsfile = stringappend(foo2, strlen(foo2), &foo[0]);
    free(foo2);

    (void)tmpnam(foo);
    foo2 = stringappend(tmp_base, tmp_len, "tab.");
    tmptabfile = stringappend(foo2, strlen(foo2), &foo[0]);
    free(foo2);
  }
#else
...
#endif /* MSDOS */
#ifdef THINK_C
  if (actfile)      (void)remove(actfile);
  if (tmpattrsfile) (void)remove(tmpattrsfile);
  if (tmptabfile)   (void)remove(tmptabfile);
#endif
  exit(k);
#endif /* not VMS */
}
