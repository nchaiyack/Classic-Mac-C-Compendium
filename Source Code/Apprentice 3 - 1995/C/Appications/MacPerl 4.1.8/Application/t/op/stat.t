...
`delete -y Op.stat.tmp2; ln Op.stat.tmp Op.stat.tmp2; chmod 644 Op.stat.tmp`;
...
`delete -y Op.stat.tmp Op.stat.tmp2`;
...
chdir "$ENV{'MPW'}Tools:" || die "Can't cd to $ENV{'MPW'}Tools:";
while (defined($_ = <Å>)) {
    $cnt++;
    $uid++ if -u;
    last if $uid && $uid < $cnt;
...
unless (open(tty,"Dev:Console")) {
    print STDERR "Can't open Dev:Console--run t/TEST outside of make.\n";
}
