...
lunghezza max delle righe nel testo (legate, e non coincidenti, perché ad es 0 ha un
significato speciale) ma è molto più semplice sia un valore costante, anche molti programmi 
seri fanno così */
...
void new_window()
{
	...
	if((i=get_window_index())<0)return; /* non dovrebbe capitare, open è disabilitato */
...
/*	se gli si fornisce l'area allocata, NewWindow non fa altro che ritornare il 
	primo parametro, quindi è inutile leggerlo a meno di test errori... */
	...
	destRect.right = destRect.left + MaxDocWidth;inutile perché SetView lo riassegna*/
#endif
	...
	/* il chiamante dovrà occuparsi dei valori corrente e max delle scrollbar 
	(non posso farlo ora perché il buffer è ancora vuoto)
...
if(n_currently_open>=n_max_windows)return -1;	/* non dovrebbe succedere, il comando è disabilitato...
...
(**myTEH).viewRect.bottom = ((WindowPtr)w)->portRect.bottom- SBarWidth;	/* perché è mantenuta
...
/*
se l'altezza del font è cambiata bisogna adeguare la posizione verticale, se la larghezza
è cambiata (e non posso guardare carattere per carattere nel caso di font proporzionali...)
...
/* purtroppo alla routine di ClikLoop è stata imposta un'interfaccia che, pur
...
{
asm{
extern myClickLoop: /* questo è il vero entry point, non voglio eseguire le operazioni
...
/* i casi sono due: o la posizione è cambiata per azioni nel contenuto della finestra
(tasti return o freccia o delete o paste o autoscroll...) e allora chiamo MaintaiScrollBars  
per adeguare la posizione dei bottoni delle scrollbar, o si è agito sulle scrollbar
...
per non chiamare queste routines se il nuovo valore è uguale al vecchio, ma se devo
...
/* se è attivato l'autoscroll, il TextEdit provvede a portare in vista il punto in cui
sta il cursore, quindi non devo fare altro che sistemare le scroll bar, e lo faccio senza
chiamare questa routine; sulle vecchie ROM l'autoscroll non è disponibile, e allora non 
...
logaritmica. Non l'ho cambiato perché viene usato solo con le ROM pre-Mac plus...
Nota che l'ultimo lineStarts è sempre uguale alla lunghezza del testo, per cui il 
 ...
 posso evitare di fare la ricerca */
	...
	if (TEp->selStart < TEp->lineStarts[topLine] ||
		TEp->selStart >= TEp->lineStarts[bottomLine]) {
			...
			TEp=*TEH;	/* SetCtlValue può chiamare il Quickdraw e quindi anche il MemMgr */
		}
}
...
register WindowPtr w;
{
	...
	if(((window_def*)w)->flags & HSCROLL)
		...
		SetCursor(&waitCursor);
		TECalText(((window_def*)w)->TEH); /* anche in caso di creazione, perché lo 
				faccio PRIMA di caricare il file, il buffer è vuoto... */
		SetCursor(&arrow);
		}
#else
	TEp->destRect.right = TEp->viewRect.right;
...
EventRecord	*theEvent;
{
	...
	if (PtInRect( theEvent->where, &(**TEH).viewRect )){
		...
		/* senza autoscroll il TEclick non può avere provocato scroll e quindi 
		il punto di selezione è ancora in vista, con autoscroll ci pensa quello
		a portarlo in vista comunque, però la posizione può 
...
Point p;
{
	...
	long	theResult;
	Rect 	r;
	/* in modalità autowrap e per file lunghi questa routine e MyZoomWindow sono lente, 
	ma visto che così non è in modalità scroll, suppongo che la colpa sia del ricalcolo 
	degli inizi di riga per cui non c'è niente da fare */
...
#endif
	/* in modalità autowrap cambiare dimensioni cambia anche la
	suddivisione in righe, bisogna verificare che più o meno quello che viene
...
/******************************************************************************
 PositionDialog		(from tinyEdit.c, another demo editor from Symantec, 
		...
		(utilities.c) ma ho la sensazione che faccia così, lascia fuori la
		menù bar dal conto
 ******************************************************************************/
