...
/* prima o poi bisognerà spezzarlo in un pezzo ad alto livello (public domain)
...
/*#define SUPPORTED(l,command) (((l)>>(31-(command)))&1) è OK ma io la
uso quasi sempre con un command costante e l'espressione sotto è più
efficiente effettuando lo shift a compile time, e un compilatore intelligente
potrebbe anche rendersi conto che si può usare BTST */
...
se riesco a sapere che la causa è quella
...
if(di.v.si.DeviceType==TAPE_UNIT){
	...
	if( floppy_buffer_size==18 ){	/* cioè se è il valore di default, e non
								stato forzato qualcos'altro /
		/* normalmente a questo punto ho circa 30k, se ne vedo di più la utilizzo
		come buffer... */
		long free=MaxBlock();

		free&=~1023L;	/* multiplo intero di 1k...*/
		if(free>=50*1024L){	/* 50 K è circa ciò di cui ha bisogno suntar
								per non rischiare di stare stretto */
			free-=50*1024L;
			if(free>256*1024L){
				free-=(free-256*1024L)>>2;	/* non esageriamo, lascio un quarto
												di ciò che supera i 256k...*/
				free&=~1023L;
				}
...
if(eof_in_buf &&!(unsupp_ops&write_filemark)){
	...
	do_write_filemarks(di.drive_number,1);	/* che sia dirty_buffers
				non basta, devo sapere se ci vuole, cioè se l'ultima operazione
				era una azzera_settore: ho introdotto una variabile per questo,
				ma è statica di diskDriver e preferisco rimanga tale */
	di.v.si.current_tape_block=0;
	}
}
	...
	if(!(unsupp_ops&skip_file_backwards) && ((supp_ops&skip_file_backwards) ||
	   di.v.si.supported_group0==0 || SUPPORTED(di.v.si.supported_group0,SCSI_SPACE)) ){
		...
		if(err_code==0){
			...
			err_code=0;	/* beginning of medium, va benissimo così, anzi sarà la normalità */
		else{
...
jmp_buf savebuf;
	...
	if(setjmp(main_loop)<0){ 	/* l'unico longjmp possibile è un comando Abort */
		fase=oldfase;
		di.v.si.current_tape_block = 0x7FFFFFFF;
...
if(di.v.si.DeviceType==TAPE_UNIT)
	...
	case tmSkipFile: /* skip filemark */
		if(di.v.si.supported_group0!=0 && !SUPPORTED(di.v.si.supported_group0,SCSI_SPACE) ||
		   (unsupp_ops&skip_file_forward) ){
			...
			if(err_code==eofErr) err_code=0;	/* perché è questo che cercavo */
			}
		else{
			flush_buffers(2);
			invalid_buffers();
	...
	case tmTestRead:
		...
		DisableItem(deviceMenu,0);	/* dovrei anche chiamare DrawMenuBar per far
					vedere che 'Device' è grigio, ma è così poco importante,
		...
		if(!my_atol((char*)&disk_buffer[i],&trlen)){
			...
			buffer grosso non l'ho e la modalità skip attualmente non conta il
	...
	if(!err_code && inq_response.AdditionalLength>=31){
		...
		if(expert_mode){
			...
			if(p<(char*)&inq_response+inq_response.AdditionalLength+4){
				...
				if(p[-1]>=' '&&p[-1]<=126) p--;		/* non c'è se non ci sono i supported commands */
...
{
/* io ovviamente ne stampo solo alcuni, che spero siano i più comuni. Comunque,
i codici hanno una gerarchia, per cui il qualifier aggiunge informazioni
al "additional sense code" per cui se il qualifier è non zero e non lo trovo,
almeno stampo il messaggio per quando il qualifier è 0, e quelli ce li ho messi
quasi tutti, lasciando fuori solo quelli evidentemente usati solo da scanner
e stampanti
N.B. sarebbe più elegante se anche la tabella dei codici fosse in una risorsa
...
static unsigned char sense_key[]={
	...
	0x5C,0x63,0x64
	/* son quasi tutti, si risparmierebbe memoria a elencare chi non c'è
	invece che chi c'è, o a metterceli proprio tutti */
	};
...
if(verbosity>=2)printf("write %d sectors starting from %ld\n",n_sect,sn);
	...
	do{
		...
		err=do_scsi_command_out(scsiID,&scsiCMD,buffer,buf_size,&returnedStat,&returnedMessage,writeTimeout);
				/* uff, non so, anche se è un direct access potrebbe avere dei tempi morti, ad es. per
...
long trlen;
	...
	if(di.v.si.block_mode==fixed_blocks || di.v.si.block_mode==firmware_fixed_blocks){
		...
		if(unsupported_ops&standard_variable_blocks){	/* allora devo leggere il vero valore o mi dà errore */
	...
	/* a questo punto non faccio nessuna gestione errori perché non
	è ben chiaro quel che una unità risponderà a ciò che ho fatto */
	PRINT_SENSE;
	CLEAR_RECOVERED_ERROR
	/* altri errori come CRC error vanno ignorati tanto volevo solo
	skippare... ma il problema è che comunque bisogna capire quanti
	blocchi sono stati passati, per di più CRC error è un sottocaso
...
short buf_size;
{
	...
	if(command!=TEST_UNIT_READY){
		...
		/* uso quello poco verboso perché si tratta di READ CAPACITY, READ BLOCK LIMITS
		ALLOW MEDIUM REMOVAL, nessuno di questi è un comando critico durante l'esecuzione */
		}
	if(err)
		err_code=err;
	else if(err_code)
	...
	if(!err_code&&returnedStat) err_code=command==TEST_UNIT_READY ? 1 : scsiBadStatus;
			/* per TEST_UNIT_READY returnStat non è un codice di errore */
}

static void do_mode_sense(scsiID,buffer,which)
/* MODE SENSE che ha la stessa necessità di doppia chiamata di inquiry, e poi
può avere un parametro variabile in più
...
unsigned char*buffer;
{
	...
	while(n_to_read>=4){
		...
		fillmem(buffer,0,16);	/* in caso di errore non voglio avere garbage, anche
			perché qualche volta lo stampo senza guardare se c'è stato errore */
		...
		*/
		...
		if(!err_code && bytes_for_reqsen< -1000){
			...
			if((n_to_read != 32  || l!=buffer[7]+8) && n_to_read!=buffer[7]+8 &&
				...
				bytes_for_reqsen= n_to_read;
			/* uff, questo non è molto naturale, ma ci provo. Ci sono delle
			unità che a chiedergli 0 bytes ritornano comunque un extended format,
			il che è contrario a ciò che dice lo standard, ma comunque è una
			possibilità perché mi evita l'imbarazzo di decidere io. Per cui provo,
			se lui si comporta così allora preferisco quel modo
...
#undef PRINT_SENSE
/* !!!! è un po' troppo complicata come macro, può aver senso che almeno
...
if(*sn>di.v.si.current_tape_block){
	...
	while(*sn>di.v.si.current_tape_block){
		...
		if(l+n_read>*sn){	/* il settore richiesto
				è nel buffer ma non è al primo posto, comunque devo ritornare
				con quello che ho */
			*sn=l;
			return n_read;
			}
...
if(unsupported_ops&standard_variable_blocks){	/* uff, posso sbagliarmi ma credo che
							questo non abbia più senso, derivava da una errata
							interpretazione di alcuni messaggi di errore, del
							resto non è mai stato provato.
		   ...
		   (req_sense_response[2]&0x20)) { /* incorrect length indicator */
			if(l>0 && l==trlen-di.v.si.block_size){	/* the block was correctly
				...
				if(l<=0 && trlen==di.v.si.block_size) /* ha trasferito i bytes richiesti
					ma non sono ancora tutti; anche l=0 perché se ILI è settato vedi
...
the actual number of blocks successfully read". Cioè, il numero è in
...
if(!err && !err_code && !returnedMessage &&
   ...
   trlen>= di.v.si.block_size && trlen!=((long)n_to_read<<9)){
	if(!sense_err_code && ((req_sense_response[0]&0x7E)==0x70 &&
		...
		/* è arrivato qualcosa ma non quello che ho chiesto, sono a EOF
		e quindi deduco che il motivo è quello */
...
if(err_code ){
	/* comunque non mi è arrivato quanto mi aspettavo. Ma potrebbe
	essermi arrivata della roba buona, e questa roba buona
	potrebbe contenere la fine dell'archivio così che non
	mi verrà mai chiesto di leggere dopo di essa */
...
/* ok, è arrivato esattamente quel che volevo */
...
/*dp("cmd=%x\n",scsiCMD[0]);*/
	...
	if(lv.gotSCSIBus){
		...
		if(lv.err == noErr){
			...
			/* una request sense per saperne di più, ma dopo la SCSIComplete..... */
...
è chiaro che se avviene un page fault mentre il bus SCSI è inutilizzabile da altri che me...
