...
Rect r;
	...
	TextSize(12);
	if(list_with_textedit){	/* ce ne è solo uno e ha un bottone di default */
		Handle h;
		Rect r;
		short kind;
...
Boolean drawit;
{
	...
	help_string=in_Italia?"usa command-clic o shift-clic per estendere/ridurre la selezione":
		...
		/* devo assegnare qui e non in updateList perché non posso cambiare stringa
		mentre è visualizzata (a meno di cancellare tutto, ovvero fare un invalidRect) */
	if(dlg_id==152) help_string="";
	semimodal_list=false;
	list_with_textedit=false;

	alertHandle = (AlertTHndl)GetResource('DLOG',dlg_id);
	/*if(alertHandle==NULL) return; o ExitToShell, è un errore gravissimo */
	oldflags=HGetState(alertHandle);
	HNoPurge((Handle)alertHandle);
...
if(fmt==tar_format){
	...
	return;		/* i messaggi sono già comparsi... */
	}
}
...
for(;;){		/* loop on disks, for POSIX only */
	...
	while(last_header<di.sectors_on_floppy){
		...
		if(linkflag=='L'||linkflag=='K'){
			...
			if(linkflag=='L'){
				...
				if(last_header<=di.sectors_on_floppy-3){/* cioè, estrarlo non comporterebbe eiettare */
					end_of_file();
	...
	while(!list_button_code){
		...
		if(list_button_code==sdFind){
			...
			if(myCell.v==last_list_cell)	/* è la cella vuota in fondo
								--the empty cell */
				more_files=false;
			if(more_files)
				select_directory();
	...
	if(list_button_code==sdCancel)
		break;
	if(list_button_code==sdExtract){	/* se sono qui, ho già selezionato il folder
				e ho già trovato il primo item selezionato
		...
		*/
		...
		while(more_files){
/* il get va fatto prima, sia per non chiedere destinazione se nessuno è selezionato,
...
*/
		...
		if(tar_version==tar_POSIX && last_header>=di.sectors_on_floppy){
			...
			sectListSize=0;
			last_list_cell=0;
			/* non c'è bisogno di deallocare sectorList perché tanto ci
			penserà la SetHandleSize */
			/* get next disk */
			last_header-=di.sectors_on_floppy;
			if(!disco_espulso){
				riprova:
...
short foundref; long founddir;
	...
	/* if lista vuota dà un dialogo di alert */
	if(last_list_cell==0){
		dispose_list_dialog();
...
if(ln+ls > ci_stanno_certo){
	...
	GetPort( &savePort );
	SetPort( ListDialog );	/* perché è dalla GrafPort che StringWidth deduce
							font e size da usare... */
	my_c2pstr(buffer);
	i=1;
	while(StringWidth(buffer)>pixel_size){
		while(i<ln && buffer[i]!='/') i++;
		if(i>=ln) break;	/* non c'è nessun path da tagliare, meglio lasciare
							così come è */
		buffer[1]='…';
...
if(myDialog!=NULL){
	if(in_Italia){
		...
		static char* titoli[]={"\pTrova","\pNon trovare",
			"\pil testo è all\'inizio del nome","\pSeleziona i nomi contenenti" };
		#else
		static char* titoli[]={"\pTrova","\pNon trovare",
...
switch(theEvent->what){
/* no, non c'è un bottone di default ! e devo lasciare attivi i comandi
case keyDown:
case autoKey:
	if((unsigned char)theEvent->message==CR || (unsigned char)theEvent->message==enter_key)
		list_button_code=sdExtract;
	...
	if(get_event(&myEvent)){
		...
		if(myEvent.what==mouseDown||myEvent.what==keyDown&&!(myEvent.modifiers & cmdKey)){
			...
			if(((char)myEvent.message|0x20)=='q')
				...
				finestra NON è accompagnato dai TEtoScrap ? */
				}
			}
