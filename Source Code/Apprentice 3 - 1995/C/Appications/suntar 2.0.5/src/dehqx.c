...
if(info_file_open==0){	/* dovrei creare il .info, ma prima dei dati hqx non c'era nulla,
	...
	/* reinit_hd_buffering(); non necessario, il buffer è vuoto avendo appena
...
/* se sono qui c'era .hqx, ma se c'è anche part devo non convertire */
...
return -1;	/* il .hqx è preceduto da part1 o part2 etc., sarebbe inutile cercare 
				di convertire */
}
...
/* per la verità non sarebbe necessario, il CRC è fatto in modo che se si calcola
...
if(!hqx_length) return -1;		/* sono già in fondo, capita se non ho trovato l'inizio */
...
/* queste informazioni non c'è bisogno di settarle, basta modificare la routine
...
e la possibilità di gestire anche files in cui la scritta di cui sopra è stata tolta, 
col che c'è solo un ":" seguito da uno dei primi codici della stringa hqx (è la 
...
/* controllo nel caso manchi la stringa ma ci siano i dati BinHex */
	if(b==LF||b==CR||b=='\f')
		...
		stat=2;
/* controllo per il caso non ci siano né stringa né dati BinHex */
	if(--timeout==0){
		char buffer[512];
		start_of_line();
		...
		if(! save_info)
			...
			flush_hqx_header(0);		/* ora passo in modalita' salvataggio 
							normale, vorrà dire che mando tutto nel .info 
...
/* copia dei caratteri sulla console e/o file .info */
	if(flush_needed || pos==sizeof(startstr)-1){
		if(save_info && oldpos!=0 && pos!=sizeof(startstr)-1)
			if(write_hd(startstr,oldpos))	/* è poco pulito farlo così, fidandosi
				che il file è aperto e contiene quello che deve contenere...*/
				write_error_message();
...
/*
io uso tre buffer: quello di uscita è il solito buffer, quello di ingresso 
contiene il blocco letto dal disco; in più, le conversioni sono fatte a
...
while(nbytes--){
	...
	il merge delle azioni comuni fra più rami, e questo è il risultato
...
repeated 520 times. Unfortunately, suntar 1.1 did not handle those cases correctly */
			...
			k--;	/* caso normale: è il più corto e non è seguito da
...
if(l>lext && !ci_strcmp(full_name+l-lext,ext)){
	for(i=l-1;i>=0 && (full_name)[i]!='/';i--)
		;
	/* ora i è -1 o punta ad un '/', quindi la lunghezza del nome è : */
	i=l-i-1;
	if(i>=31)
...
if(show_info){
	...
	disable_autoflush(2);	/* in alcuni casi un messaggio (nuovo disco...) può averlo
							abilitato */
...
while(current_run_length>255){
	/* per la verità non è necessario ripetere il carattere ogni volta,
	ma farlo riduce la probabilità di problemi, e poi quando non lo faceva
	...
	for(;;){
		...
		*q++=ENC(*p&0x3F);
		...
		if(chars_in_line==45){	/* since chars_in_line is increased by 3 except at the
								...
								exactly equal to 45 */
			*q=CR;		/* !!! attento, solo se su file, quando potrò
