/*
	...
	derived from protz.c by Doug Evans
*/
 ...
 *****************************************************************************
 *
 * Fitting this into Taylor UUCP:
 *
 * The protocol entry in <asProtocols> is:
 *
 *	{ 'a', FALSE, RELIABLE_EIGHT,
 *		asZproto_params, fzstart, fzshutdown, fzsendcmd, zzgetspace,
 *		fzsenddata, fzprocess, fzwait, fzfile },
 *
  *****************************************************************************
 *
 * Notes:
...
#define eSendfile fp
#define ffileseek(fd, offset, type) (lseek(fd, offset, type) != EOF)
#define xmalloc NewPtr
#define xfree DisposPtr
#define fsend_data(ptr, count, doread) swrite(ptr, count)
#define ffileclearerror(x) /* x */
#define usendfile_error(x) /* x */
#define fgot_data(a,b,c,d,e) (1 /* a b c d e */)
...
static int cZrx_buf_len = 0;			/* our receiver's buffer size */
static boolean fZesc_ctl = FESCAPE_CONTROL;	/* escape control chars */

#ifndef	THINK_C

struct scmdtab asZproto_params[] =
{
	{"timeout", CMDTABTYPE_INT, (pointer) & cZtimeout, NULL},
	{"retries", CMDTABTYPE_INT, (pointer) & cZretries, NULL},
	{"startup-retries", CMDTABTYPE_INT, (pointer) & cZstartup_retries, NULL},
	{"garbage", CMDTABTYPE_INT, (pointer) & cZmax_garbage, NULL},
	{"send-window", CMDTABTYPE_INT, (pointer) & cZtx_window, NULL},
	{"escape-control", CMDTABTYPE_BOOLEAN, (pointer) & fZesc_ctl, NULL},
	{NULL, 0, NULL, NULL}
};
...
boolean fzstart P((boolean fmaster));
boolean fzshutdown P((void));
boolean fzsendcmd P((const char *z));
char *zzgetspace P((int *pcdata));
boolean fzsenddata P((char *z, int c));
boolean fzprocess P((boolean *pfexit, long *plredo));
boolean fzwait P((void));
boolean fzfile P((boolean fstart, boolean fsend, long *plredo, long cbytes));

static boolean fzsend_data P((char *zdata, int cdata, boolean fendofmessage));
static boolean fzstart_proto P((boolean fmaster));
static int izexchange_init P((boolean fmaster, int send_type, achdrval_t send_val, achdrval_t recv_val));
static boolean fzshutdown_proto P((void));
static boolean fzstart_tx P((void));
static boolean fzfinish_tx P((long *plredo));
static boolean fzstart_rx P((void));
static boolean fzfinish_rx P((void));
static boolean fzsend_hdr P((int ipkttype, int ihdrtype, hdrval_t hdrval, boolean fcheckreceive));
static boolean fzsend_data_packet P((char *zdata, int cdata, int frameend, boolean fcheckreceive));
static int czbuild_header P((char *zresult, int ipkttype, int ihdrtype, hdrval_t hdrval));
static int czbuild_data_packet P((char *zresult, const char *zdata, int cdata, int frameend));
/*
 ...
 */
static int izrecv_hdr P((achdrval_t hdr));	/* IE: zgethdr() */
static int zrbhdr32 P((achdrval_t hdr));
static int zrhhdr P((achdrval_t hdr));
static int zrdat32 P((char *buf, int length, int *iprxcount));
static int getinsync P((boolean flag));
static char *zputhex P((char *p, int ch));
static char *zputchar P((char *p, int ch));
static int zgethex P((void));
static int zdlread P((void));
static int noxrd7 P((void));
static int realreadchar P((int timeout));
...
static winpos_t lzupdate_rxpos P((achdrval_t rx_hdr, winpos_t rxpos, winpos_t lrxpos, winpos_t txpos));

#ifdef THINK_C

#define READCHAR(b, i) b = realreadchar(i)
...
/* 
	And now for a thin layer of uupc glue on top of the a-protocol glue on
   top of the ZMODEM code...
*/
...
int awrmsg(char *str, boolean nowait) /* nowait semantic not implemented yet */
{
	if (fzsendcmd(str)) {
		return 0;
	} else {
...
boolean fmaster;
{
	...
	printmsg (1, "Protocol 'a' starting: %d, %d, %d, %d, %d, %d",
	...
	if (cZtx_window % 1024 != 0 ||
		...
		printmsg (0, "fzstart: cZtx_window not one of 4096, 8192, 16384, 32768, 65536");
		return FALSE;
	}

	zZtx_buf = (char *) xmalloc (CPACKETSIZE);

	zZtx_packet_buf = (char *) xmalloc (CFRAMELEN + 2 * CPACKETSIZE + CSUFFIXLEN + 42 /*slop*/);
	zZrx_packet_buf = (char *) xmalloc (CFRAMELEN + 2 * CPACKETSIZE + CSUFFIXLEN + 42 /*slop*/);
	 ...
	 */
	if (cZblklen_override > cZtxwspac || (!cZblklen_override && cZtxwspac < 1024))
...
#ifdef DJE_TESTING
	{
		...
		if (uucptest == -1) {
			if ((f = fopen ("/usr/local/src/bin/uucp/uucptest", "r")) != NULL) {
				fscanf (f, "%d %d %d",
...
const char *z;
{
	...
	printmsg (5, "fzsendcmd: sending command %s", z);

	if (!fzstart_tx ())	/* must be called before zzgetspace() */
		return FALSE;

	if ((zbuf = zzgetspace (&n)) == NULL)
		return FALSE;
	...
	do {
		if (!fzsend_data (zbuf, clen, TRUE))
			return FALSE;
		if (!fzfinish_tx (&lredo))
			return FALSE;
	} while (lredo >= 0);
...
boolean
fzsenddata(zdata, cdata)
char *zdata;
int cdata;
{
	printmsg (5, "fzsenddata: %d bytes", cdata);

	return fzsend_data (zdata, cdata, cdata == 0);
}
...
static boolean
fzsend_data(zdata, cdata, fendofmessage)
char *zdata;
int cdata;
boolean fendofmessage;
{
	...
	if (iZlast_tx_data_packet == -1 || iZlast_tx_data_packet == ZCRCW) {
		cZtxwcnt = cZrxwcnt = 0;
		iZjunk_count = 0;
		if (!fzsend_hdr (ZBIN, ZDATA, hvzencode_data_hdr (wpZtxpos), TRUE))
			return FALSE;
	}
	...
	if (FDEBUGGING(DEBUG_PROTO)) {
		...
		ulog (LOG_DEBUG, "fzsend_data: %s, pos 0x%lx, %d bytes",
			type, wpZtxpos, n);
	}
#endif

	if (!fzsend_data_packet (zdata, n, iZlast_tx_data_packet, TRUE))
		return FALSE;
	...
	if (iZlast_tx_data_packet == ZCRCW) {
		 ...
		 */
		switch (getinsync (FALSE)) {
		case ZACK:
			break;
		case ZRPOS:
			iZlast_tx_data_packet = -1; /* trigger ZDATA */
			ffileclearerror (eSendfile);
			if (!ffileseek (eSendfile, wpZrxpos - wpZtxstart, 0 /*SEEK_SET*/)) {
				printmsg (0, "seek: %s", strerror (errno));
				usendfile_error ();
				return FALSE;
			}
			break;
	...
	while (wpZtxpos - wpZlrxpos >= cZtx_window - 2048) {
		if (iZlast_tx_data_packet != ZCRCQ) {
			if (!fzsend_data_packet (zdata, 0,
					iZlast_tx_data_packet = ZCRCQ, TRUE))
				return FALSE;
		}
		/*
 ...
 * This function is only called when sending a file.
 *
 * Set *pfexit to TRUE if a file or a command has been
 * completely received. (FIXME: not supported at present).
 *
 * If the protocol needs uucp to fseek() to some point, *plredo is set to
 * that point. Otherwise it is -1L.
 */

boolean
fzprocess(pfexit, plredo)
boolean *pfexit;
long *plredo;
{
	int c,ch;
	...
	*plredo = -1;
	...
	while (fzreceive_ready ()) {
		...
		case ZPAD:
			...
			printmsg (5, "fzprocess: possible ZRPOS packet");
			c = getinsync (TRUE);
			if (c == ZACK)
				break;
			...
			if (!fzsend_data_packet (zZtx_packet_buf, 0, ZCRCE, TRUE))
				return FALSE;
#endif
			if (c == ZRPOS) {
...
fzwait()
{
	...
	for (;;) {
		if (!fzsend_hdr (ZHEX, ZRPOS, hvzencode_data_hdr (wpZrxbytes), FALSE))
			return FALSE;
nxthdr:
		c = izrecv_hdr (rx_hdr);
		...
		case ZNAK:
			if (--cerr < 0) {
				printmsg (0, "fzwait: retries exhausted");
				return FALSE;
			}
			continue;
		case ZM_ERROR:
		...
		case ZDATA: {
			...
			if (rx_bytes != wpZrxbytes) {
				...
				(void) zrdat32 (zZrx_packet_buf, 1024, &rxcount);
				/*fport_break ();*/
				/*
...
moredata:
			 ...
			 */
			c = zrdat32 (zZrx_packet_buf, 1024, &rxcount);
#if DEBUG > 0
			if (FDEBUGGING(DEBUG_PROTO)) {
				const char *msg;
				...
				if (msg != NULL)
					ulog (LOG_DEBUG, "fzwait: zrdat32: %s, %d bytes", msg, rxcount);
				else
					ulog (LOG_DEBUG, "fzwait: zrdat32: %d, %d bytes", c, rxcount);
			}
#endif
			switch (c) {
			...
			case GOTCRCW:
				...
				cerr = cZretries;
				if (rxcount != 0 && !fgot_data (zZrx_packet_buf, rxcount, FALSE, FALSE, &fexit))
					return FALSE;
				wpZrxbytes += rxcount;
				if (!fzsend_hdr (ZHEX, ZACK, hvzencode_data_hdr (wpZrxbytes), FALSE))
					return FALSE;
				fsend_data (&xon, 1, FALSE);
				goto nxthdr;
			case GOTCRCQ:
			...
			case GOTCRCF:
				...
				if (!fzfinish_rx ())
					return FALSE;
				if (!fgot_data (zZrx_packet_buf, rxcount, FALSE, FALSE, &fexit))
					return FALSE;
		...
		default:
			printmsg (0, "fzwait: received header %s",
				ZZHEADER_NAME(c));
			return FALSE;
		}
	}
 ...
 *
 * When starting to send a file:	(TRUE, TRUE, NULL, cbytes)
 * When starting to receive a file:	(TRUE, FALSE, NULL, -1)
 * When send EOF, check resend:		(FALSE, TRUE, &lredo, -1)
 * When receive EOF, check re-receive:	(FALSE, FALSE, &fredo, -1)
 */

boolean
fzfile(fstart, fsend, plredo, cbytes)
boolean fstart;
boolean fsend;
long *plredo;
long cbytes;
{
	printmsg (5, "fzfile: fstart=%d, fsend=%d", fstart, fsend);
	if (plredo != NULL)
		*plredo = -1;
...
const static long crc_32_tab[] = { /* CRC polynomial 0xedb88320 */
 ...
 *
 * As with 'g', we ignore <fmaster>.
 */

static boolean
fzstart_proto(fmaster)
boolean fmaster;
{
	int i,n;
	...
	for (i = 0; i < cZstartup_retries; i++) {
		...
		switch (izexchange_init (fmaster, ZINIT, tx_hdr, rx_hdr)) {
		case -1: return FALSE;
		case 0:  continue;
		case 1:  break;
		}
		...
		if (rx_hdr[ZF0] == 0) {
			printmsg (0, "Old protocol version, init failed");
			return FALSE;
		}
#endif
		...
		stohdr (0L, tx_hdr);
		switch (izexchange_init (fmaster, ZDATA, tx_hdr, rx_hdr)) {
		case -1: return FALSE;
		case 0:  continue;
		case 1:  break;
		}
		...
		stohdr (0L, tx_hdr);
		switch (izexchange_init (fmaster, ZACK, tx_hdr, rx_hdr)) {
		case -1: return FALSE;
		case 0:  continue;
		case 1:  break;
		}
		...
		stohdr (0L, tx_hdr);
		switch (izexchange_init (fmaster, ZINITEND, tx_hdr, rx_hdr)) {
		case -1: return FALSE;
		case 0:  continue;
		case 1:  break;
		}

		printmsg (5, "fzstart_proto: Protocol started");
		return TRUE;

		/* FIXME: see protg.c regarding sequencing here. */
	}

	printmsg (0, "Protocol init failed");
	return FALSE;
}
...
static int
izexchange_init(fmaster, send_type, send_val, recv_val)
boolean fmaster;
int send_type;
achdrval_t send_val;
achdrval_t recv_val;
{
	...
	for (i = 0; i < CEXCHANGE_INIT_RETRIES; i++) {
		if (!fzsend_hdr (send_type == ZDATA ? ZBIN : ZHEX,
		...
		if (send_type == ZDATA) {
			count = czbuild_data_packet (zZtx_packet_buf, "", 1, ZCRCF);
			if (!fsend_data (zZtx_packet_buf, count, FALSE))
				return -1;
		}

		recv_type = izrecv_hdr (recv_val);
		...
		case ZINITEND:
			break;
		case ZDATA:
			if (zrdat32 (zZrx_packet_buf, 1024, &count) == GOTCRCF)
				break;
			continue;
		default:
			continue;
...
fzshutdown_proto()
{
	(void) fzsend_hdr (ZHEX, ZFIN, 0L, FALSE);
	return TRUE;
}
...
fzfinish_tx(long *plredo)
{
	...
	*plredo = -1;
	...
	for (;;) {
		...
		switch (c) {
		case ZRPOS:
			wpZrxpos = lzupdate_rxpos (rx_hdr, wpZrxpos, wpZlrxpos, wpZtxpos);
			/*
		...
		case ZACK:
			wpZrxpos = lzupdate_rxpos (rx_hdr, wpZrxpos, wpZlrxpos, wpZtxpos);
			wpZlrxpos = wpZrxpos;
			if (wpZtxpos == wpZrxpos)	/* is this the ACK we want? */
				return TRUE;
			break;
		case ZDATA:
			/*
		...
		case ZNAK:
			...
			if ((wpZrxbytes + 1024L & ~1023L) == rx_bytes)
				return TRUE;
			if (rx_bytes == wpZrxbytes) {
				if (!fzsend_hdr (ZHEX, ZACK, hvzencode_data_hdr (wpZrxbytes), TRUE))
					return FALSE;
			}
			break;	/* ignore, out of sync (old) */
		case ZFIN:
		...
		case ZM_TIMEOUT:
			if (--cerr < 0) {
				printmsg (0, "fzfinish_tx: retries exhausted");
				return FALSE;
			}
			/*
			 ...
			 */
			if (++ctimeouts % 2 == 0)
				if (!fzsend_hdr (ZHEX, ZNAK, hvzencode_data_hdr (wpZtxpos), TRUE))
					return FALSE;
			break;
		case ZM_ERROR:
		default:
...
fzfinish_rx()
{
	printmsg (5, "fzfinish_rx: message/file received");

	return fzsend_hdr (ZHEX, ZACK, hvzencode_data_hdr (wpZrxbytes), FALSE);
}
...
static boolean
fzsend_hdr(ipkttype, ihdrtype, hdrval, fcheckreceive)
int ipkttype;
int ihdrtype;
hdrval_t hdrval;
boolean fcheckreceive;
{
	...
	printmsg (5, "fzsend_hdr: %s, data = 0x%lx",
	...
	if (ihdrtype == ZACK || ihdrtype == ZDATA) {
		...
		uucptest = uucptest2;
		cZheaders_sent++;
		fresult = fsend_data (zZtx_packet_buf, cpacketlen, fcheckreceive);
		uucptest = old;
		return fresult;
	}
#endif
#endif
	cZheaders_sent++;
	return fsend_data (zZtx_packet_buf, cpacketlen, fcheckreceive);
}
...
static boolean
fzsend_data_packet(zdata, cdata, frameend, fcheckreceive)
char *zdata;
int cdata;
int frameend;
boolean fcheckreceive;
...
hdrval_t hdrval;
{
	...
	case ZHEX: 	/* build hex header */
		...
		*p++ = CR;
		...
		break;
	default:
		printmsg (0, "czbuild_header: ipkttype == %d", ipkttype);
		break;
	}
...
fifi:
	...
	printmsg (5, "izrecv_hdr: %s, data = 0x%x",
		ZZHEADER_NAME(c), rclhdr (hdr));
...
boolean flag;
{
	...
	for (;;) {
		...
		case ZM_TIMEOUT:
			if (--cerr < 0) {
				printmsg (0, "getinsync: retries exhausted");
				return ZM_ERROR;
			}
...
int timeout;
{
...
#else
	int c;
	if ((c = breceive_char (timeout, TRUE)) >= 0)
		return c;
	...
	printmsg (0, "realreadchar: breceive_char() returned %d", c);
	return ZM_ERROR;
}
