...
int  dec_number(Str255 value);
int  dec_init(t_nshc_parms *nshc_parms,t_nshc_calls *nshc_calls,Str32 name,Str255 var,long *value,long *offset);
...
int dec_init( t_nshc_parms *nshc_parms,
			 ...
			 long *offset )	 
{
	...
	if ( ( argc < 2 ) || ( argc > 3 ) ) {
		...
		nshc_calls->NSH_putStr_err( "\pdec variable [offset]\r" );
	...
	while (c = *p++) {
		...
		if ( ( c >= '0' ) && ( c <= '9' ) ) error = 0;
		if (error) {
			nshc_calls->NSH_putStr_err( "\pdec: Invalid variable name = " );
	...
	if ( i > 31 ) {
		nshc_calls->NSH_putStr_err( "\pdec: Variable name too long = " );
		nshc_calls->NSH_puts_err( q );
		nshc_calls->NSH_putchar_err( '\r' );
		nshc_parms->result = NSHC_ERR_PARMS;
		nshc_parms->action = nsh_idle;
	...
	if ( nshc_calls->NSH_var_env( name, var ) ) {
		nshc_calls->NSH_putStr_err( "\pdec: Variable not found = " );
	...
	if ( dec_number( var ) ) {
		nshc_calls->NSH_putStr_err( "\pdec: variable must contain an integer.\r" );
		nshc_parms->result = NSHC_ERR_PARMS;
		nshc_parms->action = nsh_idle;
		return( 0 );
		}
	...
	if ( argc == 3 )
		if ( nshc_is_numeric_operand( nshc_parms, 2 ) )
			*offset = arg_to_num( nshc_parms, 2 );
			...
			nshc_calls->NSH_putStr_err( "\pdec: Offset must be an integer.\r" );
			nshc_parms->result = NSHC_ERR_PARMS;
			nshc_parms->action = nsh_idle;
			return( 0 );
			}
...
void main(t_nshc_parms *nshc_parms, t_nshc_calls *nshc_calls)
{
	...
	if ( dec_init( nshc_parms, nshc_calls, name, var, &value, &offset ) ) {
		...
		result = nshc_calls->NSH_var_set( name, var );
