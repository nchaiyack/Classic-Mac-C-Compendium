...
#pragma unused ( userDataHandle )
	...
	extern void MPSR_Preflight(short count,Handle* dataHdl);
	
	if (opening)// not while printing...
		MPSR_Preflight(itemCount,userDataHandle);
...
#pragma unused ( userDataHandle )
	...
	extern void MPSR_Postflight(short count,Handle dataHdl);
	...
	*/
	...
	MPSR_Postflight(itemCount,userDataHandle);
}
...
static OSErr ProcessItem(FSSpecPtr myFSSPtr)
{
	extern OSErr MPSR_Process(FSSpecPtr theSpec);
	OSErr	err = noErr;
	
	// do something here
	
	err=MPSR_Process(myFSSPtr);
