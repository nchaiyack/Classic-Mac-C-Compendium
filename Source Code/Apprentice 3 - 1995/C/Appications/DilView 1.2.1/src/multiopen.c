...
pascal void initList(DialogPtr theDialog)
{
	...
	GetDItem(theDialog, myScrollBar, &temp, &gTheScrollBar, &dataBounds);
...
pascal Boolean myFileFilter(CInfoPBPtr pb)
{
	...
	if( pb->hFileInfo.ioFlFndrInfo.fdType == 'GIFf' )
		return false;
	else
		return true;
}
...
void processData(void)
{
	...
	for (theCell.v = 0; theCell.v < (**gTheList).dataBounds.bottom; theCell.v++)
	{
		...
		// hopefully we have a valid FSSpec now..
		LoadAndAddGIF( &theFile );
	}
}
