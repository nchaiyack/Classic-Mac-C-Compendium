...
#include "wind.h"
#include "rsmac.proto.h"	/* For RSGetSelText */
...
void printPages(TPPrPort prPort, THPrint PrRecHandle, Str255 Title, short columns, char **charh, short refNum, short useTitle, short theScreen)
{
	...
	while (count<charlen) {
		...
		if (useTitle) {
			...
			DrawString("\pSession Name: ");			/* BYU LSC */
...
void PrintPageSetup(void)
{
	...
	if ( PrRecHandle != NULL ) {							/* AISD: JSA */
		...
		PrRecHandle=(THPrint)NewHandle((long)sizeof(TPrint));	/* AISD: JSA */
...
THPrint PrintSetupRecord(void)
{
	...
	if ( PrRecHandle != NULL ) {
		...
		PrRecHandle=(THPrint)NewHandle((long)sizeof(TPrint));
