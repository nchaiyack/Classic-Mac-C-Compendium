...
/* ChangedBranchTo: the branch toÉ field has changed */
...
/* Éstuff it into the micro instruction */
tempMir = editmir;
...
if (which == kL_COMMENTS) {
	if (inCkLoop) {
/* we must update the associative memoryÉ */
		*(long *)&instr = 0L;
...
/* Éand update the control store memory */
