...
/*#include <MacHeaders>*/  /* this causes problems */
#include <Serial.h>
#include <Types.h>		/* in MacHeaders */
#include <THINK.h>		/* in MacHeaders */
#include <Memory.h>		/* in MacHeaders */
#include <Fonts.h>		/* in MacHeaders */
#include <LoMem.h>		/* in MacHeaders */
#include <Desk.h>		/* in MacHeaders */
#include <Notification.h>	/* in MacHeaders */
#include <Resources.h>		/* in MacHeaders */
#include <Timer.h>		/* in MacHeaders */
#include <Dialogs.h>	/* in MacHeaders */
#include <asm.h>		/* in MacHeaders */
#include <TextEdit.h>	/* in MacHeaders */
#include <Errors.h>		/* in MacHeaders */
#include <Controls.h>	/* in MacHeaders */
#include <BDC.h>		/* in MacHeaders */
...
/* define a trap macro for _PBStatus, Immed */
...
struct TMtimer
{
	...
	voidProcPtr		proc;		/* procedure to execute on timeout */
...
struct LapInfo {
	...
	OSErrProcPtr	lapInit;	/* ptr to LAP init routine */
	OSErrProcPtr	lapOpen;	/* LAP open rtn */
	OSErrProcPtr	lapClose;	/* LAP close rtn */
	voidProcPtr		lapUnload;	/* LAP unload rtn, undoes lapInit */
	OSErrProcPtr	lapAttach;	/* LAP attach PH rtn */
	OSErrProcPtr	lapDetach;	/* LAP detach PH rtn */
	OSErrProcPtr	lapOutput;	/* LAP output rtn */
	OSErrProcPtr	lapControl;	/* LAP control rtn */
	voidProcPtr		lapFault;	/* LAP fault isolation rtn */
	OSErrProcPtr	lapStatistics;	/* LAP statistic reading rtn */
	voidProcPtr		lapConfigure;	/* LAP configuration rtn */
	BooleanProcPtr	lapProbe;	/* send a LAP-specific addr probe pkt */
	BooleanProcPtr	lapRegister;	/* register our IP address on net */
	voidProcPtr		lapFindGateway;	/* LAP-specific way to find gateway  */
	BooleanProcPtr	lapGwyCheck;	/* LAP-specific way to gateway is up */
/* IP Parameters */
...
/* LAP Parameters */
	...
	voidProcPtr	ip_ph;		/* ptr to IP protocol handler */
	...
	struct	DeferredTask  {
		...
		long		dtParm;
		long		dtReserved;
	}	defer_tx, defer_rx, defer_txcomplete;
...
OSErr 	PPPAttachPH(	LapInfo *, voidProcPtr);
...
/* timer.c */
void	set_timer(TMtimer *, struct fsm_s *, voidProcPtr);
