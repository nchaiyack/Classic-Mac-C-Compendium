...
int				inputRefNum, outputDFRefNum, outputRFRefNum;
...
int OpenInputFile(void)
{
	...
	if (!isHuman)
	{
//		GetEOF(inputRefNum, &blob->sourceDFeof);
...
int SetupTempFile(void)
{
	...
	if (outputDFeof!=0)
	{
		if (gHasFSSpecs)
			...
			isHuman=HOpenDF(tempFS.vRefNum, tempFS.parID,
	...
	{
// not enough room on disk to create temp file.  Check if we will eventually be
// deleting a destination file of the same name; if so, ask user if they want to
// delete that now.
	}
...
void FinalizeFiles(Boolean good)
{
	...
	Mymemset(&theInfo, 0, sizeof(theInfo));
...
unsigned long ReadInputFile(int thisFile, unsigned char* buffer, unsigned long count)
{
	OSErr			isHuman;
	unsigned long	temp;
	...
	return ((isHuman==kEofErr) || (isHuman==noErr)) ? temp : kDiskReadErr;
}

int WriteTempFile(int thatFile, unsigned char* buffer, unsigned long theLength)
{
	return (FSWrite(thatFile, &theLength, buffer)==noErr ? allsWell : kDiskWriteErr);
}
