...
void UpdateTheWindow(ExtendedWindowDataHandle theData)
{
	...
	if (gWhichWipe!=0)
		WipeDispatch(gHasColorQD ? (GrafPtr)gTheGWorld[index] : 
					gBWGrafPtr[index], gTheWindow[index], gBoundsRect[index]);
	else
...
void ShutDownTheGraphics(void)
{
	...
	{											/* is necessary, since these are all */
		...
		if (gTheWindowData[i]!=0L)				/* anyway, but it's still good to Do */
			DisposeHandle(gTheWindowData[i]);	/* The Right Thingª even if it kills */
