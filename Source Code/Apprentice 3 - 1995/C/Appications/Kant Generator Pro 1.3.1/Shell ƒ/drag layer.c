...
static	short HitTest(Point theLoc, WindowRef *theWindow);
static	void DrawCaret(short offset, TEHandle theTE);
#if USE_STYLED_TEXT
static	void InsertTextAtOffset(short offset, char *theBuf, long size, StScrpHandle theStyl,
				TEHandle theTE);
#else
static	void InsertTextAtOffset(short offset, char *theBuf, long size, TEHandle theTE);
#endif
static	short GetSelectionSize(WindowRef theWindow);
static	Ptr GetSelectedTextPtr(WindowRef theWindow);
#if USE_STYLED_TEXT
...
static	Boolean DragText(WindowRef theWindow, EventRecord *theEvent, RgnHandle hiliteRgn);
...
static	pascal OSErr MySendDataProc(FlavorType theType, void *refCon,
							ItemReference theItem, DragReference theDrag)
{
	WindowRef		theWindow=(WindowRef)refCon;
...
pascal OSErr MyReceiveDropHandler(WindowRef duhWindow, unsigned long handlerRefCon,
...
{	OSErr			result;
	...
	SetPort(duhWindow);
	theWindow=(WindowRef)handlerRefCon;
	hTE=GetWindowTE(theWindow);
...
pascal OSErr MyTrackingHandler(short message, WindowRef duhWindow,
							   void *handlerRefCon, DragReference theDrag)
{
	#pragma unused(duhWindow)
	...
	ItemReference		theItem;
	RgnHandle			theRgn;
	WindowRef			theWindow=(WindowRef)handlerRefCon;
	WindowRef			hitWindow;
	...
	switch (message)
	{
		...
		case dragTrackingInWindow:
			...
			if (attributes & dragHasLeftSenderWindow)
			{
				if (PtInRect(localMouse, &(**hTE).viewRect))
				{
...
static	Boolean DragText(WindowRef theWindow, EventRecord *theEvent, RgnHandle hiliteRgn)
{
...
#if USE_STYLED_TEXT
	StScrpHandle	theStyl;
	DragSendDataProc	mySendDataUPP;
#endif
	AEDesc			dropLocation;
	DragAttributes	attributes;
	...
	/*	Now we insert the 'TEXT' data and promise 'styl'
	...
	mySendDataUPP=NewDragSendDataProc(MySendDataProc);
...
#if USE_STYLED_TEXT
	DisposeRoutineDescriptor(mySendDataUPP);
#endif
	...
	if (!(attributes & dragInsideSenderApplication))
	{
		...
		if ((!copyText) && (DropLocationIsFinderTrash(&dropLocation)))
		{
			TEDelete(hTE);
			SetWindowIsModified(theWindow, TRUE);
			if ((vScrollBar=GetWindowVScrollBar(theWindow))!=0L)
			{
...
Boolean DragInWindow(WindowRef theWindow, EventRecord *theEvent)
...
void ResetHiliteRgn(WindowRef theWindow)
{
	TEHandle		hTE;
	RgnHandle		hiliteRgn;
...
Boolean CursorInDraggableRgn(Point thePoint, WindowRef theWindow)
{
	RgnHandle		hiliteRgn;
