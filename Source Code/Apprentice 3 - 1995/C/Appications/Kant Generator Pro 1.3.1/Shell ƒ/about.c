...
static	void DrawTheLogos(WindowRef theWindow, Boolean doFade, short theDepth);
static void DrawTheTextPattern(WindowRef theWindow, MyOffscreenComboPtr offscreenComboPtr,
				short theDepth);

void SetupTheAboutWindow(WindowRef theWindow)
{
...
void OpenTheAboutWindow(WindowRef theWindow)
/* leaks memory on error */
{
	...
	if (gLogoComboPtr==0L)
	{
		...
		if (gLogoComboPtr==0L)
			return;
		SetPortToOffscreenWindow(gLogoComboPtr->offscreenWorldPtr);
		logoPict=DrawThePicture(logoPict, kLogoPictID, 0, 0);
		InvertRect(&logoSourceRect);
		SetPortToOnscreenWindow(theWindow, gLogoComboPtr->offscreenWorldPtr);
		...
		if (gTextComboPtr==0L)
			return;
		SetPortToOffscreenWindow(gTextComboPtr->offscreenWorldPtr);
		FillRect(&(gTextComboPtr->offscreenWorldPtr->offscreenPtr->portRect), &qd.black);
		SetPortToOnscreenWindow(theWindow, gTextComboPtr->offscreenWorldPtr);
...
void IdleInTheAboutWindow(WindowRef theWindow)
{
	if (gDoChangeDepth)
	{
...
void DisposeTheAboutWindow(WindowRef theWindow)
{
...
void KeyDownInAboutWindow(WindowRef theWindow, unsigned char theChar)
{
	#pragma unused(theChar)
	
	CloseTheWindow(theWindow);
}

void MouseDownInAboutWindow(WindowRef theWindow, Point thePoint)
{
...
void ChangeDepthTheAboutWindow(WindowRef theWindow)
{
	#pragma unused(theWindow)
...
void DrawTheAboutWindow(WindowRef theWindow, short theDepth)
{
	#pragma unused(theDepth)
	
	short			titleWidth, versionWidth;
	unsigned char	*titleStr=APPLICATION_NAME;
	
	FillRect(&(theWindow->portRect), &qd.black);
	SetTheDrawingFont(kCalligraphyFont, 12, 0, notSrcCopy);
	titleWidth=StringWidth(titleStr);
	MoveTo((GetWindowWidth(theWindow)-titleWidth)/2, DEAD_SPACE_V+18);
	DrawString(titleStr);
	SetTheDrawingFont(geneva, 9, 0, notSrcCopy);
...
void CopybitsTheAboutWindow(WindowRef theWindow, WindowRef offscreenWindowRef)
{
...
static	void DrawTheLogos(WindowRef theWindow, Boolean doFade, short theDepth)
{
	...
	if (theDepth>=4)
	{
		...
		for (j=startJ; j<=64; j++)
		{
			...
			SetPortToOffscreenWindow(gLogoComboPtr->patternWorldPtr);
			...
			SetPortToOnscreenWindow(theWindow, gLogoComboPtr->patternWorldPtr);
	...
	short theDepth)
{
	...
	SetPortToOffscreenWindow(offscreenComboPtr->patternWorldPtr);
	...
	SetPortToOnscreenWindow(theWindow, offscreenComboPtr->patternWorldPtr);
}
