/*
	...
	Converted to MetroWerks DR3 @MacHack94 -bh
 */
...
typedef struct xQDGlobals xQDGlobals;
...
void DrawColorIcon(short iconId)
{
	...
	/* punch out the mask. */	
	CopyBits(&maskBits, (BitMapPtr)&port->portPixMap, &bounds, &iconRect, srcBic, (RgnHandle)nil);
	
	/* draw the actual color icon. */
	HLock((Handle)source);
	CopyBits((BitMapPtr)*source, (BitMapPtr)&port->portPixMap, &bounds, &iconRect, srcOr, (RgnHandle)nil);
