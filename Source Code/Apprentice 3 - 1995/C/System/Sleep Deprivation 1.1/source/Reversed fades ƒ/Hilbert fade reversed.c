...
pascal void HilbertFadeRecurseReversed(Rect boundsRect, Pattern *thePattern,
	int level, int whichpattern, int* x, int* y);
pascal short HilbertFadeReversed(Rect boundsRect, Pattern *thePattern);
	...
	int level, int whichpattern, int* x, int* y)
{
	...
	for (i=0; i<11; i++)
	{
		switch (HilbertPattern[whichpattern][i])
		{
			...
			case 0x42:    /* call X */
				if (level>1)
					HilbertFadeRecurseReversed(boundsRect,thePattern,level-1,0,x,y);
				break;
			case 0x69:    /* call Y */
				if (level>1)
					HilbertFadeRecurseReversed(boundsRect,thePattern,level-1,1,x,y);
				break;
		}
	}
}
...
pascal short HilbertFadeReversed(Rect boundsRect, Pattern *thePattern)
{
	...
	curx=0;
	HilbertFadeRecurseReversed(boundsRect,thePattern,RecursionLevel,0,&curx,&cury);
