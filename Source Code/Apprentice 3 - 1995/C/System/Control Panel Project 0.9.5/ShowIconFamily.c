/*
	...
	and for use as a separate code resource.
	
	Modified 11/19/93 by Ken Worley to remove any necessity for statics and
	globals.  No longer requires A4 register to be set up.
	
	Instructions for use:
	
		¥ Create a family of icons with ResEdit 2.1 or later.  This will
			include 'ICN#', 'icl4', & 'icl8' icons.
		
		To use within a larger INIT project:
			¥ #define SEPARATE_CODE 0  below.
			¥ Use SetUpA4 to set up Think C globals.	** not required - kw **
			¥ Call ShowIconFamily() with the resource id of the family
				that you used.
		...
		To use as a separate code resource:
			...
			¥ In your main INIT code, call ShowIconFamily like so:
...
static void		GetIconRect( register Rect* iconRect, GrafPtr port );
static void		Next_position( void );
static void		DrawBWIcon( short iconId, GrafPtr port );
static Handle	ChooseIcon( short iconId, short* suggestedDepth );
static void		DrawColorIcon( short iconId, CGrafPtr port, short theDepth );

/* this is where it all happens. */

#if SEPARATE_CODE
void main(short iconId)
#else
...
#endif
{
	...
	short		theDepth;			/* the depth the monitor is in */	/* kw */
	...
	if (theDepth == 1)
		DrawBWIcon( iconId, (GrafPtr)&gp );
	else
		DrawColorIcon( iconId, &gp, theDepth );
...
/*
	...
	kw - I modified this routine to take the extra 'port' argument.
 */
...
/* kw - I modified this routine to take the extra arguments 'port' and 'theDepth' */
...
static void DrawColorIcon(short iconId, CGrafPtr port, short theDepth)
{
	...
	if (!icon) {
		DrawBWIcon(iconId, (GrafPtr)port);
		return;
	}
	HLock(icon);
	...
	GetIconRect(&iconRect, (GrafPtr)port);
	...
	/* punch out the mask. */	
	CopyBits(&maskBits, (BitMap*)&port->portPixMap, &bounds, &iconRect, srcBic, nil);
	
	/* draw the actual color icon. */
	HLock((Handle)source);
	CopyBits((BitMap*)*source, (BitMap*)&port->portPixMap, &bounds, &iconRect, srcOr, nil);
