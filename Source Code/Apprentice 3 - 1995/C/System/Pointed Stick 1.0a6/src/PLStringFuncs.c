...
pascal short		PLstrcmp(StringPtr str1, StringPtr str2)
{
	...
	// -1, 0, or 1), but itÕs not as short as calling the Toolbox.
	//
	short			result;
	...
	*/
	...
	asm
	{
		...
		swap	D0
		move.b	(A1)+,D0
		RelString	CASE		; RelString(É, É, TRUE, FALSE) {case sensitive}
		move.w	D0,16(A6)
	}
#endif
#endif
...
pascal short		PLstrcompare(StringPtr str1, StringPtr str2)
{
	...
	asm
	{
		...
		swap	D0
		move.b	(A1)+,D0
		RelString				; RelString(É, É, FALSE, FALSE) {all insensitive}
		move.w	D0,16(A6)
	}
#endif
}
