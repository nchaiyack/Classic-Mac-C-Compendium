...
{	int		i;
	...
	// and fill in the details for the code we want installed.
		...
		// We install one Shutdown task, with no address table.
		gTheParamBlock->installAddressTable		= false;
		gTheParamBlock->numCodeResources		= 1;


		// Details for a Shutdown task
		gTheParamBlock->theCodeResources[kShutdownFade].resType		= kFadeResType;
		gTheParamBlock->theCodeResources[kShutdownFade].resID		= kFadeResID;
		gTheParamBlock->theCodeResources[kShutdownFade].codeType	= kShutdownTaskType;
		gTheParamBlock->theCodeResources[kShutdownFade].theCodeThing.theShutdownTask.theFlags = sdOnPowerOff;
		}
}
