...
ZTextItem::ZTextItem(Ptr inName,ZList *inList)
{
	mText = new unsigned char[GetPtrSize(inName)+1];
	if(mText)
	{
		BlockMoveData(inName,&mText[1],GetPtrSize(inName));
		mText[0] = GetPtrSize(inName);
	}
	else
		;//throw exception
