/************************************************************************\
	...
	Tab length is set to 32 pixels for speed, as suggested by Mark Valence.
				...
				JustStyleCode styleRunPosition, WEHandle hWE)
{
	...
	for (ii = 0; ii < textLength; ii++)
	{
		if (pText[ii] == '\t')
		{
			...
			GetPen(&penPos);
     		MoveTo(destLeft + ((penPos.h - destLeft + 33) & 0xffe0), penPos.v);
			beginChar = ii + 1;
		}
	}
	...
	for (ii = 0; ii < textLength && *width > 0; ii++)
	{
		if (pText[ii] == '\t')
		{
			...
			if (*width >= 0)
			{
				...
				tabWidth = destLeft + ((hPos - destLeft + 0x00210000L) & 0xffe00000L) - hPos;
				hPos += tabWidth;
				if (*width - tabWidth < 0)
				{
	...
	for (ii = 0; ii < textLength && offset > ii; ii++)
	{
		if (pText[ii] == '\t')
		{
			width += CharToPixel(pText + beginChar, ii - beginChar, slop,
							...
							kOneToOneScaling);
			beginChar = ii + 1;
     		width += destLeft + ((hPos + width - destLeft + 33) & 0xffe0) - hPos - width;
		}
	}
