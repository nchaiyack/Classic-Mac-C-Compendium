...
pascal OSErr OpenResolver(char *fileName)
{
	short refnum;
	OSErr rc;
...
pascal OSErr StrToAddr(char *hostName, struct hostInfo *rtnStruct, 
...
pascal OSErr AddrToStr(unsigned long addr, char *addrStr)
{
	if (dnr == nil)
		/* resolver not loaded error */
		return(notOpenErr);
...
pascal OSErr EnumCache(EnumResultUPP enumResultProc, char *userDataPtr)
{
	if (dnr == nil)
		/* resolver not loaded error */
		return(notOpenErr);
...
pascal OSErr AddrToName(unsigned long addr, struct hostInfo *rtnStruct, 
				ResultUPP resultProc, char *userDataPtr)
{
	if (dnr == nil)
		/* resolver not loaded error */
...
typedef OSErr (*HInfoProcPtr)(long selector, char* hostName, struct returnRec* returnRecPtr,
								Result2UPP resultProc, char* userData);
...
pascal extern OSErr HInfo(char *hostName, struct returnRec *returnRecPtr, 
		Result2UPP resultProc, char *userDataPtr)
{
	if (dnr == nil)
		/* resolver not loaded error */
		return(notOpenErr);
...
pascal extern OSErr MXInfo(char *hostName, struct returnRec *returnRecPtr, 
					Result2UPP resultProc, char *userDataPtr)
{
	if (dnr == nil)
		/* resolver not loaded error */
