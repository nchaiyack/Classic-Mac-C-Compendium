...
extern void AdjustOffscreenPixmap(CGrafPort *color_port, Rect *bounds);
extern Boolean CreateGraph(char *filename);
...

{
	...
	if (command > cmdShowControlsWindowBase)					//  Show or hide a controls window
		...
		CControlsDirector *director =
					controls_directors.NthItem(dimension);		//  Find the director of window	
...
void CHyperCuberDoc::DoKeyDown(char theChar, Byte keyCode, EventRecord *macEvent)
{
	...
	for (i = 0; i <= gPrefs->prefs.num_key_controls; i++)		//  Loop through all key controls
		...
		if ((key_controls[i].key_code == keyCode) &&
			...
			{
			KeyControl key_control = key_controls[i];			//  Get the key control record
			
			if (key_control.dimension > graphic.dimension)
				continue;										//  Don't rotate through a dimension
																//   greater than than of the graphic
...
//|~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
//| CHyperCuberDoc::NewFromEquation
//|
//| Purpose: Create a new n-dimensional object from the mathematical equation
//|
//| Parameters: none
//|___________________________________________________________________________

Boolean CHyperCuberDoc::NewFromEquation(void)
{

#define GRAPH_FILENAME	"Graph"

	Boolean error = CreateGraph(GRAPH_FILENAME);	//  Create the graph

	if (!error)
		OpenFileFromFilename(GRAPH_FILENAME);		//  Open the file

	remove(GRAPH_FILENAME);							//  Delete the file
...
void CHyperCuberDoc::OpenFileFromFilename(char *filename)
{
	...
	controls_directors.Append(NULL);					// Add null items for dimensions 1 and 2
	controls_directors.Append(NULL);
...
#define	TWO_COLOR		129
	...
	AdjustOffscreenPixmap(&theMainPane->OffscreenPort,//  Make offscreen bitmap large enough
									&pane_bounds);
