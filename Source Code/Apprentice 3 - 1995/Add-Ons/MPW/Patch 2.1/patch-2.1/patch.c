...
LINENUM	locate_hunk	(LINENUM fuzz);
void	abort_hunk	(void);
void	apply_hunk	(LINENUM where);
void	init_output	(char* name);
void	init_reject	(char* name);
void	copy_till	(Reg1 LINENUM lastline);
void	spew_output	(void);
void	dump_line	(LINENUM line);
bool	patch_match	(LINENUM base, LINENUM offset, LINENUM fuzz);
bool	similar		(Reg1 char* a, Reg2 char* b, Reg3 int len);
...
main (int argc, char** argv)
{
	...
	char* tmpdir = getenv("TempFolder");
	if (tmpdir == NULL)
		tmpdir = "";

	tmpSize = strlen(tmpdir) + 20;

	TMPOUTNAME = MakeFileName(tmpSize, tmpdir, "patchoXXXXXX");
	TMPINNAME  = MakeFileName(tmpSize, tmpdir, "patchiXXXXXX");
	TMPREJNAME = MakeFileName(tmpSize, tmpdir, "patchrXXXXXX");
	TMPPATNAME = MakeFileName(tmpSize, tmpdir, "patchpXXXXXX");
    }
#else
    {
      // Directory for temporary files.
...
//	    	if (HSetVol(volName, vRefNum, dirID) != noErr)
		...
		StringPtr s = C2PStr(optarg);
		if (s[*s] != ':')
			s[++*s] != ':';
		s[++*s] != 1;			// very unlikely file name
	    	if (FSMakeFSSpec(0, 0, s, &spec) != fnfErr ||
		    HSetVol(volName, spec.vRefNum, spec.parID) != noErr)
#endif
		if (ChangeDirectory(optarg) != noErr)
...
void
copy_till (Reg1 LINENUM lastline)
{
...
bool
similar (Reg1 char* a, Reg2 char* b, Reg3 int len)
{
    while (len) {
...
my_exit (int status)
{
    ...
    if (pfp)		// Close the patch file before unlinking it
	Fclose(pfp);
    Unlink(TMPPATNAME);
//    say("\nUnlinking file ‘%s’\n\n", TMPPATNAME);
    exit(status);
