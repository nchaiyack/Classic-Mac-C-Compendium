...
// Translate a unix file path into a Mac file path (if necessary).
char*
SavePath (const char* path)
{
#if 0
	char* d = xmalloc(strlen(path) + 2);	// In case needs leading ':'
	char* result = d, c;

	if (*path == ':') {	// Must be a mac path
		strcpy(d, path);
	} else {
		if (*path == '/')
			++path;
	...
	return result;
#else
	return (*path == ':') ? savestr(path) : UnixToMac(path);
#endif
}
...
intuit_diff_type()
{
	...
	if (!stars_last_line && strnEQ(s, "*** ", 4))
	    ...
	    oldtmp = SavePath(s+4);	/* pretend it is the old name */
...
{
#ifdef macintosh
	fatal("Cannot do_ed_script(): ÔedÕ is unavailable\n");
#else
    Reg1 char *t;
